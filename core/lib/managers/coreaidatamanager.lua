...
-- Should return a path with only one point in the correct format
function AiDataManager:destination_path( position, rotation )
	return { points = { { position = position, rotation = rotation } } }
end
...
function AiDataManager:save_data()
	...
	-- Fix data here by removing units before saving
end
