...
--------------------------------------------------
-- SubtitlePresenter base class
--------------------------------------------------

function SubtitlePresenter:destroy()
	-- Overload to clean up your stuff.
end

function SubtitlePresenter:update(time, delta_time)
	-- Overload to implement fade effects, etc.
end

function SubtitlePresenter:show()
	-- Overload to show the subtitle overlay.
end

function SubtitlePresenter:hide()
	-- Overload to hide the subtitle overlay.
end

function SubtitlePresenter:show_text(text, duration)
	-- Overload to present text on the screen.
	-- The duration is just a hint for doing transitions, etc. - The SubtitleSequencePlayer will call show_text with an empty string when the duration has expired.
end

function SubtitlePresenter:preprocess_sequence(sequence)
	-- Overload to preprocess the SubtitleSequence.
	-- This is useful if you have limited screen space to display strings, and need to split up long strings into multiple show_text() calls.
	return sequence
end
...
function OverlayPresenter:set_font(font_name, font_size)
	...
	-- Fonts must be preloaded through packages or through the editor interface
	-- Overlay:newgui():preload_font(self.__font_name)
...
function OverlayPresenter:set_width(pixels)
	...
	local safe_width = self:_gui_width() -- * safe_rect.width
...
function OverlayPresenter:show_text(text, duration)
	local label = self.__subtitle_panel:child("label") or self.__subtitle_panel:text{ name = "label", x = 1, y = 1, font = self.__font_name, font_size = self.__font_size, color = Color.white, align = "center", vertical = "bottom", layer = 1, wrap = true, word_wrap = true }
	local shadow = self.__subtitle_panel:child("shadow") or self.__subtitle_panel:text{ visible = false, name = "shadow", x = 2, y = 2, font = self.__font_name, font_size = self.__font_size, color = Color.black:with_alpha(0.5), align = "center", vertical = "bottom", layer = 0, wrap = true, word_wrap = true }
	label:set_text(text)
...
function OverlayPresenter:_clear_workspace()
	...
	-- self.__ws = Overlay:newgui():create_scaled_screen_workspace(self:_gui_width(), self:_gui_height(), 0, 0, RenderSettings.resolution.x)
...
function OverlayPresenter:_split_string_into_lines(subtitle_string, owning_sequence)
	-- This can be overloaded in game-specific implementations, at which point the owning_sequence parameter might become useful.
	-- The default implementation simply wraps the text to the layout field width, controlled by OverlayPresenter:set_width()
...
function OverlayPresenter:_auto_word_wrap_string(subtitle_string)
	...
	local line_breaks = table.collect(layout_text_field:line_breaks(), function(index) return index + 1 end) -- GuiText:line_breaks() returns zero-based indexes.
...
function OverlayPresenter:_layout_text_field()
	...
	return self.__subtitle_panel:child("layout") or self.__subtitle_panel:text{ name = "layout", width = self.__width, visible = false, font = self.__font_name, font_size = self.__font_size, align = "center", vertical = "bottom", wrap = true, word_wrap = true }
end

function OverlayPresenter:_string_width(subtitle_string)
	local string_width_measure_text_field = self.__ws:panel():child("string_width") or self.__ws:panel():text{ name = "string_width", visible = false, font = self.__font_name, font_size = self.__font_size, wrap = false }
...
function OverlayPresenter:_on_resolution_changed()
	self:set_font(self.__font_name or self:_default_font_name(), self.__font_size or self:_default_font_size() ) -- , 28 * _G[ "tweak_data" ].scale.subtitle_font_multiplier  )
	local width = self:_gui_width()
	local height = self:_gui_height()
	local safe_rect = managers.gui_data:corner_scaled_size() -- managers.viewport:get_safe_rect()
		
	managers.gui_data:layout_corner_saferect_workspace( self.__ws )
	-- self.__ws:set_screen(width, height, 0, 0, RenderSettings.resolution.x)
	-- self.__subtitle_panel:set_debug( true )
	-- self.__subtitle_panel:set_width(width * safe_rect.width)
	self.__subtitle_panel:set_width( safe_rect.width )
	-- self.__subtitle_panel:set_height(height * safe_rect.height - 320 * _G[ "tweak_data" ].scale.subtitle_pos_multiplier ) -- Fast fix offset from bottom /Martin
	self.__subtitle_panel:set_height( safe_rect.height - 120 )
	self.__subtitle_panel:set_x( 0 )
	self.__subtitle_panel:set_y( 0 )
	--self.__subtitle_panel:set_center_x(width * 0.5)
	-- self.__subtitle_panel:set_center_y(height * 0.5)
	
	self:set_width(--[[self.__width or]] self:_string_width("The quick brown fox jumped over the lazy dog bla bla bla bla bla bla bla bla bla blah blah blah blah blah ."))
...
function OverlayPresenter:_gui_width()
	return self.__subtitle_panel:width() -- RenderSettings.resolution.x
end

function OverlayPresenter:_gui_height()
	-- local resolution = RenderSettings.resolution
	return self.__subtitle_panel:width() -- self:_gui_width() / (resolution.x / resolution.y)
end
