...
function Shape:_create_size_ctrl( name, property, value, parent, sizer )
	local ctrl_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		spin:connect( "EVT_SCROLL_LINEUP", callback( self, self, "update_size_spin" ), { ctrl = ctrl, property = property, step = 0.1 } )
		spin:connect( "EVT_SCROLL_LINEDOWN", callback( self, self, "update_size_spin" ), { ctrl = ctrl, property = property, step = -0.1 } )
...
-- Creates a string formatet for saving to xml
...
-- Creates a table to be saved to level file
...
function ShapeBoxMiddle:is_inside( pos )
	...
	-- local z = rot:z() * self._properties.height/2
	
	-- local position = self:position() - x - y - z
...
function ShapeBoxMiddle:draw( t, dt, r, g, b )
	...
	-- local c2 = c1 + rot:x()*self._properties.width + rot:y()*self._properties.depth + rot:z()*self._properties.height
