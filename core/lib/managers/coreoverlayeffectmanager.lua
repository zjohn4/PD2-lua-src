-- OverlayEffectManager can play effect in a simple overlay plane, it can have different colors, fades and durations.
-- Todo: Add support for bitmaps
...
-- Initailize the GUI
	...
	self._presets = {}		-- Can contain preset effects, these are the ones that the editor will display for example
	self:add_preset( "custom", { blend_mode = "normal", sustain = 0.0, fade_in = 0, fade_out = 0.0, color = Color( 1.0, 0, 0, 0 ) } )
	
	self:set_default_layer( 30 )
	
-- Set up the callback that handles a change in resolution
...
function OverlayEffectManager:play_effect( data )
	if data then	
		...
		if data.gradient_points then
			rectangle = self._ws:panel():gradient( { w = RenderSettings.resolution.x, h = RenderSettings.resolution.y, color = data.color:with_alpha( spawn_alpha ), gradient_points = data.gradient_points, orientation = data.orientation } )
		else
			rectangle = self._ws:panel():rect( { w = RenderSettings.resolution.x, h = RenderSettings.resolution.y, color = data.color:with_alpha( spawn_alpha ) } )
		end 
		...
		local effect = { rectangle = rectangle, start_t = ( data.timer or TimerManager:game() ):time(), data = {}, current_alpha = spawn_alpha, gradient_points = data.gradient_points }
		
		for key, value in pairs( data ) do	--Keep copies of the effect data because we might need to change them
