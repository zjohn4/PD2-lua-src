...
function EnvironmentUnderlayFeeder:feed( nr, scene, vp, data, block, ... )
	...
	if args[1] == "underlay_effect" then
		if Underlay:loaded() then
			...
			--local material = assert(Underlay:material(material_name), "[EnvironmentUnderlayFeeder] Could not find material " .. material_name .. " in Underlay!")
			local material = Underlay:material(Idstring(material_name))
			if not material then return true end
			
			for k,v in pairs(block) do
				local value = v
				if k == sun_color_scale or
					k == color0_scale or
					k == color1_scale or
					k == color2_scale or
					k == color_opposite_sun_scale or
					k == color_sun_scale or
					k == sky_intensity or
					k == sky_intensity
				then
