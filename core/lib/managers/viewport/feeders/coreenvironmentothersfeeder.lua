...
-- Changed to new animation group name : Ulf
			-- Global._global_light:link( Underlay:get_object( Idstring("anim_sun") ) )			
		...
		if not CoreCode.alive( Global._underlay_ref_camera ) then
			...
			Global._underlay_ref_camera:set_near_range( 1000.0 )
			Global._underlay_ref_camera:set_far_range( 10000000.0 )
			Global._underlay_ref_camera:set_fov( 75.0 )
		...
		-- Global._global_light:set_color( sun_ray_color * (sun_ray_color_scale/640)*16 )
...
--[[	   	if Underlay:get_object( Idstring("anim_sun_horizontal") ) then
...
-- Changed to new animation group name : Ulf
	...
	elseif args[1] == "sky_orientation" then
		...
		if nr == 1 then
			-- UGLY, NEED TO ROTATE BOTH SKYSPEHERE AND SKY - SHOULD BE FIXED
			-- local object = Underlay:get_object( Idstring("skysphere") ) or Underlay:get_object( Idstring("g_skysphere") )
			-- object:set_rotation( Rotation( -block["rotation"], 0, 0 ) )
			-- Underlay:get_object( Idstring("sky") ):set_rotation( Rotation( -block["rotation"], 0, 0 ) )
