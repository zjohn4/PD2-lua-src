...
function EnvironmentNetworkFeeder:feed( nr, scene, vp, data, block, ... )
	if nr == 1 then -- Check if this is the first viewport?
		if assert(managers.slave:type()) == "slave" then
			...
			self:send( self._block_nr, block, {...},  self._peer)
		end
...
function EnvironmentNetworkFeeder:pack_data( block, params )
	...
	for k,v in pairs(block) do
		...
		bstr = string.format("%s%s=", bstr, string.match(k, "[%w_]+")) -- This filter realy should be an assert. /AJ
