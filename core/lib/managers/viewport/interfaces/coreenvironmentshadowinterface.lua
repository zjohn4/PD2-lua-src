...
EnvironmentShadowInterface.DATA_PATH = {"post_effect", "shadow_processor", "shadow_rendering", "shadow_modifier"} 
...
function EnvironmentShadowInterface:_process_return(block)
	if ( block and block["d0"] and block["d1"] and block["d2"] and block["d3"] and
			block["o1"] and block["o2"] and block["o3"] ) then
	...
	elseif( block["slice0"] and block["slice1"] and block["slice2"] and block["slice3"] and block["shadow_slice_depths"] and
			block["shadow_slice_overlap"] and block["shadow_fadeout"] ) then 
		return block 
	else
