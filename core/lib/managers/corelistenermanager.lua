...
-- Set id's are activated and deactivated for certain stack id's.
-- All categories in a set are active when the set is active.
-- The top most set in all stacks are active at the same time so stack id's are mostly used when displaying multiple viewports.
ListenerManager = ListenerManager or class()

function ListenerManager:init()
	self._set_map = {}						-- { set_id = { category_id = { listener_id = listener } } }
	self._category_map = {}					-- { category_id = { listener_id = listener } }
	self._listener_map = {}					-- { listener_id = { category_id = category_id, active = active } }
	self._active_category_map = {}			-- { category_id = active }, Used to determine whether a category is currently active or not.
	self._active_set_count_map = {}			-- { set_id = active_set_count }
	self._active_set_stack_map = {}			-- { stack_id = CoreLinkedStack }, The active set id's at the top of the stacks are the currently active ones. It is added to the top of the stack when a set id is activated and removed from where ever in the stack when deactivated.
	self._activation_map = {}				-- activation_id = { "stack_id" = stack_id, "set_id" = set_id, "link_id" = link_id }
	self._stack_activation_map = {}			-- stack_id = activation_id
	self._set_activation_map = {}			-- set_id = activation_id
...
function ListenerManager:set_enabled( enabled )
	enabled = not not enabled	-- Convert to bool.

	if( self._enabled ~= enabled ) then
		-- self:callback_on_all_active_listeners( function( listener_id ) Sound:set_listener_enabled( listener_id, enabled ) end )
		self:callback_on_all_active_listeners( function( listener_id ) self._listener_map[ listener_id ].listener:activate( enabled ) end )
...
--[[
	category_id: Which category it belongs to. Creates the category if it doesnt exist.
	distance_obj: Object used to determine distance.
	orientation_obj: Object used to determine the rotation. (Defaults to distance_obj)
	occlusion_obj: Object used to determine whether sounds are occluded or not. (Defaults to distance_obj)
	disabled: Whether it is initially disabled or not.
]]
function ListenerManager:add_listener( category_id, distance_obj, orientation_obj, occlusion_obj, disabled )
	...
	local listener_map = self._category_map[ category_id ]
	-- local listener_id = Sound:add_listener( distance_obj, orientation_obj, occlusion_obj )
	...
	local listener_data = { listener = listener, 
	...
	if( not self._enabled or disabled or not self._active_category_map[ category_id ] ) then
		-- Sound:set_listener_enabled( listener_id, false )
	...
	if( listener ) then
		...
		if( listener_map ) then
			if( listener_map[ listener_id ] ) then
				-- Sound:remove_listener( listener_id )
...
function ListenerManager:set_listener( listener_id, distance_obj, orientation_obj, occlusion_obj )
	...
	if( listener ) then
		...
		if( listener_map ) then
			if( listener_map[ listener_id ] ) then
				-- Sound:set_listener( listener_id, distance_obj, orientation_obj, occlusion_obj )
...
function ListenerManager:set_listener_enabled( listener_id, enabled )
	...
	if( data ) then
		...
		if( self._active_category_map[ data.category_id ] ) then
			...
			-- Sound:set_listener_enabled( listener_id, self._enabled and enabled )
		end
	else
		Application:stack_dump_error( "Unable to set non-existing listener id \"" .. tostring( listener_id ) .. "\"." )
	end
...
function ListenerManager:_set_listener_set_active( set_id, active )
	for category_id,listener_map in pairs( self._set_map[ set_id ] ) do
		...
		for _,data in pairs( listener_map ) do
			...
			data.active = active
			-- Sound:set_listener_enabled( listener_id, self._enabled and listener.enabled and active )
		end
	end
end
