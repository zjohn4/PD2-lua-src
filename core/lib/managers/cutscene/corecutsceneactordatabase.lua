...
function CoreCutsceneActorDatabaseUnitTypeInfo:_append_unit_info(unit)
	...
	if self._object_names == nil then
		self._object_names = table.collect(unit:get_objects("*"), function(object) return object:name() end)
	...
	if self._object_visibilities == nil then
		self._object_visibilities = table.remap(unit:get_objects("*"), function(_, object) return object:name(), (object.visibility and object:visibility() or nil) end)
	end
...
function CoreCutsceneActorDatabaseUnitTypeInfo:_argument_names_for_function(func)
	-- This is only used in the cutscene editor, so there is no point in doing all that work in public builds.
