...
--
-- Required members in host class:
--   self:_all_keys_sorted_by_time() - List of cutscene keys sorted by time.
--------------------------------------------------
...
function CoreCutsceneKeyCollection:first_key(time, element_name, properties)
	for index, key in ipairs(self:_all_keys_sorted_by_time()) do
		if key:time() >= time then
			if element_name == nil or element_name == key.ELEMENT_NAME then
				if properties == nil or table.true_for_all(properties, function(value, attribute_name) return key[attribute_name](key) == value end) then
					return key, index
				end
			end
		end
