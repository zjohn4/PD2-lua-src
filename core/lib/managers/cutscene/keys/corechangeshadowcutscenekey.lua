...
function CoreChangeShadowCutsceneKey:init( keycollection )
	...
	self._modify_func = 
		function(interface)
			return managers.viewport:first_active_viewport():environment_mixer():static_parameters( 
					self:name(), "post_effect", "shadow_processor", "shadow_rendering", "shadow_modifier" )
		end
end
