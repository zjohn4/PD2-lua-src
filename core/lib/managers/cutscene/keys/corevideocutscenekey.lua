...
function CoreVideoCutsceneKey:_play_video(video_ws)
	if not alive(self._video_object) or self:video() ~= self._video_played or self:loop() ~= self._loop_played or self:speed() ~= self._speed_played then
		...
		video_ws:show()
		video_ws:panel():rect{ width = video_ws:width(), height = video_ws:height(), color = Color.black }

		self._video_object = video_ws:panel():video{ video = self:video(), loop = self:loop() > 0, speed = self:speed(), layer = self:gui_layer() }
