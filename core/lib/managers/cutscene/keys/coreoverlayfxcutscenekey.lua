...
function CoreOverlayFXCutsceneKey:preroll(player)
	if self:fade_in() == 0 then
		...
		-- Set up the effect to sustain at full intensity until we receive further instructions.
...
function CoreOverlayFXCutsceneKey:evaluate(player, fast_forward)
	local effect_data = table.remap(self:attribute_names(), function(_, attribute_name) return attribute_name, self:attribute_value(attribute_name) end)
	managers.cutscene:play_overlay_effect(effect_data)
end
...
function CoreOverlayFXCutsceneKey:_on_pick_color(sender)
	if self.__color_picker_dialog == nil then
		...
		self.__color_picker_dialog:connect("EVT_CLOSE_WINDOW", function() self.__color_picker_dialog = nil end)
...
function CoreOverlayFXCutsceneKey:_effect_data()
	return table.remap(self:attribute_names(), function(_, attribute_name) return attribute_name, self:attribute_value(attribute_name) end)
end
