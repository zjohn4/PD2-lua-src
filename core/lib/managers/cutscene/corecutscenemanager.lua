...
--------------------------------------------------
-- Global state - Survives level unloads
--------------------------------------------------
...
--------------------------------------------------
-- CoreCutsceneManager instance methods
--------------------------------------------------
...
function CoreCutsceneManager:register_cutscene_actor(unit)
	...
	if existing_unit ~= nil then
		-- Return false if the name is already taken by another unit.
...
function CoreCutsceneManager:cutscene_actors_in_world()
	...
	-- Clear out any dead units.
	local dead_units = table.collect(self._cutscene_actors, function(unit) return (not alive(unit)) or nil end)
...
-- This fuction starts a cutscene the next time the script is initialized. (Use for debug only!)
function CoreCutsceneManager:debug_next_exec(scene_name)
	self:delay_cutscene_debug()
	Global.debug_cutscene = scene_name
end

-- This flag should be set in order to delay Global.debug_cutscene to the next exec.
function CoreCutsceneManager:delay_cutscene_debug()
	self._delay_cutscene_debug = true
end

-- This is pure debug! Global.debug_cutscene can be set before doing 
-- a script exec. Ex. when doing a level jump from the menu for doing debug.
function CoreCutsceneManager:start_delayed_cutscene()
	...
	if not self._delay_cutscene_debug and debug_scene then
		...
		self:connect("EVT_PLAYBACK_FINISHED", function() managers.cutscene._manager_locked = nil end)
	end
end

function CoreCutsceneManager:update()
	-- Hello, I've disabled this since we are not using it in current project. Don't have time to analyze why cutscene needs
	-- to do thing when its not doing anything (0.9% on PS3) /Martin Waern 110208
	if true then
		return
	end
	...
	-- Not a nice solution. This should be handled by the TransitionManager but it's crapy!
	-- This function is for debug only. (Can be excluded in final.)
...
function CoreCutsceneManager:_create_gui_workspace()
	-- You can override this in your game-specific CutsceneManager subclass.
	return nil
end
...
--- Primes playback of the specified cutscene at the supplied time offset. If the cutscene has not been prepared, stalls until all assets are loaded.
...
--- Primes and starts playback of the specified cutscene.
function CoreCutsceneManager:play_cutscene(name)
	-- This is another hack because the mission script is uncontrollable once started.
	-- Good! I hope "Mad Mats" doesnt see this one.
...
--- Starts playback of the primed cutscene, or resumes playback of a paused cutscene.
...
--- Stops playback at the beginning of the next update call.
...
-- Skips the remainder of the cutscene.
...
--- Stops playback, hides cutscene units, and returns to normal in-game camera control.
...
--- Freezes playback, but does not relinquish control of the camera or hide any cutscene units.
...
--- Evaluates a single frame from the specified cutscene. If the cutscene has not been prepared, stalls until all assets are loaded.
...
function CoreCutsceneManager:_on_playback_started(cutscene_name)
	-- Override in subclasses. Will trigger before any connected EVT_PLAYBACK_STARTED events.
end

function CoreCutsceneManager:_on_playback_finished(cutscene_name)
	-- Override in subclasses. Will trigger after any connected EVT_PLAYBACK_FINISHED events.
end
