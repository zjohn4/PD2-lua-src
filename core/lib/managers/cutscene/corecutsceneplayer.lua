...
CoreCutscenePlayer.BLACK_BAR_GUI_LAYER = 29 -- Below the default layer for the OverlayEffectManager, though note this can be changed with managers.overlay_effect:set_default_layer().
...
function CoreCutscenePlayer:init(cutscene, optional_shared_viewport, optional_shared_cast)
	...
	self._dof_environment_modifier = assert(self._viewport:environment_mixer():create_modifier(false, "dof", function(...) return self:_dof_modifier_cb(...) end), "[CoreCutscenePlayer] Environment modifier alredy created by: " .. tostring(self._viewport:environment_mixer():modifier_owner("dof")))
...
function CoreCutscenePlayer:add_keys(key_collection)
	...
	-- Create a copy of each cutscene key, so we can modify it during playback.
	for _, template_key in ipairs(key_collection:_all_keys_sorted_by_time()) do
		if self:_is_driving_sound_key(template_key) then
			...
			local cutscene_key = template_key:clone() -- The clone method returns an unfrozen copy of the key.
...
function CoreCutscenePlayer:_driving_sound_offset()
	if alive(self._driving_sound_instance) then
		...
		if master_sound_instance == nil then
			return 0 -- This is a sequence sound that hasn't started yet.
		elseif not alive(master_sound_instance) or not master_sound_instance:is_playing() then
			return nil -- This sound has finished playing.
		end
		
		local name, offset = master_sound_instance:offset()
		
		-- HACK: Watch for bad return values from SoundInstance:offset().
		-- There is a bug that causes the cutscene playhead to jump around randomly, as it is driven by the sound offset.
		-- This code filters the return value so we don't get unexpected jumps. The next frame should be ok, though.
...
function CoreCutscenePlayer:unload()
	...
	-- Call unload() on keys in reverse order.
...
function CoreCutscenePlayer:destroy()
	...
	-- Invoke on_cutscene_player_destroyed() callback on owned Gui objects.
...
function CoreCutscenePlayer:refresh()
	-- Correct exposure during camera cuts.
...
function CoreCutscenePlayer:preroll_cutscene_keys()
	...
	for _, cutscene_key in ipairs(self:_all_keys_sorted_by_time()) do
		if cutscene_key:frame() > 0 then break end
...
function CoreCutscenePlayer:set_camera_attribute(attribute_name, value)
	...
	func(camera, value)
	func(self._future_camera, value) -- Assumes the function pointer to be identical.
end
...
function CoreCutscenePlayer:set_camera_depth_of_field(near, far)
	...
	self._dof_attributes.near_focus_distance_min = math.max(0.000001, near - range * 0.33)
	self._dof_attributes.near_focus_distance_max = math.max(0.000001, near)
...
function CoreCutscenePlayer:play_camera_shake(shake_name, amplitude, frequency, offset)
	...
	return function() self._viewport:director():shaker():stop_immediately(shake_id) end
end
	...
	if alive(gui_object) and gui_object:has_script() then
		...
		if type(callback_func) == "function" then
			if Application:production_build() then
				local argument_string = table.concat(table.collect({ ... }, function(arg) return type(arg) == "string" and string.format("%q", arg) or tostring(arg) end), ", ")
...
function CoreCutscenePlayer:_gui_panel(gui_name, preloading)
	...
	if panel == nil then
		...
		panel = self._workspace:panel():panel{ name = gui_name, visible = false, halign = "grow", valign = "grow", x = viewport_rect.px, y = viewport_rect.py, width = viewport_rect.pw, height = viewport_rect.ph }
...
function CoreCutscenePlayer:set_key_handler(delegate)
	-- When using the cutscene editor, we want all cutscene keys to go through there,
	-- as there are certain things we don't want to happen during scrubbing, etc.
	-- This allows us to intercept those method calls.
	self._key_handler = delegate
end


-------------------------------------------------------------
-- These methods can be overridden by assigning a key handler
-------------------------------------------------------------
...
function CoreCutscenePlayer:_wide_viewport_rect()
	...
	local cutscene_aspect = 9 / 16	
	local viewport_width = math.min(resolution_aspect / cutscene_aspect, 1.0)
...
function CoreCutscenePlayer:_clear_workspace()
	...
	self._workspace:panel():rect{ visible = self._widescreen, layer = self.BLACK_BAR_GUI_LAYER, name = self.BLACK_BAR_TOP_GUI_NAME, color = Color.black }
	self._workspace:panel():rect{ visible = self._widescreen, layer = self.BLACK_BAR_GUI_LAYER, name = self.BLACK_BAR_BOTTOM_GUI_NAME, color = Color.black }
...
function CoreCutscenePlayer:_configure_viewport()
	...
	-- We use a full-screen viewport, to get past the viewport rendering bugs present in the engine at the time.
	if alive(self._workspace) then
		local resolution = RenderSettings.resolution
		...
		self._workspace:panel():child(self.BLACK_BAR_TOP_GUI_NAME):configure{ visible = black_bars_enabled, x = 0, y = 0, width = viewport_rect.pw, height = viewport_rect.py }
		self._workspace:panel():child(self.BLACK_BAR_BOTTOM_GUI_NAME):configure{ visible = black_bars_enabled,x = 0, y = resolution.y - viewport_rect.py, width = viewport_rect.pw, height = viewport_rect.py }
		
		-- Destroy and re-create Gui objects.
...
function CoreCutscenePlayer:_reparent_camera()
	-- This relocates the camera and listener to the currently active camera position.
	-- Note that neither the listener or the camera need to be active (i.e. we've yet
	-- to switch from the in-game camera and listener). We still relocate them so they
	-- are ready to use when we switch to them.
	...
	if camera_object ~= nil then
		if camera_object ~= self:_camera_controller():get_camera() then
...
function CoreCutscenePlayer:_update_future_camera()
	if self._cutscene:is_optimized() then
		local position, rotation = self._cast:evaluate_object_at_time(self._cutscene, "camera", "locator", self._time + 5 / 30)
