...
function PortalManager:add_unit( unit )
	...
	if added then -- Prevent unit added to a unit group to be added to a portal as well
		return
	end
	...
	for _,portal in ipairs( self._portal_shapes ) do
		...
		if added then
			-- If added, the unit will be considered visible
			self._all_units[ unit:key() ] = ( self._all_units[ unit:key() ] or 0 ) + amount
						
			local inverse = unit:unit_data().portal_visible_inverse
			local i = 0
			-- A check will be done to see if the portal isn't visible. In that case
			-- we might want to change the visible state of the unit, imidiatly
...
function PortalManager:unit_deactivated( tag, unit, body, activated )
	if not activated then
		...
		self:add_unit( unit ) -- must consider the states of the portals it is added to
...
-- Editor function, called from CoreLayer:remove_unit
...
function PortalManager:change_visibility( unit, i, inverse )
	...
	-- If a unit is shown on inverse, the meaning of the key value will also be inverse.
...
-- add effect is called from WorldHolder, it will ask portals to add it and count up usage amount
...
-- Portal shapes reports here when they think that the effect should be turn on/off. The
-- manager decides if it really will happen based on the amount
...
-- restart_effects effects has been added to give the possiblilty to restart stoped effects when
-- a load call is made (don't want the engine to do that). Only fully supported for multiplayer.
function PortalManager:restart_effects()
	for _,portal in ipairs( self._portal_shapes ) do
		portal:clear_effects()
	end
	-- local restart = clone( self._all_effects )
	local restart = {}
...
function PortalManager:render()
	...
	while table.remove( self._check_positions ) do end
	-- self._check_positions = nil
end
...
-- Return the unit groups table
function PortalManager:unit_groups()
	return self._unit_groups
end

-- Returns the group a certain shape belongs to
...
-- Removes a group from unit groups
function PortalManager:remove_unit_group( name )
	self._unit_groups[ name ] = nil
end

-- Clears the unit groups table
function PortalManager:clear_unit_groups()
	self._unit_groups = {}
end

-- Returns first availible group name
...
-- Returns positions based on current viewport cameras. Only creates the table once per frame.
...
-- Creates a string, format to save to world xml
...
-- Creates a table to be saved in the level file
...
function PortalShape:init( polygon_tbl, min, max )
	...
	self._is_inside = true -- When camera is created we will now whats inside and whats is not. Everything visible before that.
	self._min = min
	self._max = max
end
...
function PortalShape:add_unit( unit )
	if self:inside( unit:position() ) then
		...
		-- If the unit should be visible inverse it should be hidden as a start state and add -1 to the amount in the all units list
		local inverse = unit:unit_data().portal_visible_inverse
		if inverse then
			unit:set_visible( false )
		end
...
-- Called from managers.portal:clear(), used after simulation in editor
...
-- Check if position is inside the shape
...
-- Checks if the camera is in the same portal and tells managers.portal if it want a unit to be hidden or shown
...
--------------------------------------------------------------------
-- PortalUnitGroup contains box shapes. When units are spawned
-- they check if they should be added to this group
--------------------------------------------------------------------
...
--		unit:set_visible( self._is_inside )
