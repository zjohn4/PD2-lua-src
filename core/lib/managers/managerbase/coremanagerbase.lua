...
function ManagerBase:init( name )
	...
	self.__aos = {}
	self.__ao2prio = {}
	self.__really_active = {}    -- derived from __aos, only needed for optimization
	self.__active_requested = {} -- derived from __aos, only needed for optimization
	self.__changed = false       -- used to remember if we should log at end of frame
end

-- Subclass must call this method when a new ao is created
...
-- Subclass must call this method when an ao is destroyed
...
function ManagerBase:_ao_by_name( name )
	return table.find_value( self.__aos, function(ao) return ao:name() == name end )
end

function ManagerBase:_all_ao_by_prio( prio )
	return table.find_all_values( self.__aos, function(ao) return self.__ao2prio[ao] == prio end )
end

function ManagerBase:_all_really_active_by_prio( prio )
	return table.find_all_values( self.__really_active, function(ao) return self.__ao2prio[ao] == prio end )
end

function ManagerBase:_all_active_requested_by_prio( prio )
	return table.find_all_values( self.__active_requested, function(ao) return self.__ao2prio[ao] == prio end )
end

-- This method is called by CoreAccessObjectBase only
function ManagerBase:_prioritize_and_activate()
	self.__active_requested = table.find_all_values( self.__aos, function(ao) return ao:active_requested() end )
	...
	for ao,prio in pairs(self.__ao2prio) do
		if prio < req_prio then       -- lingering previously activated high prio ao => deactivate
			if ao:really_active() then
				ao:_really_deactivate()
			end
		elseif prio == req_prio then  -- ao is on the current priority level => activate/deactivate as requested
			if not ao:active_requested() and ao:really_active() then
				ao:_really_deactivate()
			end
		else                          -- ao has low priority => deactivate
			if ao:really_active() then
				ao:_really_deactivate()
			end
		end
	...
	for ao,prio in pairs(self.__ao2prio) do
		if prio == req_prio then  -- ao is on the current priority level => activate/deactivate as requested
			if ao:active_requested() and not ao:really_active() then
				ao:_really_activate()
			end
		end
	end
	self.__really_active = table.find_all_values( self.__aos, function(ao) return ao:really_active() end )
	self.__changed = true
end
