...
function DebugManager.draw_rot_list( list )
	for index,point in ipairs( list ) do
		...
		Application:draw_sphere_unpaused( point._pos, point._radius, unpack( color ) )
		Application:draw_line_unpaused( point._pos, point._pos + point._rot:x() * DebugManager.ROT_LINE_LENGTH, 1, 0, 0 )	-- X
		Application:draw_line_unpaused( point._pos, point._pos + point._rot:y() * DebugManager.ROT_LINE_LENGTH, 0, 1, 0 )	-- Y
		Application:draw_line_unpaused( point._pos, point._pos + point._rot:z() * DebugManager.ROT_LINE_LENGTH, 0, 0, 1 )	-- Z
	end
end
...
function DebugManager.get_color( scale )
	if( scale < 0.25 ) then
		...
		return { 1, b, 0 }
	elseif( scale < 0.50 ) then
		local b = DebugManager.get_interval_brightness( scale, 0.25, 0.50 )
		return { 1 - b, 1, 0 }
	elseif( scale < 0.75 ) then
		local b = DebugManager.get_interval_brightness( scale, 0.50, 0.75 )
		return { 0, 1, b }
	else
...
-- This function should be called by the console on QA-computers:
...
function DebugPoint:init( pos, rot, red, green, blue, radius )
	...
	if( red ) then
		self._color = { red or 1, green or 1, blue or 1 }
	end
...
function DebugRaycast:init( copy_ray_wrapper )
	...
	self._color = copy_ray_wrapper and copy_ray_wrapper:color() or { 1, 1, 1 }
	self._normal = copy_ray_wrapper and copy_ray_wrapper:normal() or nil
	self._normal_color = copy_ray_wrapper and copy_ray_wrapper:normal_color() or { 0, 0, 0 }
...
function DebugRaycast:set_normal_color( red, green, blue )
	self._normal_color = { red or 1, green or 1, blue or 1 }
end
...
function DebugProfilerCounter:set_enabled( enabled )
	enabled = not not enabled		-- Converts to boolean.
...
function GraphDebug:update( t, dt )
	...
	if( contains_values ) then
		...
		local camera = self._camera or ( managers.viewport and managers.viewport:get_current_camera() )	-- Why is there no camera in core???
		local rotation
		local origo
...
function GraphDebug:set_max_count( list_index, max_count )
	if( list_index ) then
		...
		if( list and ( #list > max_count ) ) then
	...
	if( type( param_list[ 1 ] ) == "string" ) then
		...
		while( i < #param_list ) do
			...
			elseif( param == "bundle" ) then
				...
				i = i + 1
			elseif( ( param == "ignore_body" ) or ( param == "ignore_unit" ) or
							( param == "target_body" ) or ( param == "target_unit" ) or
							( param == "passed" ) or ( param == "ray_type" ) ) then
				i = i + 1
...
function ProfilerDebug:init()
	...
	self._counter_list = {}
	self._counter_map = {}	-- Same as _counter_list except that key is counter name.
end
...
function ProfilerDebug:set_managers_enabled( enabled, ignore_map, include_only_map, min_range, max_range, disabled, graph_disabled, gui_disabled, print_counters )
	for manager_name,manager in pairs( managers ) do
		...
		if( enabled ) then
			if( ( type( manager ) == "table" ) and manager.update and ( not ignore_map or not ignore_map[ manager_name ] ) and ( not include_only_map or include_only_map[ manager_name ] ) ) then
...
function MacroDebug:get_cleaned_path( path )
	if( path ) then
		...
		repeat
			clean_path, hit_count = string.gsub( clean_path, "\\[^\\:]+\\%.%.", "" )	-- Translates ".."
		until( hit_count == 0 )
...
function MacroDebug:effect( effect )
	...
	effect_manager:spawn( { effect = effect:id(), position = cam:position() + cam:rotation():y() * 200, normal = math.UP } )
end
...
MemoryDebug.CALC_TYPE_FUNC_MAP = DebugManager.CALC_TYPE_FUNC_MAP or { string = "add_calc_string", boolean = "add_calc_boolean", number = "add_calc_number", userdata = "add_calc_userdata", table = "add_calc_table", [ "function" ] = "add_calc_function" }
MemoryDebug.PRIMITIVE_VALUE_TYPE_MAP = DebugManager.PRIMITIVE_VALUE_TYPE_MAP or { string = true, boolean = true, number = true }
...
function MemoryDebug:extensions()
	...
	for _,unit in ipairs( unit_list ) do
		...
		for _,extension_name in ipairs( extension_list ) do
			...
			if( not data ) then
				data = { count = 0, unit_count = {}, unit_list = {} }
			end
...
function MemoryDebug:find_instance( find_value, is_meta_data, print_path, find_all, seen_map, map )
	if( find_value ~= nil ) then
		local func = function( path, key, value, populate_map, info_map, seen_map, func )
...
function MemoryDebug:traverse_instances( func, seen_map, map )
	...
	local populate_map = {}
	local info_map = { count = 0, seen_count = 0, found_count = 0 }
...
function ConsoleDebug:add_debug( ... )
	self:add_text( self:get_arg_text( ... ), Color( 60 / 255, 162 / 255, 67 / 255 ) )
end
...
function ConsoleDebug:add_text( text, color )
	...
	local text_data = { time = TimerManager:wall():time(), text = formatted_text, color = color }
...
function MenuDebug:setup_menu_shape()
	...
	-- Sets x on the options now that max width is known:
	for _,option in ipairs( self._option_panel:children() ) do
		local x, y, w, h = option:text_rect()
