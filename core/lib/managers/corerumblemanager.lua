...
function RumbleManager:init()		
	self._last_played_ids = {} -- a table holding the last played id of every non-cumulative rumble
...
-- Added this to be able to add a custom rumble from the mission script
...
function RumbleManager:stop( rumble_id ) --if rumble_id == "all" , all rumbles are stopped
	if rumble_id then
...
function RumbleManager:play( name, controller_wrapper, multiplier_data, custom_data ) -- type of rumble, controller_wrapper to experience the rumble or nil to affect all registred ones, amplitude multiplier
	...
	if effect then
		...
		for _, controller in pairs( rumble_controllers ) do
			if self._last_played_ids[ controller:key() ] then
				...
				if redundant_rumble then --if it is a non-cumulative rumble and we have played it before delete the old one before playing a new one		
					if controller:is_rumble_playing( redundant_rumble[ 1 ] ) or controller:is_rumble_playing( redundant_rumble[ 2 ] ) then
			...
			if effect.engine == "hybrid" then 	--	Hybrid rumbles contain both left and right rumble that are not necessary identical. 
				table.insert( rumble_id, 1, controller:rumble( { engine = "left", timer = timer,
																			peak = ( effect.peak.l or 1 ) * multiplier, 
																			attack = effect.attack.l, 
																			sustain = effect.sustain.l,
																			release = effect.release.l } ) )
				table.insert( rumble_id, 2, controller:rumble( { engine = "right", timer = timer,
																			peak = ( effect.peak.r or 1 ) * multiplier, 
																			attack = effect.attack.r, 
																			sustain = effect.sustain.r,
																			release = effect.release.r } ) )													
			else
				rumble_id[ 1 ] = controller:rumble( { engine = effect.engine,  timer = timer, 
...
function RumbleManager:set_multiplier( rumble_id, multiplier )
	if( not self._enabled or not ( rumble_id and multiplier ) ) then
		return false
	end
	local effect = self._preset_rumbles[ rumble_id.name ]
	for _, controller in pairs( rumble_id.controllers ) do
		if rumble_id[ 2 ] then --it is a hybrid. so it consists of 2 effects, a left and a right one
		...
		else	--it is not a hybrid, it has only 1 effect
