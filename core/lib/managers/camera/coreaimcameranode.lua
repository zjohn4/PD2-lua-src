...
function AimCameraNode:update(t, dt, in_data, out_data)
	...
	--local current_position = parent_position + local_position:rotate_with(parent_rotation)
	--local direction = (eye_target_position - current_position):normalized()
...
--	local rotation = parent_rotation:inverse()*Rotation(direction, math.UP)
...
function AimCameraNode:_update_pitch_offset(parent_position, parent_rotation)
	...
	if current_position_to_eye_target:length() > 0 then
		...
		if pitch < 0 and pitch > -90 then
			...
			local y_offset = -math.sign(pitch) * (math.sin(270 + normalized_pitch * 180) * 0.5 + 0.5) * 90 --tweak_data.player.camera.AIMING_DOWN_FORWARD_OFFSET
