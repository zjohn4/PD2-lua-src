...
-- set_local/world_position/rotation must only be called in TransformCameraNode.update
...
-- Must not call update on children
...
function TransformCameraNode:debug_render(t, dt)
	local x_pen = Draw:pen(Color(0.05, 1.0, 0.0, 0.0))
	local y_pen = Draw:pen(Color(0.05, 0.0, 1.0, 0.0))
	local z_pen = Draw:pen(Color(0.05, 0.0, 0.0, 1.0))
	...
	local brush = Draw:brush(Color(0.3, 1.0, 1.0, 1.0))
	brush:sphere(position, 1.0)	
end
