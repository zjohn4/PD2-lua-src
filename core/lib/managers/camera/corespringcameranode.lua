...
function SpringCameraNode:acceleration( displacement, velocity, force )
	...
	return Vector3(
		-(displacement.x * spring.x) - damping.x * velocity.x + force.x,
		-(displacement.y * spring.y) - damping.y * velocity.y + force.y,
		-(displacement.z * spring.z) - damping.z * velocity.z + force.z
	)
end
...
function SpringCameraNode:rk4_integration( dt, force )
	...
	local vf = v1 + (dt/6.0)*(a1 + 2*a2 + 2*a3 + a4) 
...
function SpringCameraNode:update(t, dt, in_data, out_data)
	...
	mvector3_set_static(
		displacement,
		math.clamp(displacement.x, -max_displacement.x, max_displacement.x), 
		math.clamp(displacement.y, -max_displacement.y, max_displacement.y), 
		math.clamp(displacement.z, -max_displacement.z, max_displacement.z)
		)
