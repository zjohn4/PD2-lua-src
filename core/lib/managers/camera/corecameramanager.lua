...
--	cat_debug("camera", "Viewing camera '" .. camera_name .. "' in layer '" .. (layer_name and layer_name or "default") .. "' with blend time " .. (blend_time and blend_time or "?"))
...
function CameraTemplateManager:load_cameras()
	...
	if DB:has(CameraTemplateManager.camera_db_type, CameraTemplateManager.camera_db_path) then
		...
		for xml_child_node in xml_node_children do
			if xml_child_node:name() == "camera_file" then
				if xml_child_node:has_parameter("file") then
