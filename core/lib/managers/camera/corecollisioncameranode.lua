...
function CollisionCameraNode:init(settings)
	...
	self._pop_controller = SmootherPopController() --FastSmootherPopController()
...
function CollisionCameraNode:_update_fast_smooth(t, dt, in_data, out_data)
	...
	if camera_distance > 0.0 then
		camera_direction = camera_direction * (1.0/camera_distance)
		...
		if collision_distance < self._camera_distance then
			...
			local diff = math.clamp(collision_distance - self._camera_distance, 0.0, self._camera_max_velocity*dt)
