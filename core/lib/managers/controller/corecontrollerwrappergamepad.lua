...
function ControllerWrapperGamepad:init( manager, id, name, controller, setup, debug, skip_virtual_controller )
	...
	ControllerWrapperGamepad.super.init( self, manager, id, name, { gamepad = controller, keyboard = Input:keyboard(), mouse = Input:mouse() }, "gamepad", setup, debug, skip_virtual_controller, { gamepad = func_map } )
end
...
function ControllerWrapperGamepad:get_input_axis( connection_name )
	...
	if connection_name == "look" then -- This is a hack cause the ps3 controller on windows have flips left/right with up/down
