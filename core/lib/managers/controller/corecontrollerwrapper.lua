...
function ControllerWrapper:reset_cache( check_time )
	...
	if( not check_time or ( reset_cache_time > self._reset_cache_time ) ) then
		...
		if next(self._input_pressed_cache) then self._input_pressed_cache = {} end
		if next(self._input_bool_cache) then self._input_bool_cache = {} end
		if next(self._input_float_cache) then self._input_float_cache = {} end
		if next(self._input_axis_cache) then self._input_axis_cache = {} end
...
-- Delays button input by manipulating the cache.
					...
					if( not self:get_input_bool( connection_name ) ) then
						for delay_connection_name,delay_time in pairs( delay_time_map ) do
							...
							if( down ) then
								...
								elseif( wall_time >= delay_time ) then
									allow = true
								end
				...
				for _,delay_input_name in ipairs( delay_input_name_list ) do
					for _,input_name in ipairs( input_name_list ) do
						if( delay_input_name == input_name ) then
							...
							delay_data.delay_time_map[ delay_connection_name ] = false		-- Indicates that a delay hasn't started yet. Otherwise this variable is the time it stops being delayed.
...
function ControllerWrapper:get_trigger_func( connection_name, func )
	...
	if( self._delay_bool_map[ connection_name ] or self._multi_input_map[ connection_name ] ) then
		return function( ... )
			wrapper:reset_cache( true )		-- Since the triggers are executed first in the frame, before the script update.
...
function ControllerWrapper:queue_delay_trigger( connection_name, func, ... )
	self._delay_trigger_queue[ connection_name ] = { func = func, func_params = { ... } }
end
...
function ControllerWrapper:get_any_input()
	if( self._input_any_cache == nil ) then
		...
		self._input_any_cache = not not self._input_any_cache	-- Makes sure it isn't nil
	end
...
function ControllerWrapper:get_any_input_pressed()
	if( self._input_any_pressed_cache == nil ) then
		...
		self._input_any_pressed_cache = not not self._input_any_pressed_cache	-- Makes sure it isn't nil
	end
...
function ControllerWrapper:get_any_input_released()
	if( self._input_any_released_cache == nil ) then
		...
		self._input_any_released_cache = not not self._input_any_released_cache	-- Makes sure it isn't nil
	end
...
function ControllerWrapper:get_input_pressed( connection_name )
	-- local id = Profiler:start( "get_input_pressed" )
...
function ControllerWrapper:get_input_bool( connection_name )
	-- local id = Profiler:start( "get_input_bool" )
...
function ControllerWrapper:get_input_axis_clbk( connection_name, func )
	...
	local f = function( axis_id, controller_name, axis )
...
function ControllerWrapper:get_modified_axis( connection_name, connection, axis )
	...
	if( multiplier ) then
		-- axis = Vector3( axis.x * multiplier.x, axis.y * multiplier.y, axis.z * multiplier.z )
	...
	if( inversion ) then
		-- axis = Vector3( axis.x * inversion.x, axis.y * inversion.y, axis.z * inversion.z )
...
function ControllerWrapper:lerp_axis( connection_name, connection, axis )
	...
	if( lerp ) then
		...
		-- axis = math.lerp( current_axis, axis, lerp )
...
function ControllerWrapper:get_connection_enabled( connection_name )
	-- local id = Profiler:start( "get_connection_enabled" )
	local connection = self._connection_map[ connection_name ] and self._setup:get_connection( connection_name )
...
function ControllerWrapper.__tostring( self )
