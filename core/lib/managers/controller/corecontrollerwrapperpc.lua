...
ControllerWrapperPC.CONTROLLER_TYPE_LIST = { "win32_keyboard", "win32_mouse" }
...
function ControllerWrapperPC:init( manager, id, name, controller, setup, debug, skip_virtual_controller, gamepads )
	...
	ControllerWrapperPC.super.init( self, manager, id, name, { keyboard = Input:keyboard(), mouse = Input:mouse(), gamepads = gamepads }, "keyboard", setup, debug, skip_virtual_controller, { keyboard = func_map } )
end
...
function ControllerWrapperPC:virtual_connect2( controller_id, controller, input_name, connection_name, connection )
	...
	if(connection._btn_connections and input_name=="buttons")then
		...
		for btn, input in pairs(connection._btn_connections)do
			...
			if(not (controller:has_button( Idstring(input.name) ) and input.type=="button" or controller:has_axis( Idstring(input.name) ) and input.type=="axis"))then
