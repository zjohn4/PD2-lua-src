...
function ControllerManager:init( path, default_settings_path )
	...
	self._controller_to_wrapper_list = {}	-- { controller_index = wrapper_index } (controller_index starts at 0)
	self._wrapper_to_controller_list = {}	-- { wrapper_index = { controller_index1, controller_index2, ... } } (controller_index starts at 0)
	...
	--[[self._supported_wrapper_types[ CoreControllerWrapperGamepad.ControllerWrapperGamepad.TYPE ] = CoreControllerWrapperGamepad.ControllerWrapperGamepad
	...
	--[[ Used variables:
	self._default_controller_connected = nil
...
function ControllerManager:check_connect_change()
	if( self._default_controller_list ) then
		...
		-- if( not self._default_controller_connected ~= not connected ) then
		if( not Global.controller_manager.default_controller_connected ~= not connected ) then
			...
			-- self._default_controller_connected = connected
		end
	end
end
...
function ControllerManager:get_preferred_default_wrapper_index()
	...
	for wrapper_index, wrapper_class in ipairs( self._wrapper_class_map ) do
		if Input:controller( wrapper_index ):connected() then -- Check with a real controller if it is connected
			if( wrapper_class.TYPE ~= "pc" ) then
...
function ControllerManager:save_settings( path )
	...
	elseif( self._last_version and path ) then
		...
		local data = { _meta = "controller_settings", core_version = self._last_core_version, version = self._last_version }
...
function ControllerManager:create_virtual_pad()
	...
	while step < num do
		...
		if controller then
			if ((controller:type() == "win32_game_controller" ) and controller:connected())then
...
function ControllerManager:verify_parsed_controller_setup_map( parsed_controller_setup_map, path )
	...
	-- Verifies that all connections exist in all wrapper types:
