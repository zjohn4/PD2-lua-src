...
-- This flag should be removed! It is for a temp test. /AJ
...
function CoreDatabaseBrowser:init(browser_data, db)
	...
	--Application:set_database_reload_callback(callback( self, self, "on_dirty_entrys" ))
...
function CoreDatabaseBrowser:create_main_frame()
	...
	-- This is a hack to force use of the material editor. ;)
	--if Application:short_game_name() ~= "BC" then
	...
	-- self._local_box.update_btn = EWS:Button(self._local_box.panel, "Refresh", "", "")
	-- self._local_box.update_btn:connect("", "EVT_COMMAND_BUTTON_CLICKED", callback( self, self, "on_update_btn" ), "")
	-- bottom_local_box:add(self._local_box.update_btn, 1, 0, "EXPAND")
	...
	-- Text preview.
	self._preview_text_ctrl = CoreEWS:XMLTextCtrl(self._main_frame, nil, nil, nil, "TE_MULTILINE,TE_RICH2,TE_DONTWRAP,TE_READONLY") -- EWS:TextCtrl(self._main_frame, "", "", "TE_READONLY,TE_MULTILINE,TE_DONTWRAP")
...
function CoreDatabaseBrowser:on_dirty_entrys()
	if self._dirty_flag then
		--self._dirty_database_dialog:show_modal()
...
function CoreDatabaseBrowser:on_commit_btn()
	...
	if(#self._local_box.list_box:selected_indices() > 0 and self._commit_dialog:show_modal() == "ID_YES" and self._comment_dialog:show_modal()) then
		...
		for id_str,entry in pairs(commit_table) do
			-- This is for tracing down a mean bug... /AJ
			...
			if self._local_changes[entry].change == "ADD_OR_MODIFY" and self._local_changes[entry].entry:type() == "texture" and (self:is_entry_raw(self._local_changes[entry].entry) or self._local_changes[entry].entry:property("platform") == "") then
				if conversion_dialog then
					--conversion_dialog:show_modal()
					conversion_dialog = nil
				end
			...
			--[[ To be included when we start using the shader compiler.
			elseif self._local_changes[entry].change == "ADD_OR_MODIFY" and (self._local_changes[entry].entry:type() == "shader_lib" or self._local_changes[entry].entry:type() == "render_template") then
				progress:update_bar(100)
				EWS:message_box(self._main_frame, "You can not commit render_template's or shader_lib's! Initiate a remote compile in the MaterialEditor instead.", "Commit", "OK", Vector3(-1,-1,-1))
		...
		while(not commit_ret) do
			...
			if commit_ret == "LOCKED" then -- If the index is locked.
		...
		if commit_ret ~= "SUCCESS" then
			...
			progress:update_bar(100)
			-- EWS:MessageDialog(self._main_frame, "Success! You now need to close down the application and update your project.\n\nRevision: " .. Application:revision(self._active_database:root() .. "/" .. new_entrys[#new_entrys]:path()), "Success", "OK,ICON_INFORMATION"):show_modal()
...
function CoreDatabaseBrowser:on_read_database()
	...
	local apply_type_filter = function(type_combobox, dest_type_combobox)
	...
	for _, entry in ipairs(data_table or {}) do
		if not self._browser_data or (entry:num_properties() == 0 or database_type ~= "texture") then -- Sort out platform textures.
...
function CoreDatabaseBrowser:check_path(path)
	if(path ~= "") then
		if(string.sub(path, 1, 1) ~= "/" or string.sub(path, string.len(path), string.len(path)) == "/") then
			return false
		end
	end
	return true
...
function CoreDatabaseBrowser:is_folder(folder_table, id)
	for _,child in pairs(folder_table.children) do
		if(child.id == id) then
			...
			if(self:is_folder(child, id)) then
				return true
			end
		end
	end
...
function CoreDatabaseBrowserMoveDialog:init(editor, p)
	self._dialog = EWS:Dialog(p, "Move", "", Vector3(-1,-1,0), Vector3(300.0, 75.0, 0.0), "CAPTION,SYSTEM_MENU,STAY_ON_TOP")
...
function CoreDatabaseBrowserImportDialog:init(editor, p)
	self._dialog = EWS:Dialog(p, "New Entry", "", Vector3(-1,-1,0), Vector3(300.0, 86.0, 0.0), "CAPTION,SYSTEM_MENU,STAY_ON_TOP")
	local box = EWS:BoxSizer( "VERTICAL" )
...
function CoreDatabaseBrowserMetadataDialog:init(p)
	self._dialog = EWS:Dialog(p, "Set Metadata", "", Vector3(-1,-1,0), Vector3(300.0, 86.0, 0.0), "CAPTION,SYSTEM_MENU,STAY_ON_TOP")
	local box = EWS:BoxSizer( "VERTICAL" )
	local text_box = EWS:BoxSizer( "HORIZONTAL" )
...
function CoreDatabaseBrowserInputDialog:init(p)
	self._dialog = EWS:Dialog(p, "Comment", "", Vector3(-1, -1, 0), Vector3(300.0, 86.0, 0.0), "CAPTION,SYSTEM_MENU,STAY_ON_TOP")
	local box = EWS:BoxSizer( "VERTICAL" )
	local text_box = EWS:BoxSizer( "HORIZONTAL" )
...
function CoreDatabaseBrowserRenameDialog:init(p)
	self._dialog = EWS:Dialog(p, "Rename", "", Vector3(-1,-1,0), Vector3(300.0, 86.0, 0.0), "CAPTION,SYSTEM_MENU,STAY_ON_TOP")
	local box = EWS:BoxSizer( "VERTICAL" )
	local text_box = EWS:BoxSizer( "HORIZONTAL" )
