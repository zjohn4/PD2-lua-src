...
-- =============================================================================
--  class: C o r e L u a P r o f i l e r V i e w e r
--
-- =============================================================================
...
function LuaProfilerViewer:_create_main_frame()
	...
	-- create frame
	self._frame = EWS:Frame( TOOLHUB_NAME,
	                         Vector3(100,400,0),
	                         Vector3(1500,800,0),
	                         "FRAME_FLOAT_ON_PARENT,DEFAULT_FRAME_STYLE",
	                         Global.frame )
	local frame_sizer = EWS:BoxSizer( "VERTICAL" )

	-- create splitter, and menu
	...
	splitter1:set_sash_gravity( 1.0 )
...
function LuaProfilerViewer:_create_menu()
	...
	self._view_menu = view_menu   -- will be filled by _redraw_menu()...
...
function LuaProfilerViewer:_on_open()
	local filedialog = EWS:FileDialog( self._frame,
	                                   "Open 'luaprofiler dump_stat' File",    -- message
	                                   managers.database:base_path(),          -- directory
	                                   "",                                     -- file
	                                   "*.pf",                                 -- pattern
	                                   "" )                                    -- style
	...
	if name ~= '' then  
		...
		if cause == '' then
			...
			self._treeview:set_profilerdata{ lpd           = self._lpd,
			                                 displayformat = self._displayformat }
			self._gridview:set_profilerdata{ lpd           = self._lpd,
			                                 displayformat = self._displayformat }
		else
