...
function InteractionEditorUI:init(owner)
	...
	self._file_menu:set_enabled("UNDO", false) -- TEMP!!!
	edit_menu:append_item("REDO", "Redo\tCtrl-Y", "")
	self._file_menu:set_enabled("REDO", false) -- TEMP!!!
...
function InteractionEditorUI:create_graph_context_menu(system)
	...
	for _,v in ipairs(InteractionDescription:node_types()) do
		...
		system:graph():window():connect("ADD_NODE_" .. string.upper(v), "EVT_COMMAND_MENU_SELECTED", callback( self._owner, self._owner, "on_add_node" ), function() system:add_node(v) end)
	end
	...
	system:graph():window():connect("DELETE_NODE", "EVT_COMMAND_MENU_SELECTED", callback( self._owner, self._owner, "on_remove_node" ),
		function()
...
function InteractionEditorUI:show_graph_context_menu(system)
	system:context_menu():set_enabled("DELETE_NODE", false)--#system:selected_nodes() > 0)
...
function InteractionEditorUI:destroy_nb_page(id)
	...
	local newid = math.clamp(self:set_nb_page(0), 0, math.clamp(self:get_nb_page_count() - 2, 0, math.huge)) --WX is broken!
