...
function FormulaMixerDummy:update(t, dt)
	...
	if (t == "string" and value ~= self._value) or
		(t == "number" and math.abs(value - self._value) > self._tol) or
		(t == "userdata" and (value - self._value):length() > self._tol)
	then
		self._value = value
	end
end
...
function Vector2Slider:init( editor, p, name, picker_bottom, picker_top, min, max, scale, display_scale )
	...
	-- self._text = EWS:StaticText(p, name, "", "")
	-- self._top_button_box:add(self._text, 1, 0, "EXPAND")
	...
	self._slider_r = EWS:Slider(p, 0.0, min, max)
	...
	self._slider_g = EWS:Slider(p, 0.0, min, max)
...
function Vector2Slider:get_value()
	return Vector3(self._slider_r:get_value() / self._scale, self._slider_g:get_value() / self._scale, 0.0)
end
...
function Vector2Slider:on_update_textctrl()
	...
	self:set_value(Vector3((r * self._display_scale) / self._scale, (g * self._display_scale) / self._scale, 0.0))
	self._editor:value_is_changed()
end
...
function DBDropdown:init( editor, p, name, db_key )
	...
	-- self._text = EWS:StaticText(p, name, "", "")
	-- self._button_box:add(self._text, 1, 0, "EXPAND")
...
function SingelSlider:init( editor, p, name, picker, min, max, scale, display_scale, picky )
	...
	self._slider = EWS:Slider(p, 0.0, min, max)
...
function EnvironmentEditorEnvMixer:init( editor, p, name )
	...
	self._slider = EWS:Slider(p, 0.0, 0.0, (#self._editor._template_environment_names - 1) * self._editor.MIX_MUL)
...
function EnvironmentEditorEnvMixer:on_slider_change()
	...
	-- Just making sure we don't exide our limit.
...
function RgbBox:init( editor, p, name )
	...
	-- self._text = EWS:StaticText(p, name, "", "")
	-- self._pick_box:add(self._text, 1, 0, "EXPAND")
	...
	self._slider_r = EWS:Slider(p, 128.0, 0.0, 255.0)
	self._slider_r:set_background_colour(195.0, 180.0, 180.0)
	self._slider_r:connect("", "EVT_SCROLL_THUMBTRACK", callback( self, self, "on_slider" ), "")
	self._slider_r:connect("", "EVT_SCROLL_CHANGED", callback( self, self, "on_slider_changed" ), "")
	self._slider_r_box:add(self._slider_r, 5, 0, "EXPAND")
	...
	self._slider_g = EWS:Slider(p, 128.0, 0.0, 255.0)
	self._slider_g:set_background_colour(180.0, 195.0, 180.0)
	self._slider_g:connect("", "EVT_SCROLL_THUMBTRACK", callback( self, self, "on_slider" ), "")
	self._slider_g:connect("", "EVT_SCROLL_CHANGED", callback( self, self, "on_slider_changed" ), "")
	self._slider_g_box:add(self._slider_g, 5, 0, "EXPAND")
	...
	self._slider_b = EWS:Slider(p, 128.0, 0.0, 255.0)
	self._slider_b:set_background_colour(180.0, 180.0, 195.0)
...
function RgbBox:on_slider_changed()
	self:set_value(Vector3(self._slider_r:get_value() / 255.0, self._slider_g:get_value() / 255.0, self._slider_b:get_value() / 255.0))
	self._editor:value_is_changed()
end
...
function PathBox:init( editor, p, name )
	...
	-- self._text = EWS:StaticText(p, name, "", "")
	-- h_box:add(self._text, 1, 0, "EXPAND")
...
function ConnectDialog:init(p)
	self._dialog = EWS:Dialog(p, "Connect Client", "", Vector3(0.0, 0.0, 0.0), Vector3(300.0, 75.0, 0.0), "CAPTION,SYSTEM_MENU")
