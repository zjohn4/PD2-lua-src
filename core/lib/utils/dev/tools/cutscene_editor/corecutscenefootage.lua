...
function CoreCutsceneFootage:colour()
	if self._colour == nil then
		...
		local divisor = precision * (1 / (1.0 - black_value))
...
function CoreCutsceneFootage:camera_icon_index(camera_name, image_count)
	image_count = image_count or (CAMERA_ICON_IMAGE_COUNT + 1) -- Image zero is the default, without a number.
	...
	if icon_index >= image_count then
		icon_index = 0
	end
...
function CoreCutsceneFootage:_camera_cut_list()
	if self._camera_cut_cache == nil then
		...
		if self._cutscene:has_cameras() then
			local add_camera_cut = function(start_frame, end_frame, camera)
				if end_frame > start_frame then
					table.insert(self._camera_cut_cache, { start_frame, end_frame, camera })
				end
			end
		
			local previous_key = responder_map{ frame = 0, camera = self._cutscene:default_camera() }
