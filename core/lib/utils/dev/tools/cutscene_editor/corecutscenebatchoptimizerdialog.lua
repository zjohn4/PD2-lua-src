...
commands:add{ id="NEW_JOB_LIST", label="&New Job List", key="Ctrl+N", help="Clears the job list so you can start with a blank slate" }
commands:add{ id="DEFAULT_JOB_LIST", label="&Default Job List", key="Ctrl+D", help="Clears the job list and populates it with all cutscene projects in the database" }
commands:add{ id="OPEN_JOB_LIST", label="&Open Job List...", key="Ctrl+O", help="Opens an existing job list" }
commands:add{ id="SAVE_JOB_LIST", label="&Save Job List", key="Ctrl+S", help="Saves the current job list to disk" }
commands:add{ id="SAVE_JOB_LIST_AS", label="&Save Job List As...", help="Saves the current job list to disk under a new name" }
commands:add{ id="EXIT", label="E&xit", help="Closes this window" }
...
function CoreCutsceneBatchOptimizerDialog:init(parent_window)
	...
	self.__window:set_status_bar_pane(0) -- This will show verbose menu item descriptions in the status bar.
...
function CoreCutsceneBatchOptimizerDialog:update(time, delta_time)
	if not self.__window then
		-- Returning true here tells the caller that we're done, and normal operation should resume.
		return true
	end
...
function CoreCutsceneBatchOptimizerDialog:_default_mappings_for_all_projects()
	...
	local mappings = table.remap(project_entries, function(_, name) return name, self:_default_optimized_cutscene_name(name) end)
	return mappings
end
...
function CoreCutsceneBatchOptimizerDialog:_verify_user_intent(operation)
	if table.empty(self.__projects:mappings()) then return true end
	...
	if choice == "ID_YES" then
		if not self:_on_save_job_list() then
			return false -- User chose to save, but then aborted the save. He does not want to proceed.
		end
	elseif choice == "ID_CANCEL" then
		return false -- User cancelled the operation.
	end
	
	return true -- User has given a clear answer, and wants to proceed.
end
