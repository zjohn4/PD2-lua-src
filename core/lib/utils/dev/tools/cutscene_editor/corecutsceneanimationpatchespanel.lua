...
function CoreCutsceneAnimationPatchesPanel:set_unit_types(unit_types)
	assert(type(unit_types) == "table" and table.true_for_all(table.map_values(unit_types), function(v) return DB:has("unit", tostring(v):id()) end), "Expected unit_types to be a table mapping actor names to Unit type names.")
...
function CoreCutsceneAnimationPatchesPanel:_absolute_dir_and_path(relative_path)
	...
	if string.begins(relative_path, "data\\") then
		...
		-- This is the name of a database entry.
		return nil, nil
	end
end
