...
function CoreCutsceneFrameExporterDialog:_folder_name_input()
	...
	if folder_name then
		...
		elseif string.match(folder_name, "[a-z_0-9]+") ~= folder_name then
			...
			if SystemFS:exists("/frames/" .. folder_name) then
				if EWS:MessageDialog(self.__window, "A folder with that name already exists. Do you want to replace it?", "Replace Existing?", "YES_NO,NO_DEFAULT,ICON_EXCLAMATION"):show_modal() == "ID_NO" then
					return nil
				end
			end
		end
...
function CoreCutsceneFrameExporterDialog:_start_end_frame_input()
	...
	end_frame = end_frame + 1 -- total number of frames to be exported (incl. frame 0) 
