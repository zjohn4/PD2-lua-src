...
CoreCutsceneEditorProject.VALID_EXPORT_TYPES = { "in_game_use", "footage_use" }
...
function CoreCutsceneEditorProject:export_type()
	local settings_node = self:child_node("settings") or responder(function() end)
...
function CoreCutsceneEditorProject:save(audio_clips, film_clips, cutscene_keys, settings)
	...
	local highest_film_track_index = table.inject(film_clips, 0, function(highest, clip) return math.max(highest, clip.track_index or 0) end)
	
	for track_index = 1, highest_film_track_index do
		local track_node = film_tracks_node:make_child("track")
		for _, clip in ipairs(table.find_all_values(film_clips, function(clip) return clip.track_index == track_index end)) do
...
function CoreCutsceneEditorProject:audio_clips()
	local clips = {}
	local audio_track_node = self:child_node("audio_track") or responder(function() end)
...
function CoreCutsceneEditorProject:cutscene_keys(key_collection)
	local cutscene_keys = {}
	local key_track_node = self:child_node("key_track") or responder(function() end)
...
function CoreCutsceneEditorProject:animation_patches()
	local patches = {}
	local settings_node = self:child_node("settings") or responder(function() end)
	local animation_patches_node = self:child_node("animation_patches", settings_node) or responder(function() end)
...
function CoreCutsceneEditorProject:child_node(child_name, parent_node, child_properties)
	...
	for child_node in parent_node:children() do
		if child_node:name() == child_name then
			if child_properties == nil or table.true_for_all(child_properties, function(value, key) return child_node:parameter(key) == value end) then
				return child_node
			end
		end
	end
