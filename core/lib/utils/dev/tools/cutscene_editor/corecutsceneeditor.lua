...
function CoreCutsceneEditor:_all_keys_sorted_by_time()
	local cutscene_keys = table.collect(self._sequencer:key_track():clips(), function(sequencer_key) return sequencer_key:metadata() end)
	table.sort(cutscene_keys, function(a, b) return a:frame() < b:frame() end)
	return cutscene_keys
end

local commands = CoreCommandRegistry:new()
commands:add{ id="NEW_PROJECT", label="&New Project", key="Ctrl+N", help="Closes the currently open cutscene project so you can start with a blank slate" }
commands:add{ id="OPEN_PROJECT", label="&Open Project...", key="Ctrl+O", help="Opens an existing cutscene project from the database" }
commands:add{ id="SAVE_PROJECT", label="&Save Project", key="Ctrl+S", help="Saves the current project to the database" }
commands:add{ id="SAVE_PROJECT_AS", label="Save Project &As...", help="Saves the current project to the database under a new name" }
commands:add{ id="SHOW_PROJECT_SETTINGS", label="&Project Settings...", help="Displays the Project Settings dialog" }
commands:add{ id="EXPORT_TO_GAME", label="&Export Cutscene to Game...", help="Exports an optimized version of the cutscene that can be used within the game" }
commands:add{ id="EXPORT_TO_MAYA", label="Export Cutscene to &Maya...", help="Exports edited animations for all actors, cameras and locators as a Maya scene" }
commands:add{ id="EXPORT_PLAYBLAST", label="Export &Playblast...", help="Exports a low-quality render of each frame in the cutscene as an image file for use as a reference" }
commands:add{ id="SHOW_BATCH_OPTIMIZER", label="&Batch Export to Game...", help="Displays the Batch Export dialog" }
commands:add{ id="EXIT", label="E&xit", help="Closes the " .. CoreCutsceneEditor.EDITOR_TITLE .. " window" }
commands:add{ id="CUT", label="Cut", key="Ctrl+X", help="Place selected clips on the clipboard. When pasted, the clips will be moved" }
commands:add{ id="COPY", label="Copy", key="Ctrl+C", help="Place selected clips on the clipboard. When pasted, the clips will be duplicated" }
commands:add{ id="PASTE", label="Paste", key="Ctrl+V", help="Paste clipboard contents into the current film track at the playhead position" }
commands:add{ id="DELETE", label="Delete", key="Del", help="Removes selected clips from the sequencer timeline" }
commands:add{ id="SELECT_ALL", label="Select &All", key="Ctrl+A", help="Select all clips and all keys in the sequencer timeline" }
commands:add{ id="SELECT_ALL_ON_CURRENT_TRACK", label="Select All on Current &Track", key="Ctrl+Shift+A", help="Select all clips on the current film track" }
commands:add{ id="DESELECT", label="&Deselect", key="Ctrl+D", help="Deselect all clips and all keys in the sequencer timeline" }
commands:add{ id="CLEANUP_ZOOM_KEYS", label="Cleanup &Zoom Keys", help="Remove all Camera Zoom keys that have no effect from the script track" }
commands:add{ id="CUTSCENE_CAMERA_TOGGLE", label="Use &Cutscene Camera", help="Check to view the scene though the directed cutscene camera" }
commands:add{ id="WIDESCREEN_TOGGLE", label="&Widescreen Aspect Ratio", key="Ctrl+R", help="Check to use 16:9 letterbox format for the directed cutscene camera" }
commands:add{ id="CAST_FINDER_TOGGLE", label="Cast &Finder", help="Visualize cast member positions using debug lines" }
commands:add{ id="CAMERAS_TOGGLE", label="&Cameras", help="Visualize cameras using debug lines" }
commands:add{ id="FOCUS_PLANE_TOGGLE", label="&Focus Planes", key="Ctrl+F", help="Visualize depth of field effects with focus planes" }
commands:add{ id="HIERARCHIES_TOGGLE", label="&Hierarchies", help="Visualize bone hierarchies using debug lines" }
commands:add{ id="PLAY_EVERY_FRAME_TOGGLE", label="Play &Every Frame", help="Check to advance the playhead by exactly one frame instead of the elapsed time since the last update during playback" }
commands:add{ id="INSERT_CLIPS_FROM_SELECTED_FOOTAGE", label="Clips from &Selected Footage", help="Inserts clips from the selected footage track at the current playhead position" }
commands:add{ id="INSERT_AUDIO_FILE", label="&Audio File...", help="Browse for an audio file to insert into the voice-over track at the current playhead position" }
commands:add{ id="LOOP_TOGGLE", label="&Loop Playback", key="Ctrl+L", help="Toggle playback looping", image="sequencer\\loop_16x16.png" }
commands:add{ id="PLAY_TOGGLE", label="&Play / Pause", key="Ctrl+Space", help="Start or pause playback from the current playhead position", image="sequencer\\play_16x16.png" }
commands:add{ id="PLAY_FROM_START", label="Play from Start", key="Ctrl+Shift+Space", help="Start playback from the start of the selected region or cutscene", image="sequencer\\play_from_start_16x16.png" }
commands:add{ id="PLAY", label="Play", key="Ctrl+Space", help="Start playback from the current playhead position", image="sequencer\\play_16x16.png" }
commands:add{ id="PAUSE", label="Pause", key="Ctrl+Space", help="Pause playback at the current playhead position", image="sequencer\\pause_16x16.png" }
commands:add{ id="STOP", label="&Stop", key="Escape", help="Stop playback and return to the start of the selected region or cutscene", image="sequencer\\stop_16x16.png" }
commands:add{ id="GO_TO_START", label="Go to &Start", key="Ctrl+Up", help="Go to the start of the selection or cutscene", image="sequencer\\go_to_start_16x16.png" }
commands:add{ id="GO_TO_END", label="Go to &End", key="Ctrl+Down", help="Go to the end of the selection or cutscene", image="sequencer\\go_to_end_16x16.png" }
commands:add{ id="GO_TO_NEXT_FRAME", label="Go to &Next Frame", key="Ctrl+Right", help="Go to the previous frame" }
commands:add{ id="GO_TO_PREVIOUS_FRAME", label="Go to &Previous Frame", key="Ctrl+Left", help="Go to the next frame" }
commands:add{ id="ZOOM_IN", label="Zoom &In", key="Ctrl++", help="Increase sequencer track zoom level", image="sequencer\\zoom_in_16x16.png" }
commands:add{ id="ZOOM_OUT", label="Zoom &Out", key="Ctrl+-", help="Decrease sequencer track zoom level", image="sequencer\\zoom_out_16x16.png" }

-- Register cutscene key insertion commands.
local cutscene_key_insertion_commands = {}
local skipped_key_types = { change_camera=true } -- We handle these types using ranged clips instead of keys.
for _, key_type in ipairs(CoreCutsceneKey:types()) do
	if not skipped_key_types[key_type.ELEMENT_NAME] then 
		...
		commands:add{ id=key, label=key_type.NAME .. " Key", help="Inserts a " .. string.lower(key_type.NAME) .. " key at the current playhead position" }
...
function CoreCutsceneEditor:camera_attributes()
	if self._player then
		...
		-- Don't ask the DOF manager... You never now what you will get. ;)
		--[[if managers.DOF then
			local near_min, near_max, far_min, far_max, clamp = managers.DOF:get_dof_values()
			attributes.near_focus_distance_min = near_min
			attributes.near_focus_distance_max = near_max
			attributes.far_focus_distance_min = far_min
			attributes.far_focus_distance_max = far_max
		end]]
...
function CoreCutsceneEditor:_create_window()
	--[[
	We're setting up quite a complex frame structure here. The window is divided vertically into:
	- Menu bar
	- Tool bar
	- Main area (see below)
	- Status bar
	
	The Main area is divided into several sub-frames using SplitterWindows like this:
	+----------------------+---+
	| Tracks               |I A|
	| (sequencer panel)    |t t|
	|                      |e t|
	...
	|                      |* b|
	+----------------------+---+
	| Selected footage trk.| C |
	+----------------------+ a |
	| Footage List         | m |
	|                      | s |
	+--------------------------+
	
	* The item attributes panel is hidden until the user selects an item with editable attributes from the timeline.
	]]--
	...
	local connect_command = function(command_id, callback_name, callback_data)
	...
	self._window:set_status_bar_pane(0) -- This will show verbose menu item descriptions in the status bar.
	...
	-- These all return the user interface component we need to add to the respective sizer.
	-- Most of them have side effects such as initializing members in self to wrapped user interface components.
	...
	bottom_left_area_top_bottom_splitter:connect("EVT_COMMAND_SPLITTER_SASH_POS_CHANGING", function(_, event) event:veto() end, "")
...
function CoreCutsceneEditor:_create_menu_bar()
	...
	-- Add insert commands for cutscene keys. 
	for label, command in table.sorted_map_iterator(table.remap(cutscene_key_insertion_commands, function(command, key_type) return key_type.NAME, command end)) do
...
function CoreCutsceneEditor:_create_tool_bar(parent_frame)
	...
	local add_labelled_field = function(label, field_text)
...
function CoreCutsceneEditor:_time_code_string_for_frame(frame)
	local consume_frames = function(frames_per_unit)
		local whole_units = math.floor(frame / frames_per_unit)
		frame = frame - whole_units * frames_per_unit
		return whole_units
	end

	local hour = consume_frames(60 * 60 * self:frames_per_second()) 
...
function CoreCutsceneEditor:_refresh_selected_footage_track()
	...
	-- Update the clip region overlay in the selected footage track
	...
	-- Select the clip camera in the camera list control
...
function CoreCutsceneEditor:_selected_footage()
	...
	if not table.empty(selected_clips) then
		-- Return the footage shared by all the the selected clips, or nil if no concensus is reached
		local selected_clip_footages = table.list_union(table.collect(selected_clips, function(clip) return clip:metadata() and clip:metadata().footage and clip:metadata():footage() or nil end))
		return #selected_clip_footages == 1 and selected_clip_footages[1] or nil
	else
		-- Return the selected footage in the footage list control, or nil if there is not exactly one item selected
...
function CoreCutsceneEditor:_refresh_footage_list()
	...
	local optimized_cutscene_names = table.find_all_values(cutscene_names, function(name) return managers.cutscene:get_cutscene(name):is_optimized() end)
	local unoptimized_cutscene_names = table.find_all_values(cutscene_names, function(name) return not managers.cutscene:get_cutscene(name):is_optimized() end)
	table.sort(unoptimized_cutscene_names)
	table.sort(optimized_cutscene_names)
	
	local add_cutscene_footage = function(name, icon_id)
...
function CoreCutsceneEditor:_evaluate_editor_cutscene_keys_for_frame(frame)
	...
	if self._last_evaluated_time == 0 and time > 0 then
		-- Make sure keys at frame 0 are evaluated as we leave it.
...
function CoreCutsceneEditor:_camera_icon_index(icon_index)
	if icon_index < 0 or icon_index >= self:_camera_icons_image_list():image_count() then
		-- We don't have a specific image for this camera. Use the first image, which is an unnumbered camera icon.
		return 0
	else
		return icon_index
	end
...
function CoreCutsceneEditor:save_project()
	if self._current_project then
		local settings = { export_type = self._project_settings_dialog:export_type(), animation_patches = self._project_settings_dialog:unit_animation_patches() }
...
function CoreCutsceneEditor:_serialized_audio_clips()
	...
	table.sort(clips, function(a, b) return a.offset < b.offset end)
	return clips
end
...
function CoreCutsceneEditor:_serialized_cutscene_keys()
	return table.collect(self._sequencer:key_track():clips(), function(sequencer_key) return sequencer_key:metadata() end)
end
...
function CoreCutsceneEditor:frames_per_second()
	return 30 -- TODO: Formalize time units.
end
...
function CoreCutsceneEditor:_process_enqueued_update_actions()
	if self._enqueued_update_actions then
		local func = table.remove(self._enqueued_update_actions) -- Pop the value that was enqueued first.
		if func == nil then
			-- Queue is empty.
...
function CoreCutsceneEditor:update(time, delta_time)
	if self._modal_window then
		-- A modal dialog is open. It should swallow all updates until dismissed by the user.
		if self._modal_window:update(time, delta_time) then
			-- When the update function returns true, we dismiss the modal window.
		...
		-- HACK: Since we might be playing around with dof settings, we want to ensure that the
		-- camera is always in full depth range mode unless we're using the cutscene camera.
...
function CoreCutsceneEditor:end_update(time, delta_time)
	if self._modal_window then
		-- A modal dialog is open. It should swallow all end_updates until dismissed by the user.
		...
		-- Draw any debug lines for the selected item.
		local selected_items = self._sequencer:selected_keys() -- TODO: Other items could also have properties.
...
function CoreCutsceneEditor:_on_exit()
	...
	if choice == "ID_YES" then
		if not self:_on_save_project() then
			return false -- User aborted the save, so we should cancel the close event.
		end
...
function CoreCutsceneEditor:_on_new_project()
	-- The easiest thing to do is to just close and reopen the cutscene editor.
...
function CoreCutsceneEditor:_on_export_to_game()
	...
	if optimized_cutscene_name == nil then return end
...
function CoreCutsceneEditor:_on_export_to_maya()
	...
	local start_frame = 0
	local end_frame = table.inject(clips_on_active_track, 0, function(final_frame, clip) return math.max(final_frame, clip:end_time()) end)
...
function CoreCutsceneEditor:_request_asset_name_from_user(asset_db_type, default_name, duplicate_name_check_func)
	...
	if asset_name == "" then
		-- EWS:get_text_from_user() returns an empty string rather than nil if the user pressed cancel.
		asset_name = nil
	end
	...
	if asset_name then
		...
		elseif string.match(asset_name, "[a-z_0-9]+") ~= asset_name then
			...
			duplicate_name_check_func = duplicate_name_check_func or function(name) return ProjectDatabase:has(asset_db_type, name) end
			if duplicate_name_check_func(asset_name) then
				if EWS:MessageDialog(self._window, "A " .. asset_type .. " with that name already exists. Do you want to replace it?", "Replace Existing?", "YES_NO,NO_DEFAULT,ICON_EXCLAMATION"):show_modal() == "ID_NO" then
					return self:_request_asset_name_from_user(asset_db_type, default_name, duplicate_name_check_func)
				end
			end
		end
...
function CoreCutsceneEditor:_on_paste()
	local earliest_item_time = table.inject(self._clipboard or {}, math.huge, function(earliest_time, item) return math.min(earliest_time, item:start_time()) end)
...
function CoreCutsceneEditor:_on_cleanup_zoom_keys()
	...
	for _, clip in ipairs(self._sequencer:key_track():clips()) do
		...
		if cutscene_key.ELEMENT_NAME == CoreZoomCameraCutsceneKey.ELEMENT_NAME then
			if clip:start_time() >= last_film_clip_end_time then
...
function CoreCutsceneEditor:_on_insert_clips_from_selected_footage(sender, event)
	-- If any clips are selected, add those. Otherwise, add all clips from the selected footage
	...
	if destination_track and #clips_to_add ~= 0 then
		...
		-- Add the clips to the currently active film track, at the playhead position
		local earliest_clip_time = table.inject(clips_to_add, math.huge, function(earliest_time_yet, clip) return math.min(earliest_time_yet, clip:start_time()) end)
		local offset = self:playhead_position() - earliest_clip_time
		local cutscene_keys = table.find_all_values(cutscene:_all_keys_sorted_by_time(), function(key) return key.ELEMENT_NAME ~= CoreChangeCameraCutsceneKey.ELEMENT_NAME end)
		
		for _, clip in ipairs(clips_to_add) do
			destination_track:add_clip(clip, offset):set_selected(true)
			
			-- Add cutscene keys from within the clip region to the key track
		...
		-- Just add cutscene keys from the selected footage
		local selected_item_index = self._footage_list_ctrl:selected_item()
		if selected_item_index > 0 then
			local selected_footage = self._footage_list_ctrl:get_item_data(selected_item_index)
			local cutscene_keys = table.find_all_values(selected_footage:keys(), function(key) return key.ELEMENT_NAME ~= CoreChangeCameraCutsceneKey.ELEMENT_NAME end)
			
			if not table.empty(cutscene_keys) then
				self._sequencer:deselect_all_items()
			end

			-- Add cutscene keys to the key track
			for _, template_key in ipairs(cutscene_keys) do
				local cutscene_key = template_key:clone()
...
function CoreCutsceneEditor:_draw_focus_planes()
	if self._player and managers.DOF then
		...
		local draw_focus_plane = function(value, color)
...
function CoreCutsceneEditor:_draw_unit_hierarchy(unit, unit_name, draw_root_point)
	unit_name = unit_name or unit:name() -- Use the unit type if we don't have an actor name.
...
---------------------------------------------------------
-- Cutscene Manager cutscene key handler delegate methods
---------------------------------------------------------

function CoreCutsceneEditor:prime_cutscene_key(player, key, cast)
	-- TODO: For now, we just ignore all cutscene keys.
end
