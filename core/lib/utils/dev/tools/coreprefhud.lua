...
function PrefHud:load_config()
	...
	for _,sub_data in ipairs( data ) do
		if( sub_data._meta == "counter" ) then
			self:add_counter(
				sub_data.name,
				tonumber( sub_data.sort ),
				tonumber( sub_data.min ),
				tonumber( sub_data.mid ),
				tonumber( sub_data.max ),
				sub_data.precision,
				( sub_data.inv == "true" ),
				( sub_data.inv_colors == "true"),
				sub_data.call
			)
		end
	end
...
function PrefHud:update_bars(t, dt)
	if (self._prev_upd or self._upd_interval) >= self._upd_interval then
		...
		for k,v in pairs(self._counters) do
			...
			local value = math.clamp(raw_value, 0.0, v._max)
			...
			if value >= v._max then
				if v._inv_colors then
					v._obj:set_color(Color(0.8, 0.0, 0))
				else
					v._obj:set_color(Color(0.0, 0.8, 0))
				end				
			elseif value >= v._mid and value <= v._max then
				local t = (value - v._mid) / (v._max - v._mid)
				if v._inv_colors then
					v._obj:set_color(math.lerp(Color(0.8, 0.8, 0), Color(0.8, 0.0, 0.0), t))
				else
					v._obj:set_color(math.lerp(Color(0.8, 0.8, 0), Color(0.0, 0.8, 0.0), t))
				end				
			elseif value >= v._min and value <= v._mid then
				local t = (value - v._min) / (v._mid - v._min)
				if v._inv_colors then
					v._obj:set_color(math.lerp(Color(0.0, 0.8, 0), Color(0.8, 0.8, 0.0), t))
				else
					v._obj:set_color(math.lerp(Color(0.8, 0.0, 0), Color(0.8, 0.8, 0.0), t))
				end				
			else
				if v._inv_colors then
					v._obj:set_color(Color(0.0, 0.8, 0))
...
function PrefHud:update_keys()
	if Application:production_build() then
		if is_win32 and Input:keyboard():down(ids_left_ctrl) and Input:keyboard():pressed(ids_f1) then
			self:toggle()
		end
	end
end
