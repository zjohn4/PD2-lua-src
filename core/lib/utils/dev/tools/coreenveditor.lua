...
function CoreEnvEditor:init(env_file_name)
	...
	if(managers and managers.environment) then
		managers.environment._handled_by_editor = true
		
		local env_node = self:database_load_env(self._env_path)
		
		-- TEMPORARY AND UGLY LOADING
		self._out_sky = managers.environment:load_sky(self._env_path, env_node:child(1):child(0))
		self._out_posteffect = managers.environment:load_post_effect(self._env_path, env_node:child(1):child(1))
		self._out_underlayeffect = managers.environment:load_underlay_effect(self._env_path, env_node:child(1):child(2))
	
		-- We need to save this value for later use.
		self._old_sky_rot = managers.environment:get_sky_rotation()
	end]]

	self._prev_environment = self._env_path
	self:database_load_env(self._env_path)
	
	managers.viewport:first_active_viewport():editor_callback(function(data) self:feed(data) end)
...
function CoreEnvEditor:read_mode()
	--self._simple_mode = not EWS:get_key_state("K_SHIFT")
end
	...
	for k,v in pairs(data:data_root()) do
		if k == "post_effect" then
			for kpro,vpro in pairs(v) do
				if kpro == "shadow_processor" then -- temp fix to feed shadow params, as multiple params are needed for the conversion
...
function CoreEnvEditor:create_main_frame()
	...
	--file_menu:append_separator()
	--file_menu:append_item("SAVE_SHADOW", "Save Shadow Settings", "")
	file_menu:append_separator()
...
--	self._main_frame:connect("SAVE_SHADOW", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "on_save_shadow" ), "")
				...
				for param in modifier:children() do
					if(param:name() == "param" and param:parameter("key") and param:parameter("key") ~= "" and param:parameter("value") and param:parameter("value") ~= "") then
						...
						if(not parameter) then
							-- Removed to allow for shadow parameters in the .environment file
							-- if string.sub(k, l - 5, l) ~= "_scale" or not mod.params[string.sub(k, 1, l - 6)] then
								-- cat_print( "debug", "[CoreEnvEditor] [Posteffect] Deprecated in: " .. self._env_path .. " " .. post_processor:parameter("name") .. " " .. modifier:parameter("name") .. " " .. k)
...
function CoreEnvEditor:close()
	...
	--[[
	if(managers and managers.environment) then
		managers.environment._handled_by_editor = false
...
function CoreEnvEditor:get_cursor_look_point( camera, dist )
	...
	local cursor_pos = Vector3(((mouse_local.x / self._screen_borders.x) * 2.0) - 1.0, ((mouse_local.y / self._screen_borders.y) * 2.0) - 1.0, dist) -- Should be between -1 and 1 for screen_to_world().
	return camera:screen_to_world( cursor_pos )
end

function CoreEnvEditor:draw_cursor()
	-- Looking for viewport manager is not nice.
	if(managers.viewport and managers.viewport.get_current_camera) then
		...
		if(alive(camera)) then
			local pos = self:get_cursor_look_point(camera, 100.0)
			Application:draw_sphere(pos, 1.0, 1.0, 0.0, 0.0)
		end
	end
end
...
function CoreEnvEditor:pick_depth()
	...
	if(managers.viewport and managers.viewport.get_current_camera) then
		local camera = managers.viewport:get_current_camera()
		if(alive(camera)) then
			local from = self:get_cursor_look_point(camera, 0.0)
			local to = self:get_cursor_look_point(camera, 1000000.0)
			local ray = World:raycast( "ray", from, to )
			
			if(ray) then
				local pos = ray.position
				return math.clamp(camera:world_to_screen( pos ).z, 0.0, camera:far_range())
			end
		end
	end
	return 0.0
