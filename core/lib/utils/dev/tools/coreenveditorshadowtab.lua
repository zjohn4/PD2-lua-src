...
function CoreEnvEditor:init_shadow_tab()
	...
	-- if self._shadow_list_box:nr_items() > 0 then
		-- --self._shadow_list_box:select_index(0)
	-- end
end
...
function CoreEnvEditor:parse_shadow_data()
	...
	--[[local node = DB:has("environment", self._env_path) and DB:load_node("environment", self._env_path)
	
	for child in node:children() do
		if child:name("data") then
			for param_type in child:children() do
				if param_type:name() == "post_effect" then
					for post_processor in param_type:children() do
						if post_processor:name() == "shadow_processor" then
							node = post_processor:child(0):child(0)
							cat_print( "debug", node:to_xml())
						end
					end
				end
			end
	...
	--[[for child in node:children() do
		...
		if math.string_is_good_vector(value) then
			values[key] =  math.string_to_vector(value)
		else
...
-- This function should be replaced by a panel interface.
...
function CoreEnvEditor:clear_param_sliders()
	for k,v in pairs(self._shadow_params) do
		-- this value also needs to be fixed, see below
		v:set_value(1.0)
	end
end
...
function CoreEnvEditor:create_shadow_tab()
	...
	-----------------------------------------------------------------------------------------------  
	-- All min values set to 1 to avoid attempts to normalize zero vectors, this should be fixed. 
	
	local settings_box = EWS:StaticBoxSizer(scrolled_window, "VERTICAL", "Settings")
	
	
	
	self._shadow_params.d0 = self:add_post_processors_param("shadow_processor", "shadow_modifier",  "d0", SingelSlider:new( self, scrolled_window, "First slice depth start", nil, 1, 10000.0, 1.0, 1.0, true ))
	settings_box:add(self._shadow_params.d0._box, 0, 0, "EXPAND")
	
	self._shadow_params.d1 = self:add_post_processors_param("shadow_processor", "shadow_modifier",  "d1", SingelSlider:new( self, scrolled_window, "Second slice depth start", nil, 1, 10000.0, 1.0, 1.0, true ))
	settings_box:add(self._shadow_params.d1._box, 0, 0, "EXPAND")
	
	self._shadow_params.d2 = self:add_post_processors_param("shadow_processor", "shadow_modifier",  "d2", SingelSlider:new( self, scrolled_window, "Third slice depth start", nil, 1, 10000.0, 1.0, 1.0, true ))
	settings_box:add(self._shadow_params.d2._box, 0, 0, "EXPAND")
	
	self._shadow_params.d3 = self:add_post_processors_param("shadow_processor", "shadow_modifier",  "d3", SingelSlider:new( self, scrolled_window, "Third slice depth end", nil, 1, 50000.0, 1.0, 1.0, true ))
	settings_box:add(self._shadow_params.d3._box, 0, 0, "EXPAND")
	
	self._shadow_params.o1 = self:add_post_processors_param("shadow_processor", "shadow_modifier",  "o1", SingelSlider:new( self, scrolled_window, "Blend overlap between first and second shadow slice", nil, 1, 10000.0, 1.0, 1.0, true ))
	settings_box:add(self._shadow_params.o1._box, 0, 0, "EXPAND")
	
	self._shadow_params.o2 = self:add_post_processors_param("shadow_processor", "shadow_modifier",  "o2", SingelSlider:new( self, scrolled_window, "Blend overlap between second and third shadow slice", nil, 1, 10000.0, 1.0, 1.0, true ))
	settings_box:add(self._shadow_params.o2._box, 0, 0, "EXPAND")
	
	self._shadow_params.o3 = self:add_post_processors_param("shadow_processor", "shadow_modifier",  "o3", SingelSlider:new( self, scrolled_window, "Blend overlap between third and forth shadow slice", nil, 1, 10000.0, 1.0, 1.0, true ))
	settings_box:add(self._shadow_params.o3._box, 0, 0, "EXPAND")
	
	self._shadow_params.f = self:add_post_processors_param("shadow_processor", "shadow_modifier",  "f", SingelSlider:new( self, scrolled_window, "Fadeout distance", nil, 1, 10000.0, 1.0, 1.0, true ))
