...
function CoreMaterialEditor:live_update_parameter(name, param_type, param_ui_type, value)
	if alive(self._selected_unit) then
		table.insert(self._live_update_parameter_list, {_name = name, _param_type = param_type, _param_ui_type = param_ui_type, _value = value})
	end
end
...
-- A very simple save/load config system.
...
function CoreMaterialEditor:_insert_libs_in_database(temp_params, make_params)
	--[[for k,v in pairs(temp_params) do
		assert(SystemFS:copy_file(v, make_params[k]), string.format("Could not copy %s -> %s", v, make_params[k]))
	end]]
...
function CoreMaterialEditor:_find_selected_unit()
	if managers.editor and managers.editor:selected_unit() and managers.editor:selected_unit() ~= self._selected_unit then
		...
		if unit_material_node and (not self._material_config_path or unit_material_path ~= self._material_config_path) then
			if unit_material_node:parameter("version") == self.MATERIAL_CONFIG_VERSION_TAG and EWS:message_box(self._main_frame, "Do you want to open: " .. unit_material_path, "Open", "OK,CANCEL", Vector3(-1,-1,-1)) == "OK" then
				self:_save_current()
