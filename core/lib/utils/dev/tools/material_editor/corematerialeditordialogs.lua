...
function CoreMaterialEditorStartDialog:on_new()
	...
	if path then
		if managers.database:has(path) then
			if EWS:MessageDialog(self._parent, "A material config with that name already exists. Do you want to replace it?", "Duplicated!", "YES_NO,ICON_ERROR"):show_modal() == "ID_NO" then
				return
			end
		end
