...
function CoreMaterialEditor:_create_main_frame()
	...
	self._main_frame:connect("PROBLEM_SOLVER", "EVT_COMMAND_MENU_SELECTED", function() EWS:launch_url("http://mondomonkey.com/MondoMonkeyWhiteB.jpg") end, "")
	...
	self._main_scroll_window:set_background_colour((EWS:get_system_colour("3DFACE") * 128):unpack()) -- Set a slightly darker backround color.
...
function CoreMaterialEditor:_build_section(shader_name, shader, node, tree)
	for child in node:children() do
		...
		if child:name() == "define" and (not project or project == Application:short_game_name()) then
			...
			local define_struct = {_define_node = child, _check_box = check_box, _checked = false}
...
function CoreMaterialEditor:_create_parameter_panel()
	...
	if self._material_config_node:num_children() > 1 and self._output_collapse_box:expanded() then
		-- progress_dialog = EWS:ProgressDialog(self._main_frame, "Building Interface", "Building interface widgets...", 100, "PD_AUTO_HIDE,PD_SMOOTH")
	end
