...
function CoreMaterialEditorParameter:init(parent, editor, parameter_info, parameter_node)
	...
	-- self._customize = (self._editor._current_material_node:parameter("src") == nil or not self._parent_node) -- This might be buggy.
...
function CoreMaterialEditorParameter:_load_value()
	if self._editor._current_material_node:parameter("src") then
		if not self._customize then
	...
	elseif self._parameter_info.type == "texture" then
		...
		if not self._value then
			...
			self._global_texture_type = "cube" --self._node:parameter("type") Since we don't have any other types we force 'cube'.
		end
	else
		 -- Little bit of a specail case.
		if self._parameter_info.ui_type:s() == "intensity" then
...
function CoreMaterialEditorParameter:_copy_to_parent(name)
	...
	if not parent_param_node then
		parent_param_node = material_node:add_child(self._parameter_node)
	end
