...
function CoreMaterialEditorTexture:on_pick_global_texture()
	-- I have changed the display name of the "current_global_texture" to "environment_cubemap" on artist request.
	-- In theory this texture does not need to be a cubemap... We need to sort out the use for this system later on. /Andreas
	local dialog = EWS:SingleChoiceDialog(self._editor._main_frame, "Pick a global texture.", "Global Textures", {"environment_cubemap"}, "")
	dialog:show_modal()
	
	local str = dialog:get_string_selection()
	if str ~= "" then
		self._value = "current_global_texture" -- str
		self._global_texture = true
		self._global_texture_type = "cube" --Just hardcoded.
		...
		self._text:set_value("environment_cubemap (Global)") -- self._value .. " (Global)")
...
function CoreMaterialEditorTexture:_on_browse()
	...
	if path then
		...
		-- This is a hard coded hack because I can't find out what type it is.
