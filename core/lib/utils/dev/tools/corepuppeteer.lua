...
function CorePuppeteer:init(unit)
	if (not unit) and managers.editor then
		-- No unit supplied. Perhaps there's a unit selected in editor-mode?
		unit = managers.editor:selected_unit()
	end

	if not unit then
		-- Nothing selected in the editor. Use the console-selected unit instead.
		unit = World:selected_unit()
	end
	
	-- Note that unit might still be nil here if nothing was selected.
...
function CorePuppeteer:_on_selected_unit_changed(selected_unit)
	if selected_unit == self:unit() then return end
