...
function CoreLuaProfiler:create_main_frame()
	...
	self._resource_sort_func = function(a, b) return a._name < b._name end
	local sort_by_name = self._resource_sort_func
	local sort_by_mem_use = function(a, b) return a._used > b._used end
	...
	--[[
	self._dump_menu = EWS:Menu("")
	self._dump_menu:append_item("DO_DUMP", "Dump XML", "")
	self._dump_menu:append_item("DUMP_OPEN", "Open Dump XML", "")
	menu_bar:append(self._dump_menu, "Dump")
	]]
	...
	---------------------------------------------------------------------------------------
	--[[ Deprecated!
	self._dump_frame_table = {}
	self._dump_frame_table._main_panel = EWS:Panel(self._main_notebook, "", "")
	self._dump_frame_table._main_panel_box = EWS:BoxSizer("VERTICAL")
	
	local box = EWS:BoxSizer( "HORIZONTAL" )
	
	self._dump_frame_table._search_text_ctrl = EWS:TextCtrl(self._dump_frame_table._main_panel, "", "", "TE_CENTRE,TE_PROCESS_ENTER")
	self._dump_frame_table._search_text_ctrl:connect("", "EVT_COMMAND_TEXT_ENTER", callback( self, self, "on_dump_search" ), "")	
	box:add(self._dump_frame_table._search_text_ctrl, 1, 0, "EXPAND")
	
	self._dump_frame_table._search_btn = EWS:Button(self._dump_frame_table._main_panel, "Search", "", "")
	self._dump_frame_table._search_btn:connect("", "EVT_COMMAND_BUTTON_CLICKED", callback( self, self, "on_dump_search" ), "")	
	box:add(self._dump_frame_table._search_btn, 0, 0, "EXPAND")
	
	self._dump_frame_table._main_panel_box:add(box, 0, 0, "EXPAND")
	
	self._dump_frame_table._tree_ctrl = EWS:TreeCtrl(self._dump_frame_table._main_panel, "", "TR_HAS_BUTTONS,TR_LINES_AT_ROOT,TR_DEFAULT_STYLE,SUNKEN_BORDER,TR_HIDE_ROOT")
	self._dump_frame_table._main_panel_box:add(self._dump_frame_table._tree_ctrl, 1, 0, "EXPAND")

	self._dump_frame_table._main_panel:set_sizer(self._dump_frame_table._main_panel_box)
	]]
	---------------------------------------------------------------------------------------
	...
	--self._main_notebook:add_page(self._dump_frame_table._main_panel, "LUA Dump", false)
...
function CoreLuaProfiler:fill_dump_tree_ctrl(node, id)
	for n in node:children() do
		...
		self._dump_tree_id_table[new_id] = {_id = new_id, _name = n:name(), _parent = id}
		
		for k,v in pairs(n:parameter_map()) do
			local new_k_id = self._dump_frame_table._tree_ctrl:append(new_id, k)
			self._dump_tree_id_table[new_k_id] = {_id = new_k_id, _name = k, _parent = new_id}
			...
			self._dump_tree_id_table[new_v_id] = {_id = new_v_id, _name = bin_str, _parent = new_k_id}
		end
...
function CoreLuaProfiler:on_select_profiler()
	...
	table.sort(selected_idices, function(a, b) return (a > b) end)
...
function CoreLuaProfiler:add_profiler(name)
	...
	if not self._profilers[func_table._source] then
		...
		--self._main_frame_table._profiler_list_ctrl:autosize_column(1)
...
function CoreLuaProfiler:update(t, dt)
	...
	if self:notebook_selected() == 0 then
		...
		if self._frames_sample_steps > 1 or not self._last_update or t - self._last_update > 0.5 then
			...
			if self._frames_since_profilers_reset >= self._frames_sample_steps then -- Means it is about to be reset.
...
function CoreLuaProfiler:on_tree_ctrl_change()
	--[[
	local id = self._resources_frame_table._tree_ctrl:selected_item()
	if id > -1 then
		local parent_id = self._resources_frame_table._tree_ctrl:get_parent(id)
		if parent_id > -1 then
			local parent_text = self._resources_frame_table._tree_ctrl:get_item_text(parent_id)
			if parent_text == "Script Extensions" then
				local unit_name = self._resources_frame_table._tree_ctrl:get_item_text(self._resources_frame_table._tree_ctrl:get_parent(parent_id))
				local extension_name = self._resources_frame_table._tree_ctrl:get_item_text(id)
					
				local unit
				for _,v in ipairs(self._unit_report) do
					if v._name == unit_name then
						unit = v._unit
						break
					end
				end
				...
				if unit then
					...
					self:remove_all_profilers()
					self._main_frame_table._profiler_list_ctrl:delete_all_items()
					self._class_name = class_name(getmetatable(unit[extension_name](unit)))
					self:find_methods(self:find_table(self._class_name)._table, self._class_table)
					
					local found = false
					if self:find_function("update") then
						self:add_profiler("update")
						found = true
					end
					
					if self:find_function("paused_update") then
						self:add_profiler("paused_update")
						found = true
					end
					...
					if found then
						core_lua_profiler_reload = true
						self._main_notebook:set_page(1)
					end
				end
			end
		end
...
function CoreLuaProfiler:on_update_resources()
	...
	-- Sort before presenting the resaults.
...
function CoreLuaProfilerSampleRateDialog:init(p)
	self._dialog = EWS:Dialog(p, "Sample Rate", "", Vector3(-1,-1,0), Vector3(200.0, 86.0, 0.0), "CAPTION,SYSTEM_MENU")
