...
function CoreUnitTestBrowser:search_frame()
	...
	-- self._search_box.search_btn = EWS:Button(self._search_frame, "Search", "", "")
	-- self._search_box.search_btn:connect("", "EVT_COMMAND_BUTTON_CLICKED", callback( self, self, "on_search" ), "")
	-- top_search_box:add(self._search_box.search_btn, 0, 0, "EXPAND")
...
function CoreUnitTestBrowser:on_ignore(custom_data, event)
	...
	if(custom_data) then
		self._error_box.tree_ctrl:set_item_text_colour(id, Vector3(1.0, 1.0, 1.0))
		self._error_box.tree_ctrl:set_item_background_colour(id, Vector3(1.0, 0.0, 0.0))
	else
		self._error_box.tree_ctrl:set_item_text_colour(id, Vector3(1.0, 1.0, 1.0))
		self._error_box.tree_ctrl:set_item_background_colour(id, Vector3(0.5, 0.0, 0.0))
	end
...
function CoreUnitTestBrowser:on_include(custom_data, event)
	local id = self._error_box.tree_ctrl:selected_item()
	if(custom_data) then
		self._error_box.tree_ctrl:set_item_text_colour(id, Vector3(1.0, 0.0, 0.0))
		self._error_box.tree_ctrl:set_item_background_colour(id, Vector3(1.0, 1.0, 1.0))
	else
		self._error_box.tree_ctrl:set_item_text_colour(id, Vector3(0.5, 0.0, 0.0))
		self._error_box.tree_ctrl:set_item_background_colour(id, Vector3(1.0, 1.0, 1.0))
	end
	...
	if(search_str ~= "") then
		for key, value in pairs(self._unit_msg) do
			if(value.author and value.diesel and value.id) then		
				...
				if(string.len(search_str) <= string.len(str)) then
					if(value.note == "failed" or value.note == "critical") then
	...
	for unit_node in self._report_xml:children() do
		if(unit_node:name() == "unit") then
			...
			if(found_critical) then
				...
				self._error_box.tree_ctrl:set_item_text_colour(self._unit_msg[unit_node:parameter("name")].id, Vector3(1.0, 0.0, 0.0))
			elseif(found_error) then
				...
				self._error_box.tree_ctrl:set_item_text_colour(self._unit_msg[unit_node:parameter("name")].id, Vector3(0.5, 0.0, 0.0))
			else
				...
				self._error_box.tree_ctrl:set_item_text_colour(self._unit_msg[unit_node:parameter("name")].id, Vector3(0.0, 0.5, 0.0))
			end
		end
	end
	
	self._unit_msg["Units"] = {}; self._unit_msg["Units"].msg = tostring(num_units) .. " units tested."
	self._unit_msg["Passed"] = {}; self._unit_msg["Passed"].msg = tostring(num_passed) .. " / " .. tostring(num_units) .. " units passed the test."
	self._unit_msg["Failed"] = {}; self._unit_msg["Failed"].msg = tostring(num_failed) .. " / " .. tostring(num_units) .. " units failed the test."
	self._unit_msg["Critical"] = {}; self._unit_msg["Critical"].msg = tostring(num_critical) .. " / " .. tostring(num_units) .. " units is in a critical condition."
end
...
function CoreUnitTestBrowserInputDialog:init(p)
	self._dialog = EWS:Dialog(p, "Receiver", "", Vector3(0.0, 0.0, 0.0), Vector3(300.0, 86.0, 0.0), "CAPTION,SYSTEM_MENU")
