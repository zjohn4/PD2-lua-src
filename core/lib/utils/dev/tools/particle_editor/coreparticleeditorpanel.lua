...
function CoreParticleEditorPanel:init(editor, parent, effect)
	...
	self._undo_stack = CoreUndoStack:new({name=effect:name(), xml=n:to_xml()}, 20)
...
function CoreParticleEditorPanel:create_panel(parent)
	...
	gv_splitter:set_sash_gravity(0.0)
...
function CoreParticleEditorPanel:on_paste_atom()
	if self._editor._clipboard_type == "atom" and self._editor._clipboard_object then
		...
		local i = 0
		local n = self._editor._clipboard_object:name() .. "" -- create copy of string
		while e do 
...
function CoreParticleEditorPanel:on_stack_add(stacktype)
	if not self._atom then return end
	...
	if to_add_idx < 0 then return end
...
function CoreParticleEditorPanel:on_select_stack_member(stacktype)
	...
	if (selected < 0) then 
		--self._stack_panels[stacktype]:clear()
	else
...
function CoreParticleEditorPanel:reload_effect_definition()
	...
	CoreEngineAccess._editor_reload_node(n, Idstring("effect"), Idstring("unique_test_effect_name")) -- this will kill existing instance
	self._effect_id = nil
end
...
function CoreParticleEditorPanel:update(t, dt)
	if self._valid_effect then 
		...
		if (not self._effect_id or not World:effect_manager():alive(self._effect_id)) and self._frames_since_spawn > 1 then -- spawn effect
			local quality = self._quality
			if not quality then quality = 0.5 end
			self._quality = nil
			
			local gizmo = self._editor:effect_gizmo()
			self._effect_id = World:effect_manager():spawn({effect=Idstring("unique_test_effect_name"), parent=gizmo:get_object(Idstring("rp_root_point")), custom_quality=quality})
			self._frames_since_spawn = 0
		else
...
function CoreParticleEditorPanel:update_graph_view()
	...
	if not self._atom then return end
	--if not self._valid_effect then return end
	
	local stacks = {"initializer", "simulator", "visualizer"}
	local row = 0
...
function CoreParticleEditorPanel:update_view(clear, undoredo)
	-- keep xml in stack for value comparison
	local n = Node("effect")
	self._effect:save(n)
	local new_xml = n:to_xml()
	
	if not undoredo then 
		self._undo_stack:push({name=self._effect:name(), xml=new_xml}) -- effect has been changed, push this state onto undo stack
	end
	
	local name = self._effect:name()
	if name == "" then name = "New Effect" end
	...
	if clear then 
		for stacktype,c in pairs(self._stacklist_boxes) do
			...
			self._stack_panels[stacktype]:set_sizer(EWS:BoxSizer("VERTICAL")) -- must do this since old sizer in panel references dead child windows
		end
	end
...
function CoreParticleEditorPanel:on_stack_paste(stacktype)
	if self._editor._clipboard_type ~= stacktype or not self._editor._clipboard_object then return end
...
function CoreParticleEditorPanel:on_key_stack_member(stacktype, event)
	if event:control_down() then
		if event:key_code() == 67  then -- C 
			self:on_stack_copy(stacktype)
		elseif event:key_code() == 86 then -- V
...
function CoreParticleEditorPanel:create_atom_panel(parent) -- TODO: make pages scrollwindows
	local panel = EWS:Panel(parent, "", "")
...
function CoreParticleEditorPanel:on_save_as()
	...
	self._undo_stack:push({name=self._effect:name(), xml=new_xml}) 
...
function CoreParticleEditorPanel:do_save(warn_on_overwrite)
	if warn_on_overwrite and managers.database:has(self._effect:name()) then
		...
		if ret ~= "YES" then return false end
	end
	...
	if self._valid_effect then 
		...
		PackageManager:reload(Idstring("effect"), managers.database:entry_path(self._effect:name()):id()) -- update definition at save, useful for effects including other effects etc.
	end
...
function CoreUndoStack:push(state)
	if self._stack[self._ptr] == state then return end
	
	while self._ptr < #self._stack do table.remove(self._stack, self._ptr) end -- push invalidates redo - remove all elements above pointer
	
	table.insert(self._stack, state) -- append state, remove first element if stack size is exceeded
...
function CoreUndoStack:undo()
	if self._ptr == 1 then return nil end
	self._ptr = self._ptr - 1
	return self._stack[self._ptr]
end

function CoreUndoStack:redo()
	if self._ptr == #self._stack then return nil end
	self._ptr = self._ptr + 1
	return self._stack[self._ptr]
end
