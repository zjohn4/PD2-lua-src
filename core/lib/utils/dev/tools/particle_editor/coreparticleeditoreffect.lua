-- CoreEffectDefinition/CorreEffectStackMember
-- CoreEffectDefinition/CorreEffectStackMember
...
-- CoreEffectDefinition/CoreEffectStack
...
function CoreEffectStack:move_down(idx)
	if idx == #self._stack then return end
...
function CoreEffectStack:validate(channels)
   ...
   if self._type == "visualizer" then
	...
	-- check that no reads are made from channels that haven't been written
	...
	for _,m in ipairs(self._stack) do 
		...
		for _,a in ipairs(access) do 
			if a["access"] == "WRITE" or a["access"] == "WRITE_READ" then -- skip WRITE_SOME since write is not full
	...
	-- special case for simulator, check that position channel is fully produced for bounding volume
...
The default playback mode is the medium quality."
	...
	--self:add_property(CoreEffectProperty:new("start_time", "time", "0", help))
...
For more advanced event handling, script callbacks can be registered with effect instances at runtime."
	...
	-- stacks
	self._stacks = { initializer = CoreEffectStack:new("initializer"),
...
function CoreEffectAtom:collect_stack_time_events()
	...
	local function traverse_property(pref, ret, p) 
		if p._type == "time" then
			table.insert(ret, {pref .. p:name() , p, "_value"})
		...
		elseif p._type == "keys" then
			for _,k in ipairs(p._keys) do
				table.insert(ret, {pref .. p:name(), k, "t"})
			end
		end
	end
...
-- returns a list of { caption name, reference to table with time member, name of time member (or index) }
function CoreEffectAtom:collect_time_events()
	...
	-- times are relative to start time, offset each for display 
	...
	table.insert(ret, {"start_time", start_time , "_value"})
	
	if tonumber(lifetime._value) >= 0 then
		table.insert(ret, {"end_time", {tonumber(start_time._value) + tonumber(lifetime._value)}, 1} )
	end
...
function CoreEffectAtom:scale_timeline(istart, iend, tstart, tend)
	...
	local start_time = self:get_property("random_start_time")._variants["false"]
	local lifetime = self:get_property("lifetime")
	local end_time = {"end_time", lifetime, "_value"} 

	if tonumber(lifetime._value) >= 0 then -- end time does not exist for infinite time effects
		table.insert(events, end_time)
	end
	
	local start_time_v = tonumber(start_time._value)
	
	for _,e in ipairs(events) do -- transfer all from relative to start time to absolute
		e[2][e[3]] = e[2][e[3]] + start_time_v
	end
	
	table.insert(events, {"start_time", start_time , "_value"})
	...
	for _,e in ipairs(events) do -- transfer all except start time/end time back to relative
		if e[2] ~= start_time then e[2][e[3]] = e[2][e[3]] - new_start_time_v end
	end
end
...
function CoreEffectAtom:validate()
	...
	--local cull_gracetime = tonumber(self:get_property("cull_gracetime"):value())
	--if not cull_gracetime or cull_gracetime < 0 then 
	--	ret["valid"] = false
	--	ret["message"] = "Invalid gracetime for atom " .. self:name()
	--	return ret
	--end
...
function CoreEffectDefinition:init()
	--self._atoms = {CoreEffectAtom:new("default")}
...
function CoreEffectDefinition:validate()
	-- check that definition is valid for feeding to the effect system, if not return error message
	local ret = {}
	ret["valid"] = true
	ret["message"] = ""
	ret["node"]= nil -- TODO: set to the node in the view "closest to the error"
...
function CoreEffectDefinition:save(n)
	...
	--cat_debug( "debug", "effect xml:")
	--cat_debug( "debug", n:to_xml())
end
