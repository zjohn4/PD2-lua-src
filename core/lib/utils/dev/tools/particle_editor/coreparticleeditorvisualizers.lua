...
add_no_alpha - color becomes color of destination + billboard color"
	...
	-- p:add_value("effect_op_add_no_fog")
	p:add_value("effect_geometry_fade_out")
	p:add_value("effect_geometry_fade_out_add")
	-- p:add_value("effect_op_no_fog_depth_patch")
	-- p:add_value("effect_nm_df_specmod_op")
	-- p:add_value("effect_nm_specmod_op")
	...
	-- make variant parameter that changes appearance and value based on combo box selection
...
scaled_channel - read from channel, scale with key curve"
	...
	size_keys:set_presets( { {"Grow",            { {0, Vector3(100, 100, 0)}, {2, Vector3(200, 200, 0) } }},
							 {"Shrink",          { {0, Vector3(200, 200, 0)}, {2, Vector3(100, 100, 0) } }},
							 {"Grow And Shrink", { {0, Vector3(100, 100, 0)}, {1, Vector3(200, 200, 0) }, {2, Vector3(100, 100, 0)} }} })
...
scaled_channel - read from channel, scale with key curve"
	...
	color_keys:add_key({t=0, v="255 255 255"})
...
scaled_channel - read from channel, scale with key curve"
	...
	rotation_keys:set_presets( { {"Spin Then Pan",   { {0, Vector3(0, 0, 0)}, {1, Vector3(360*10, 0, 0) } }},
							     {"Spin",        { {0, Vector3(0, 0, 0)}, {2, Vector3(360*20, 0, 0) } }},
								 {"Spin Fast Then Slow",{ {0, Vector3(0, 0, 0)}, {1, Vector3(360*10, 0, 0) }, {2, Vector3(360*15, 0, 0) } }} })
