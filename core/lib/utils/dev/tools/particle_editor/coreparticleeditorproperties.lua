...
function CoreEffectPropertyContainer:ui_name() 
	if self._ui_name then return self._ui_name else return self:name() end
end
...
function CoreEffectPropertyContainer:validate_properties()
	local ret = {valid=true, message="", node=nil}
	
	for _,p in ipairs(self._properties) do
		ret = p:validate()
		if not ret["valid"] then return ret end
	end
...
function CoreEffectPropertyContainer:fill_property_container_sheet(window, view)
	...
	--set_widget_help(window, property_container:description())
	...
	for i,p in ipairs(property_container:properties()) do
		if p._visible then 
			...
			--set_widget_help(name_text, p:help())
...
-- TODO: replace this union with inheritance and overloaded create_widget() etc
-- types: (variant, compound, list_objects), value_list, time, int, vector2, vector3, percentage, texture, color, opacity, angle, float, keys, box
function CoreEffectProperty:init(name, ptype, value, help)
	...
	self._values = {}
	self._variants = {}
	self._can_be_infinite = false -- time can be infinite
	self._silent = false -- will not be saved
...
function CoreEffectProperty:set_key_type(t)
	if t == "angle" then t = "float" 
	elseif t == "time" then t= "float" 
	elseif t == "opacity" then t= "float" 
	end
	self._key_type = t
	self._keys = {}
	self._silent = true
...
-- Widgets should be used that enforce valid values, but until all widgets are implemented
-- and also to prevent errors from loaded definitions (with ex. missing textures), extra validation is needed
	...
	if self._type == "value_list" then
		local contains = function(l,v)
			for _, value in ipairs( l ) do
				if type(value) == "userdata" and type(v) == "string" or
					type(value) == "string" and type(v) == "userdata"
				then
					if value:s() == v:s() then
						return true
					end
...
function CoreEffectProperty:add_value(v) -- for value list type properties
	table.insert(self._values, v)
end

function CoreEffectProperty:add_variant(name, prop) -- for variant type properties - a different property is presented/saved depending on selected variant
...
function CoreEffectProperty:on_commit(widget_view)
	-- TODO: handle different widget types by looking at self._type
	if self._type == "null" then return end
...
function create_color_selector(parent, view, prop)
	...
	local function on_click(vars)
		local cdlg = EWS:ColourDialog(vars.button, true, math.string_to_vector(vars.prop._value)*(1.0/255))
	...
	local vars = {button=button, prop=prop, view=view, update_colour=update_colour}
...
function create_percentage_slider(parent, view, prop)
	...
	local function on_thumbtrack(vars)
		vars.prop._value = "" .. vars.slider:get_value() / 100.0
		vars.view:update_view(false)
	end
	
	slider:connect("EVT_SCROLL_THUMBRELEASE", on_thumbtrack, {slider=slider,view=view,prop=prop})
	return slider
end
...
function create_check(parent, view, prop)
	...
	check:connect("EVT_COMMAND_CHECKBOX_CLICKED", on_check, {check=check,view=view,prop=prop})
	return check
end

-- TODO: create custom widget engine-side that can display key curves / migrate old particleeditor widget
function create_key_curve_widget(parent, view, prop)
	...
	local function on_add(vars)
		...
		if #prop._keys < prop._max_keys then
			prop:add_key({t=t:get_value(), v=v:get_value()})
	...
	local function on_remove(vars)
		...
		local v = vars["v"]
		local prop = vars["prop"]
		if listbox:selected_index() < 0 then return end
	...
	local vars = {listbox=listbox, t=t, v=v, prop=prop, refresh_list=refresh_list, view=view}
	refresh_list(vars)
...
function CoreEffectProperty:create_widget(parent,view)
	...
	-- create different widgets depending on type
	
	if self._type == "value_list" then -- combobox with set values
	...
	elseif self._type == "variant" then
		...
		combo:connect("EVT_COMMAND_TEXT_UPDATED",  callback(self, self, "on_set_variant", {combo=combo, container=widget, view=view, variant_panel=variant_panel, container_sizer=sizer, update=true}))
		combo:set_value(self._value)
		
		widget:set_sizer(sizer)
		widget:set_min_size(sizer:get_min_size())
		
		self:on_set_variant({combo=combo, container=widget, view=view, variant_panel=variant_panel, container_sizer=sizer, update=false})
	...
	elseif self._type == "list_objects" then
		...
		local function on_remove_object(vars)
			if vars.list_box:selected_index() < 0 then return end
		...
		local top_sizer = EWS:BoxSizer("VERTICAL")
		local vars = {property=self, combo=combo, container=widget, container_sizer=top_sizer, list_box=list_box, fill_list=fill_list, sheet=sheet, view=view, on_select_object=on_select_object}
		...
		--add_button:connect("EVT_COMMAND_BUTTON_CLICKED", on_add)
	...
	elseif self._type == "keys" then
		-- widget = create_key_curve_widget(parent, view, self)
		...
		local function on_keys_commit(widget_view)
			...
			for _,k in ipairs(keys) do 
				...
				table.insert(prop._keys, {t=k.t, v=s})
			end
		...
		widget:connect("EVT_SELECTOR_UPDATED", on_keys_commit, {widget=widget, view=view, prop=self})
	...
	--removed by request since it obstructs the entry field:
	--set_widget_help(widget, self._help)
	return widget
end
...
function CoreEffectProperty:save(node)
	...
	elseif self._type == "list_objects" then 
		...
		elseif self._type == "keys" then
			...
			local ls = "false"
			if self._looping then ls = "true" end
