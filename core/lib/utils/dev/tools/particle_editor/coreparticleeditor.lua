...
function collect_member_names(members, member_names)
	for k,v in pairs(members) do
		local vi = v()
		table.insert(member_names, {ui_name=vi:ui_name(), key=k})
	end
	
	table.sort(member_names, function(a, b) return a.ui_name < b.ui_name end)
end

stack_members = {initializer={}, simulator={}, visualizer={}}
stack_member_names = {initializer={}, simulator={}, visualizer={}}
...
function CoreParticleEditor:init()
	if managers.editor then
		managers.editor:set_listener_enabled( true ) -- enable sounds
	end
...
function CoreParticleEditor:new_dialog()
	...
	local function on_select_type(combo_desc) 
		...
		local desc = ""
		-- TODO: keep description per template somehow, perhaps in its own file?
...
function CoreParticleEditor:on_undo()
	...
	if cur_effect then cur_effect:undo() end
end
...
function CoreParticleEditor:on_batch_all_remove_update_render()
	...
	for _, name in ipairs(managers.database:list_entries_of_type("effect")) do
		...
		for _,atom in ipairs(effect._atoms) do
			...
			if cull_policy._value == "update_render" then -- check if visualizer stack has a billboardvisualizer with screen_aligned set
				local had_screen_aligned = false
				
				--cull_policy._value = "freeze"
				for _,visualizer in ipairs(atom._stacks["visualizer"]._stack) do
					if visualizer:name() == "billboard" then
						if visualizer:get_property("billboard_type")._value == "screen_aligned" then
		...
		if should_save then
			...
			--cat_debug( "debug", "Saving " .. name .. "...")
			--any_saved = true
			--cat_debug( "debug", "Saving " .. name .. "...")
			--n = Node("effect")
			--effect:save(n)
			--ProjectDatabase:save_node(n, name)
		end
		
		--if had_freeze then
		--	cat_debug( "debug", "Effect " .. entry:name() .. " had cull_policy=update_render") 
		--end
		--
		--if had_screen_aligned then
		--	cat_debug( "debug", "Effect " .. entry:name() .. " had billboard_type=screen_aligned") 
		--end
	end
...
function CoreParticleEditor:on_batch_all_load_unload()
	...
	for _, name in ipairs(managers.database:list_entries_of_type("effect")) do
		...
		if not valid["valid"] then
			...
			CoreEngineAccess._editor_reload_node(n, Idstring("effect"), Idstring("unique_test_effect_name")) -- this will kill existing instances
		end
	end
...
function CoreParticleEditor:on_redo()
	...
	if cur_effect then cur_effect:redo() end
end

function CoreParticleEditor:on_effect_changed(arg, event)
	if event:get_id() ~= "EFFECTS_NOTEBOOK" then return end
	...
	if new_page >= 0 and new_page < #self._effects then
		...
		if self._view_menu:is_checked("SHOW_STACK_OVERVIEW") then new_effect:show_stack_overview(true) end
	end
...
function CoreParticleEditor:on_play()
	...
	if cur_effect then cur_effect:update_effect_instance() end
end

function CoreParticleEditor:on_play_lowest()
	local cur_effect = self:current_effect()
	
	if cur_effect then cur_effect:update_effect_instance(0) end
end

function CoreParticleEditor:on_play_highest()
	local cur_effect = self:current_effect()
	
	if cur_effect then cur_effect:update_effect_instance(1) end
end
...
function CoreParticleEditor:on_show_stack_overview()
	local cur_effect = self:current_effect()
	if cur_effect then cur_effect:show_stack_overview(self._view_menu:is_checked("SHOW_STACK_OVERVIEW") ) end
end
...
function CoreParticleEditor:update(t, dt)
	...
	if cur_effect then cur_effect:update(t, dt) end
...
function CoreParticleEditor:on_close_effect()
	...
	if curi > 0 then 
		if not self:current_effect():close() then return end
...
function CoreParticleEditor:on_close()
	...
	managers.toolhub:close("Particle Editor")
	if(managers.editor) then
		managers.editor:set_listener_enabled( false ) -- disable sounds
	end
end
...
function CoreParticleEditor:add_effect(effect)
	...
	effect_panel:set_init_positions() -- workaround for wxSplitterWindow not being resizable when window is not visible
...
function CoreParticleEditor:on_save()
	...
	if cur then cur:on_save() end
end

function CoreParticleEditor:on_save_as()
	local cur = self:current_effect()
	if cur then cur:on_save_as() end
end
