...
function ToolHub:update( time, rel_time )
	...
	-- Now see if we have to close any tools down.
...
function ToolHub:buildmenu()
	...
	-- Sort keys
	local sortkeys = {}
	for n in pairs( self._shed ) do sortkeys[#sortkeys + 1] = n end
	table.sort( sortkeys ) 
	...
	-- file_menu:append_item("TB_MESHDUMP", 	"Mesh Dump ( 100 M )", "")
	...
	-- self._main_frame:connect("TB_MESHDUMP", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "on_meshdump" ), "")

	-- Cheats menu
	-- self._cheatmenu = EWS:Menu("")
	-- self._cheatmenu:append_check_item("TB_CHEAT_GOD", "God Mode", "")
	-- menu_bar:append(self._cheatmenu, "Cheat")
 	-- self._main_frame:connect("TB_CHEAT_GOD", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "on_cheat" ), "")
	...
	for _, key in ipairs( sorted_category_list ) do
		...
		-- Set correct checked status of menu
	...
	if( SystemInfo:platform() == Idstring("WIN32") ) then
		...
		self._main_frame:connect("4/3", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "change_aspect_ratio" ), 4/3 )
		self._main_frame:connect("16/9", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "change_aspect_ratio" ), 16/9 )
		self._main_frame:connect("16/10", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "change_aspect_ratio" ), 16/10 )
