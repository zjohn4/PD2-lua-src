...
-- =============================================================================
--  class: D e p e n d e n c y P a r s e r
--
-- =============================================================================
...
function DependencyParser:_dn(...) --> dn/nil
...
function DependencyParser:nodes(pattern) --> { dn1, dn2 ... }
	local dn_list = {}
...
function DependencyParser:complement(dn_list, pattern) --> { dn1, dn2 ...}
	local all_dn = self:nodes()
	local list = set_difference(all_dn, dn_list)
	return filter(list, pattern)
end

function DependencyParser:reached(start_dn_list, pattern) --> { dn1, dn2 ... }
	local reached_dn = {}
...
function DependencyParser:not_reached(start_dn_list, pattern) --> { dn1, dn2 ... }
...
-- =============================================================================
--  Functions for manipulating lists as set, which is stupid,
--  but easier for the user since many of the functions will
--  initially be used on the console.
-- =============================================================================

function _set2list(set) --> { n1, n2 ... }
	local list = {}
...
function _list2set(list) --> { n1=n1, n2=n2 ... }
	local set = {}
...
function union(A_list, B_list) --> C_list
	local set = {}
...
function intersect(A_list, B_list) --> C_list
...
function set_difference(A_list, B_list) --> C_list
...
function names(A_list) --> { 'name1', 'name2' ... }
	local names = {}
	for _,dn in ipairs(A_list) do
...
function filter(dn_list, pattern) --> { dn1, dn2 ... }
	res_list = {}
...
-- =============================================================================
--  Helper functions for generating XML-report.
-- =============================================================================
...
function generate_report( filepath, protected_list, dp )
	...
	function make_protected_worksheet()
		...
		for i = 1,size,1 do
			ws:add_row( CoreSsRow.Row:new( levels[i] or '', 
			                               units[i] or '',
			                               objects[i] or '', 
			                               models[i] or '', 
			                               mtrlcfgs[i] or '',
...
function generate_BC_report( filepath )
	...
	prot = union( prot, filter( textures, 'security_camera_.*' ) ) -- HUD related
...
function generate_FAITH_report( filepath )
	...
	local prot = {}
--	prot = union( prot, filter( levels, 'abc.*' ) )
--	prot = union( prot, filter( units, 'abc.*' ) )
--	prot = union( prot, filter( objects, 'abc.*' ) )
--	prot = union( prot, filter( models, 'abc.*' ) )
--	prot = union( prot, filter( mtrlcfgs, 'abc.*' ) )
--	prot = union( prot, filter( textures, 'abc.*' ) )
--	prot = union( prot, filter( cutscenes, 'abc.*' ) )
--	prot = union( prot, filter( effects, 'abc.*' ) )
