...
--	managers.ai_graph:internal_load( managers.editor:get_open_dir() )
			...
			if selected then
				for id,_ in pairs( unit:ai_editor_data().visibilty_exlude_filter ) do
					for _,to_unit in ipairs( self._created_units ) do
						if to_unit:unit_data().unit_id == id then
							-- Application:draw_arrow( unit:position(), to_unit:position(), 1, 0, 0, 0.5 )
							Application:draw_link( { from_unit = unit, to_unit = to_unit, r = 1, g = 0, b = 0 } )
							-- Application:draw_link( { from_unit = unit, to_unit = to_unit, r = 1, g = 0, b = 0 } )
						end
					end
				end
				for id,_ in pairs( unit:ai_editor_data().visibilty_include_filter ) do
					for _,to_unit in ipairs( self._created_units ) do
						if to_unit:unit_data().unit_id == id then
							-- Application:draw_arrow( unit:position(), to_unit:position(), 1, 0, 0, 0.5 )
							Application:draw_link( { from_unit = unit, to_unit = to_unit, r = 0, g = 1, b = 0 } )
							-- CoreMissionElement._draw_link( { from_unit = unit, to_unit = to_unit, r = 1, g = 0, b = 0 } )
						end
					end
				end
			end
		elseif unit:name() == self._patrol_point_unit then
		--	self:_draw_patrol_point( unit, t, dt )
		end
	end
...
function AiLayer:_draw_surface( unit, t, dt, a, r, g, b )
	-- self:draw_sizes( unit, t, dt, a, r, g, b )
...
function AiLayer:_draw_patrol_path( name, path, t, dt )
	...
	if #path.points > 0 then
		for i,point in ipairs( path.points ) do
			...
			-- self._patrol_path_brush:cylinder( point.unit:position(), to_unit:position(), selected_path and 4 or 3, 4 )
			-- self._patrol_path_brush:cylinder( point.unit:position(), to_unit:position(), selected_path and 4 or 3, 4 )
			-- Application:draw_line( point.unit:position(), to_unit:position(), selected_path and 1 or 0.5, selected_path and 1 or 0.5, selected_path and 1 or 0.5 )
			Application:draw_link( { from_unit = point.unit, to_unit = to_unit, r = 1, g = 1, b = 1, thick = true, circle_multiplier = selected_path and 0.5 or 0.25, height_offset = 0 } )
...
function AiLayer:build_panel( notebook )
	...
	local ai_sizer = EWS:BoxSizer("VERTICAL")
		...
		--[[local draw_sizes_cb = EWS:CheckBox( self._ews_panel, "Draw Sizes", "" )
		draw_sizes_cb:set_value( self._draw_sizes )
		ai_sizer:add( draw_sizes_cb, 0, 5, "EXPAND,TOP" )
		draw_sizes_cb:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "cb_toogle" ), { cb = draw_sizes_cb, value = "_draw_sizes" } )]]
		...
		local graphs_sizer = EWS:StaticBoxSizer( self._ews_panel, "VERTICAL", "Graphs" )
			...
			local visualize_sizer = EWS:StaticBoxSizer( self._ews_panel, "VERTICAL", "Visualize" )
				...
				-- self._debug_buttons[ "covers" ] = EWS:RadioButton( self._ews_panel, "Covers", "draw_debug", "" )
...
-- Callback function for when the navigation segments have been calculated
...
function AiLayer:_build_visibility_graph()
	-- local all_visible = nil or self._all_visible:get_value() and "all_visible"
	...
	managers.navigation:build_visibility_graph( callback( self, self, "_visibility_graph_done" ), all_visible, exclude, include, ray_lenght ) -- complete callback, visibility filter, ray distance
	
end

-- Callback function for when the visibility graph have been calculated
function AiLayer:_visibility_graph_done()
	managers.editor:output( 'Visibility graph calculated' )
end

-- Returns a table with build settings for the navigation manager
...
-- Returns a table with units, depending of graph type and build type
...
function AiLayer:_add_to_visible_exlude_filter( unit )
	...
	if self._selected_unit and self._editor_data.virtual_controller:down( Idstring( "visible_exlude_filter" ) ) then
		if unit ~= self._selected_unit then
			if self._selected_unit:ai_editor_data().visibilty_exlude_filter[ unit:unit_data().unit_id ] then
				...
				self._selected_unit:ai_editor_data().visibilty_include_filter[ unit:unit_data().unit_id ] = nil -- Remove from include
				unit:ai_editor_data().visibilty_include_filter[ self._selected_unit:unit_data().unit_id ] = nil -- Remove from include
	...
	if self._selected_unit and self._editor_data.virtual_controller:down( Idstring( "visible_include_filter" ) ) then
		if unit ~= self._selected_unit then
			if self._selected_unit:ai_editor_data().visibilty_include_filter[ unit:unit_data().unit_id ] then
				...
				self._selected_unit:ai_editor_data().visibilty_exlude_filter[ unit:unit_data().unit_id ] = nil -- Remove from exlude
				unit:ai_editor_data().visibilty_exlude_filter[ self._selected_unit:unit_data().unit_id ] = nil -- Remove from exlude
...
function AiLayer:_insert()
	...
	local point = table.remove( path.points ) -- Remove from last ..
	table.insert( path.points, i + 1, point ) -- .. and insert at correct index
end

function AiLayer:delete_unit( unit )
	for _,u in ipairs( self._created_units ) do
		if u:name() == self._nav_surface_unit then
			if u ~= unit then
...
-- function NavigationManager:build_visibility_graph( complete_clbk )
