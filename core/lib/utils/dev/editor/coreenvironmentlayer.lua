...
function EnvironmentLayer:init( owner )
	...
	self._wind_speeds = {}
	table.insert( self._wind_speeds, { speed = 0, beaufort = 0, description = "Calm" } )
	table.insert( self._wind_speeds, { speed = 0.3, beaufort = 1, description = "Light air" } )
	table.insert( self._wind_speeds, { speed = 1.6, beaufort = 2, description = "Light breeze" } )
	table.insert( self._wind_speeds, { speed = 3.4, beaufort = 3, description = "Gentle breeze" } )
	table.insert( self._wind_speeds, { speed = 5.5, beaufort = 4, description = "Moderate breeze" } )
	table.insert( self._wind_speeds, { speed = 8.0, beaufort = 5, description = "Fresh breeze" } )
	table.insert( self._wind_speeds, { speed = 10.8, beaufort = 6, description = "Strong breeze" } )
	table.insert( self._wind_speeds, { speed = 13.9, beaufort = 7, description = "Near Gale" } )
	table.insert( self._wind_speeds, { speed = 17.2, beaufort = 8, description = "Fresh Gale" } )
	table.insert( self._wind_speeds, { speed = 20.8, beaufort = 9, description = "Strong Gale" } )
	table.insert( self._wind_speeds, { speed = 24.5, beaufort = 10, description = "Whole storm" } )
	table.insert( self._wind_speeds, { speed = 28.5, beaufort = 11, description = "Violent storm" } )
	table.insert( self._wind_speeds, { speed = 32.7, beaufort = 12, description = "Hurricane" } )
		
	self._draw_wind = false

	self._wind_speed = 6.0
	self._wind_speed_variation = 1.0
	...
	self._environment_modifier_id = self._owner:viewport():create_environment_modifier(false, function(interface) return self:sky_rotation_modifier(interface) end, "sky_orientation")
end
...
function EnvironmentLayer:load( world_holder, offset )
	...
	-- Remove any selections made by load
...
function EnvironmentLayer:old_load( environment )
	...
	if environment._wind then -- This check will ensure that we wont get nil values in the system
...
function EnvironmentLayer:save()
	...
	for _,unit in ipairs( self._created_units ) do
		if unit:name() == Idstring( self._effect_unit ) then
			...
			table.insert( effects, { name = effect, position = unit:position(), rotation = unit:rotation() } )
	...
	self._env_panel:set_sizer(self._env_sizer)
		...
		self._environment_sizer:add( sky_sizer, 0, 0, "EXPAND")
			...
			local env_cc_sizer = EWS:BoxSizer( "HORIZONTAL" )
				...
				local gradings = { "color_off", "color_payday", "color_heat", "color_nice", "color_sin", "color_bhd", "color_xgen", "color_xxxgen", "color_matrix", }
				-- table.sort( gradings )
		...
		self._environment_sizer:add( EWS:StaticLine( self._env_panel, "", "LI_HORIZONTAL" ), 0, 0, "EXPAND" )
			...
			local transition_sizer = EWS:BoxSizer( "HORIZONTAL" )		
				...
				-- transition:connect( "EVT_COMMAND_TEXT_UPDATED", callback( self, self, "set_transition_time" ), transition )
		...
		local resolution_sizer = EWS:BoxSizer( "HORIZONTAL" )
				...
				local resolution = EWS:ComboBox( self._env_panel, "", "", "CB_DROPDOWN,CB_READONLY")
					...
					for _,res in pairs( { 64, 128, 256, 512, 1024 } ) do
	...
	if type == "all" then
		for _,unit in ipairs( self._created_units ) do
			if unit:name() == Idstring( self._cubemap_unit ) then
				table.insert( cubes, { position = unit:position(), name = unit:unit_data().name_id, output_name = "outputcube" } )
			end
		end
	elseif type == "selected" then
		if self._selected_unit:name() == Idstring( self._cubemap_unit ) then
			table.insert( cubes, { position = self._selected_unit:position(), name = self._selected_unit:unit_data().name_id, output_name = "outputcube" } )
		end
	end
...
function EnvironmentLayer:change_color_grading( ctrlr )
	-- print( "EnvironmentLayer:change_color_grading()", ctrlr:get_value() )
...
function EnvironmentLayer:unit_ok( unit )
	return unit:name() == Idstring( self._effect_unit ) or unit:name() == Idstring( self._cubemap_unit ) 
			or unit:name() == Idstring( self._environment_area_unit )
			or unit:name() == Idstring( self._dome_occ_shape_unit )
end
...
function EnvironmentLayer:delete_unit( unit )
	...
	if unit:name() == Idstring( self._dome_occ_shape_unit ) then
		if unit:unit_data().occ_shape:panel() then
...
function EnvironmentLayer:play_effect( unit, effect )
	...
	if DB:has( "effect", effect ) then
		...
		unit:unit_data().current_effect = World:effect_manager():spawn( { effect = Idstring( effect ), position = unit:position(), rotation = unit:rotation() } )
	end
end
...
function EnvironmentLayer:update_unit_settings()	
	...
	if alive( self._selected_unit ) then 
		if self._selected_unit:name() == Idstring( self._effect_unit ) then
...
function EnvironmentLayer:reset_environment_values()
	...
	-- "color_off", "color_payday", "color_heat", "color_nice", "color_sin", "color_bhd", "color_xgen", "color_xxxgen", "color_matrix",
end
...
function EnvironmentLayer:clear()
	...
	-- sky:set_value( "rotation", self._environment_values.sky_rot )
	--managers.environment:setrotation(self._environment_values.sky_rot) -- Should you do this????
	
--	managers.environment:set( self._environment_values.environment )
	managers.environment_area:set_default_environment( self._environment_values.environment ) 
	self._environments:set_value( self._environment_values.environment )
	self._sky_rotation:set_value( self._environment_values.sky_rot )
	self._color_gradings:set_value( self._environment_values.color_grading )
