...
function WorldCameraLayer:update( t, dt )
	if not managers.worldcamera._current_world_camera and not self._look_through_camera then
	...
	self._sizer = EWS:BoxSizer("VERTICAL")
		...
		local edit_sizer = EWS:StaticBoxSizer( self._ews_panel, "VERTICAL", "Settings" )
			...
			local in_out_sizer = EWS:BoxSizer("HORIZONTAL")
				...
				local in_sizer = EWS:StaticBoxSizer( self._ews_panel, "VERTICAL", "In" )
					...
					for _,name in ipairs( { "ease", "linear", "fast" } ) do
			...
			self._duration_params = {
				...
				ctrlr_proportions 	= 1,
				events				= {
										{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "set_duration" ) },
										{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "set_duration" ) }
									}
			}
			...
			self._delay_params = {
				...
				ctrlr_proportions 	= 1,
				events				= {
										{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "set_delay" ) },
										{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "set_delay" ) }
									}
			}
			...
			self._dof_paddding_params = {
				...
				ctrlr_proportions 	= 1,
				events				= {
										{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "set_dof_padding" ) },
										{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "set_dof_padding" ) }
									}
			}
			...
			self._dof_clamp_params = {
				...
				ctrlr_proportions 	= 1,
				events				= {
										{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "set_dof_clamp" ) },
										{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "set_dof_clamp" ) }
									}
			}
		...
		local keys_sizer = EWS:StaticBoxSizer( self._ews_panel, "VERTICAL", "Keys" )
			...
			local dof_text_sizer = EWS:BoxSizer( "HORIZONTAL" )
				...
				dof_text_sizer:add( self._current_far_dof_text, 0, 40, "ALIGN_CENTER_VERTICAL,RIGHT" ) -- ALIGN_CENTER_HORIZONTAL
			...
			time:connect( "EVT_SCROLL_CHANGED", callback( self, self, "set_time" ), { slider = time, text = self._time_text } )
			time:connect( "EVT_SCROLL_THUMBTRACK", callback( self, self, "set_time" ), { slider = time, text = self._time_text } )
			...
			local roll_params = {
				...
				ctrlr_proportions 	= 3,
				events				= {
										{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "on_set_roll" ) },
										{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "on_set_roll" ) }
									}
			}
...
function WorldCameraLayer:test_done()
	...
	if self._look_through_camera then
		-- managers.worldcamera:start_dof()
	end
end
...
function WorldCameraLayer:look_through_camera( data, event )
	...
	if not self._look_through_camera then
		managers.editor:set_camera_fov( managers.editor:default_camera_fov() )
		managers.editor:set_camera_roll( 0 )
		-- TODO Needs fixing with now vp and env system.
...
-- Add a new key on the current time
...
function WorldCameraLayer:on_set_roll()
	...
	if camera then
		local key = camera:key( tonumber( self._keys:get_value() ) )
		local roll = self._key_types.roll.value -- self._key_types.roll:get_value() 
		--if roll == "" then
		--	roll = 0
		--end
		key.roll = roll
...
function WorldCameraLayer:set_key( index )
	...
	if camera then
		...
		self:set_time( { slider = self._time, text = self._time_text } )
		self:set_key_values( time, fov, near_dof, far_dof, roll )
...
function WorldCameraLayer:on_stop_sequence()
	managers.worldcamera:stop_world_camera() -- Should be something else
end
...
function WorldCameraLayer:select_sequence()
	...
	--[[
		self._current_world_camera = managers.worldcamera:world_camera( name )
		self._duration:set_value( self._current_world_camera:duration() )
		self._delay:set_value( self._current_world_camera:delay() )
		self._point_list:clear()
		for i,pos in ipairs( self._current_world_camera:get_points() ) do
			self._point_list:append( i )
		end
