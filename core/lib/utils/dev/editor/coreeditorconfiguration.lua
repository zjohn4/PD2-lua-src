function CoreEditor:build_configuration()
	...
	local page_edit = EWS:Panel( notebook, "_edit", "" )
		...
		page_edit:set_sizer( edit_sizer )
			...
			--[[local edit_light_dialog = EWS:CheckBox( page_edit, "Use edit light dialog", "" )
			edit_light_dialog:set_value( self._use_edit_light_dialog )
			self._config[ "_use_edit_light_dialog" ] = { ctrlr = edit_light_dialog, callback = callback( self, self, "changed_use_edit_light_dialog" ) }
			edit_sizer:add( edit_light_dialog, 0, 0, "EXPAND")]]
...
--[[function CoreEditor:changed_use_edit_light_dialog( changed, value )
	if changed then
		EWS:message_box( self._configuration, "Change of 'Use edit light dialog' requires editor restart", "Restart required", "", self._configuration:get_position() )
	end
end]]
