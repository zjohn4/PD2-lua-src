...
--[[
function CoreEditorGroups:deleted_unit( unit )
	for _,group in pairs( self._groups ) do
...
function CoreEditorGroups:load_group_file( path )
	...
	local layer_name = "Statics" -- Allows convertion from old GroupHandler
	...
	if pos then 
		for unit in node:children() do
			...
			-- Assign any edited settings to the unit
...
-- Parse and set saved light settings
...
-- Parse and set material variation
...
function CoreEditorGroup:remove()
	for _,unit in ipairs( self._units ) do
		table.delete( unit:unit_data().editor_groups, self )
		-- unit:unit_data().editor_group = nil
	end
end
...
function CoreEditorGroup:save_to_file()
	-- local savedlg = EWS:FileDialog( Global.frame, "Save As..", "", self:name(), "*.xml", "SAVE" )
	-- if savedlg:show_modal() then
		-- local path = string.sub( savedlg:get_path(),string.len( Application:base_path() ), -1 )
		
		-- if not string.find( string.lower( savedlg:get_directory() ), string.lower( Application:base_path() ), 1, true ) then
			-- managers.editor:output( 'Cant save file outside the project' )
			-- self:save_to_file()
			-- return
		-- end
...
function CoreEditorGroup:save_lights( file, t, unit, data_table )
	...
	--	table.insert( data_table[ "lights" ], { name = light:name(), enable = light:enable(), far_range = light:far_range(), color = light:color(), angle_start = as, angle_end = ae, multiplier = multiplier } )
	end
end
...
function CoreEditorGroup:draw( t, dt )
	...
	Application:draw( self._reference, 0.0, 1 * i, 0.0 )
end
