...
------ Layers Menu
	...
	for i = 0, page_count - 1 do
		...
		Global.frame:connect( "DISABLE_"..text, "EVT_COMMAND_MENU_SELECTED", callback( self, self, "on_disable_layer" ), { name = text, id = "DISABLE_"..text } )
		
		self._hide_layer_menu:append_item( "HIDE_"..text, text, "Hide layer "..text )
		Global.frame:connect( "HIDE_"..text, "EVT_COMMAND_MENU_SELECTED", callback( self, self, "on_hide_layer" ), { name = text, id = "HIDE_"..text } )
		
		self._unhide_layer_menu:append_item( "UNHIDE_"..text, text, "Unhide layer "..text )
		Global.frame:connect( "UNHIDE_"..text, "EVT_COMMAND_MENU_SELECTED", callback( self, self, "on_unhide_layer" ), { name = text, id = "UNHIDE_"..text } )
...
------ Edit Menu
	...
	Global.frame:connect( "TB_SURFACE_MOVE", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "menu_toolbar_toggle" ), { value = "_use_surface_move", menu = "_edit_menu", toolbar = "_toolbar" } )
	...
	Global.frame:connect( "TB_SNAPPOINTS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "menu_toolbar_toggle" ), { value = "_use_snappoints", menu = "_edit_menu", toolbar = "_toolbar" } )
...
------ Group Menu
	...
	Global.frame:connect( "DEBUG_DRAW_GROUPS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "menu_toolbar_toggle" ), { value = "_debug_draw_groups", menu = "_group_menu" } )
--	Global.frame:connect( "DEBUG_DRAW_GROUPS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_info" ), { info_menu, "INFO OUTPUT" } )
...
------ Mission Menu
	...
	Global.frame:connect( "MISSION_DEBUG_DRAW_AREAS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "menu_toolbar_toggle" ), { value = "_simulation_debug_areas", menu = "_mission_menu" } )
...
------ View Menu
	local view_menu = EWS:Menu( "" )
		...
		Global.frame:connect( "INFO OUTPUT", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_info" ), { info_menu, "INFO OUTPUT" } )
	...
	-- Global.frame:connect( "REFLECTION", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "change_visualization" ), "reflection_visualization" )
	...
	Global.frame:connect( "LOCK_1280_720", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_lock_1280_720" ), { view_menu, "LOCK_1280_720" } )
	Global.frame:connect( "ORTHOGRAPHIC", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_orthographic" ), { view_menu, "ORTHOGRAPHIC" } )
	Global.frame:connect( "USE LIGHT", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_light" ), { view_menu, "USE LIGHT" } )
	Global.frame:connect( "SHOW CENTER", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_show_center" ), { view_menu, "SHOW CENTER" } )
	Global.frame:connect( "SHOW CAMERA INFO", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_show_camera_info" ), { view_menu, "SHOW CAMERA INFO" } )
	Global.frame:connect( "SHOW_MARKER_BALL", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_draw_marker_ball" ), { view_menu, "SHOW_MARKER_BALL" } )
...
------ Hide/Unhide Menu
	...
	Global.frame:connect( "RENDER_EFFECTS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_render_effects" ), { hide_menu, "RENDER_EFFECTS" } )
...
------ Advanced Menu
	...
	Global.frame:connect( "EXPERT MODE", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "on_expert_mode" ), { advanced_menu, "EXPERT MODE" } )
...
------ Debug Menu
	...
	Global.frame:connect( "TB_DRAW_OCCLUDERS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "menu_toolbar_toggle" ), { value = "_draw_occluders", menu = "_debug_menu", toolbar = "_left_upper_toolbar" } )
--	Global.frame:connect( "TB_DRAW_OCCLUDERS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_draw_occluders" ), { self._debug_menu, "DRAW OCCLUDERS" } )
...
------ Window Menu
	...
	Global.frame:connect( "SHOW MARKERS", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_show_markers" ), { window_menu, "SHOW MARKERS" } )
...
-- Displays a message box. Returns true if result was Cancel.
...
-- Is also called from toolbar
function CoreEditor:on_new()
	
	if self:confirm_on_new() then return end
...
function CoreEditor:on_close( custom_data, event_object )
	...
	-- Save current edit setting values
	self:save_edit_setting_values()
	-- Save frame layout before quiting
	self:save_layout()
...
-- Disabled all layers but the curernt one
function CoreEditor:on_disable_layers()
	for name,layer in pairs( self:layers() ) do
		if layer ~= self._current_layer then
			...
			self:on_disable_layer( { name = name, id = "DISABLE_"..name } )
		end
	end
end

-- Called from menu event and from CoreEditor:on_disable_layers()
...
-- Reloads the currently selected unit, also called from toolbar
...
-- Go through all units and checks if any shares position and rotation
			...
			for _,data in ipairs( unit_name ) do
				if data.pos == pos then
					if data.rot == rot then
						table.insert( collisions.complete, { u1 = data.unit, u2 = unit, pos = pos } )
					else
						table.insert( collisions.only_positions, { u1 = data.unit, u2 = unit, pos = pos } )
					end
				end
			end
			table.insert( unit_name, { unit = unit, pos = pos, rot = rot } )
		else
			units[ unit:name():s() ] = { { unit = unit, pos = pos, rot = rot } }
		end
	end
	local pos
...
function CoreEditor:on_make_screenshot()
	...
	-- local path = self:create_temp_saves( "screenshots" ).."\\"..name
	-- Application:screenshot( path )
	-- self:output( "Screenshot created in "..path.."." )

	-- Screenshots are now always saved to the user folder
...
-- Hides / unhides all helper units
function CoreEditor:on_hide_helper_units( vis )
	for name,layer in pairs( self._layers ) do
		for _,unit in ipairs( layer:created_units() ) do
			if unit:unit_data().only_visible_in_editor 
			or unit:unit_data().only_exists_in_editor
			or unit:has_material_assigned( Idstring( "leveltools" ) ) then
