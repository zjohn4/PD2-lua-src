...
function HubTimeline:set_hub_unit( unit )
	self._hub_unit = unit
	-- for id,action in pairs( self._hub_unit:hub_element_data().actions_data ) do
...
function HubTimeline:add_action( unit )
	...
	-- self:_select_clip( clip ) -- don't have to do this. The hub calls :select_action() from its select_action
end

-- Called from hub when it selects an action
...
function HubTimeline:_on_mouse_left_down(sender, event)
	...
	-- self:_select_clip( key ) -- Don't need to do select clip here, the hub will call it through :select_action()
	if key then
