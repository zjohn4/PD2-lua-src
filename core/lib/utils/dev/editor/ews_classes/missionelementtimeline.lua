...
function MissionElementTimeline:init( caption )
	...
	self:create_panel( "VERTICAL" )	
		...
		self._panel_sizer:add( self._scrolled_area, 1, 0, "EXPAND" )
			...
			-- self._ruler:set_major_divisions( 100, self._multiplier )
			self._ruler:set_major_divisions( 100, 1 )
			self._sequence_track:set_units( 100, self._multiplier )
			-- self._sequence_track:set_units_from_ruler( self._ruler )
			
			-- self._sequence_track:set_icon_bitmap( CoreEWS.image_path("sequencer\\track_icon_key.bmp"), 11 )
...
-- Called from element when a on executed element is added
function MissionElementTimeline:add_element( unit, params )
	self:_add_unit( unit, params )
end

-- Called from element when it selects an on executed element
...
-- Updates information base on a clip
...
-- Called from element when a on executed element is removed
...
-- Creates a new key on the timeline and assigns meta data to it
...
-- Return a color, either specified by the unit extension or a random one
...
function MissionElementTimeline:_on_mouse_left_down(sender, event)
	...
	-- Don't need to do select clip here since set_on_executed_element will trigger a call back to select_element
	if key then
...
function MissionElementTimeline:_on_mousewheel(track, event)
	...
	-- self._sequence_track:set_max_size( self._sequence_track:get_min_size() )
