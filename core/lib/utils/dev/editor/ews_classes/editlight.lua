...
function EditUnitLight:init( editor )
	...
	self._near_range_params = {
		...
		ctrlr_proportions 	= 1,
		events				= {
								{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "update_near_range" ) },
								{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "update_near_range" ) }
							}
	}
	...
	self._range_params = {
		...
		ctrlr_proportions 	= 1,
		events				= {
								{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "update_far_range" ) },
								{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "update_far_range" ) }
							}
	}
	...
	self._upper_clipping_params = {
		...
		ctrlr_proportions 	= 1,
		events				= {
								{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "update_clipping", "x" ) },
								{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "update_clipping", "x" ) }
							}
	}
	...
	self._lower_clipping_params = {
		...
		ctrlr_proportions 	= 1,
		events				= {
								{ event = "EVT_COMMAND_TEXT_ENTER", callback = callback( self, self, "update_clipping", "y" ) },
								{ event = "EVT_KILL_FOCUS", callback = callback( self, self, "update_clipping", "y" ) }
							}
	}
...
function EditUnitLight:_draw_light( light, t, dt )
	...
	if self:_is_type( "omni" ) then
		self._brush:set_color( Color (0.15, c.x*1.0, c.y*1.0, c.z*1.0) )
		...
		Application:draw_sphere( light:position(), light:far_range(), c.x*1.0, c.y*1.0, c.z*1.0 )
	else
		...
		self._brush:set_color( Color (0.25, c.x*1.0, c.y*1.0, c.z*1.0) )
		...
		Application:draw_cone( light:position(), light:position() - light:rotation():z() * light:far_range(), far_radius, c.x*1.0, c.y*1.0, c.z*1.0 )
	...
	-- Clipping
	self._brush:set_color( Color (0.5, c.x*1.0, c.y*0.5, c.z*0.0) )
	...
	self._brush:set_color( Color (0.5, c.x*1.0, c.y*0.2, c.z*0.0) )
