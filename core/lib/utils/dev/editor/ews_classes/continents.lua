-- ContinentsDialog = ContinentsDialog or class( CoreEditorEwsDialog )
ContinentPanel = ContinentPanel or class()

function ContinentPanel:init( parent )
	-- self:create_panel( parent, "HORIZONTAL" )
	...
	-- self._dialog:set_background_colour( 255, 255, 255 )
...
--[[
function ContinentsDialog:init( toolbar, btn )
	CoreEditorEwsDialog.init( self, nil, "Continents", "", Vector3( 300, 150, 0), Vector3( 250, 180, 0), "DEFAULT_DIALOG_STYLE,RESIZE_BORDER,STAY_ON_TOP" )
...
function ContinentPanel:create_panel( parent, orientation )
	...
	-- self._panel_sizer = EWS:StaticBoxSizer( self._panel, orientation, "Continents" )
...
function ContinentPanel:add_continent( params )
	...
	panel:set_background_colour( 255, 255, 255 )
		...
		-- params.toolbar:set_background_colour( 255, 255, 255 )
	...
	--	params.toolbar:add_check_tool( "VISIBLE", "Toggle visible", CoreEWS.image_path( "world_editor\\continent\\visible_16x16.png" ), "Toggle visible" )
	--	params.toolbar:set_tool_state( "VISIBLE", params.visible )
	--	params.toolbar:connect( "VISIBLE", "EVT_COMMAND_MENU_SELECTED", callback( self, self, "toggle_visible" ), params )
...
--	params.continent:set_visible( params.toolbar:tool_state( "VISIBLE" ) )
end
...
-- Updates the toolbar values for a continent
function ContinentPanel:update_continent_panel( continent )
	...
	--		params.toolbar:set_tool_state( "VISIBLE", continent:value( "visible" ) )
...
-- Calls editor which will delete current continent
...
function ContinentPanel:create_world_setting_dialog()
	...
	if path and dir then
		CreateWorldSettingFile:new( { name = managers.database:entry_name( path ) , dir = dir } )
	end
end
