-- A dialog to manage workviews in the editor
...
-- Called from editor when a workview has been added
function SelectWorkView:workview_added()
	self:fill_views_list()
end

-- Adds workviews to the list according to continent
			...
			if c_values and c_values.workviews then
				for v_name,view in pairs( c_values.workviews ) do
					if string.find( v_name, filter, 1 ,true ) then
						...
						self._views[ j ] = { view = view, continent = c_name, name = v_name  }
