...
function WireLayer:set_select_unit( unit )
	...
	self._selected_point = nil
	if self._selected_unit then -- Only set the selected point from the selected_unit (reference)
		self._selected_point = self._selected_unit:get_object( self._target_name )
	end
end
...
function WireLayer:update( t, dt )
	...
	if ray then
		...
		local u_rot = --[[ alive( self._selected_unit ) and self._selected_unit:rotation() or]] Rotation()
		local z = n
	...
	if self._selected_unit then
		...
		if self._creating_wire or self._grab then
			-- local dot = self._current_rot:z():dot( self._selected_unit:rotation():z() )
...
--			self._selected_unit:set_rotation( Rotation( s_pos, self._current_pos ) )
			
--			self._selected_unit:set_rotation( Rotation( (s_pos - self._current_pos):normalized(), Vector3( 0, 0, 1 ) ) )
			self:set_midpoint()
		end
	end
...
function WireLayer:change_slack( wire_slack )
	if self._selected_unit then
		-- self._selected_unit:wire_data().slack = wire_slack:get_value()
