...
function MissionLayer:toolbar_toggle_trg( data )
	if data.value == "_editing_mission_element" then -- Prevent the effect of the callback trigger to occur
		if not alive( self._selected_unit ) or not self._selected_unit:mission_element():can_edit() then
			return
		end
	end
...
function MissionLayer:build_panel( notebook )
	MissionLayer.super.build_panel( self, notebook, { units_noteboook_proportion = 0, units_notebook_min_size = Vector3( -1, 240, 0 ) } )
