...
function PortalLayer:_old_load( portal )
	...
	if portal._portal_shapes then
		for name,portal in pairs( portal._portal_shapes ) do
			...
			self._portal_shapes[ name ] = { portal = {}, top = portal.top, bottom = portal.bottom, draw_base = draw_base, r = r, g = g, b = b }
			self._current_shape = self._portal_shapes[ name ]
			self._current_portal = self._portal_shapes[ name ].portal
			for _,data in ipairs( portal.portal ) do
				self:do_spawn_unit( self._portal_point_unit, data.pos )
				-- self:create_portal_point( data.pos )
			end
		end
	end
...
function PortalLayer:save( save_params )
	...
	for name,data in pairs( self._portal_shapes ) do
		local portal_data = { name = name, top = data.top, draw_base = data.draw_base, bottom = data.bottom }
		portal_data.points = {}
		for _,unit in ipairs( data.portal ) do
			table.insert( portal_data.points, { position = unit:position(), rotation = unit:rotation() } )
		end
...
function PortalLayer:update( time, rel_time )
	...
	if not self._dont_draw then
		...
		if self._current_group then
			self._current_group:draw( time, rel_time, 1.0, self._dont_draw_boxes )
		end
	end
	
	if self._draw_units_in_no_portal_state then
		self:_draw_units_in_no_portal( time, rel_time, 1.0 )
	end
	
	if self._draw_not_current then
		self:_draw_units_in_not_current_portal( time, rel_time, 1.0 )
	end
...
function PortalLayer:draw_portal( data )
		if self._only_draw_selected then
			if data.portal ~= self._current_portal then
				return
			end
		end
		...
		for i = 1, #portal do
			...
			--[[
			local sign = (math.sign(max-min))
			if sign == 0 then sign = 1 end
			for z_s = min, max, sign*1000 do
				Application:draw_line( Vector3( s_pos.x, s_pos.y, z_s ), Vector3( e_pos.x, e_pos.y, z_s ), r, g, b )
			end
			]]
			...
			dir = dir:normalized()
			--[[
			for step = 0, length, 1000 do
				local vs_pos  = Vector3( s_pos.x, s_pos.y, min ) + dir*step
				local ve_pos  = Vector3( s_pos.x, s_pos.y, max ) + dir*step
				Application:draw_line( vs_pos, ve_pos, r, g, b )
			end
			]]
			...
			-- self._portal_brush:line( Vector3( s_pos.x, s_pos.y, min+(max-min)/2 ), Vector3( e_pos.x, e_pos.y, min+(max-min)/2), max-min, dir:cross(Vector3(0,0,1)) )
...
function PortalLayer:_draw_units_in_no_portal()
	...
	for _,unit in pairs( managers.editor:layer( "Statics" ):created_units() ) do
		if unit:visible() and not (unit:unit_data().only_visible_in_editor or unit:unit_data().only_exists_in_editor) then
			if not managers.portal:unit_in_any_unit_group( unit ) then
...
function PortalLayer:_draw_units_in_not_current_portal()
	...
	for _,unit in pairs( managers.editor:layer( "Statics" ):created_units() ) do
		if unit:visible() and not (unit:unit_data().only_visible_in_editor or unit:unit_data().only_exists_in_editor) then
			if not self._current_group:unit_in_group( unit ) then
				self._portal_brush:unit( unit )
			end
		end
	end
...
function PortalLayer:_auto_fill()
	...
	for _,unit in pairs( managers.editor:layer( "Statics" ):created_units() ) do
		if unit:visible() and not (unit:unit_data().only_visible_in_editor or unit:unit_data().only_exists_in_editor) then
			if not self._current_group:unit_in_group( unit ) then
				if self._current_group:inside( unit:position() ) then
...
function PortalLayer:build_panel( notebook )
	...
	local only_draw_selected = EWS:CheckBox( self._ews_panel, "Only draw current", "" )
		...
		only_draw_selected:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "cb_toogle" ), { cb = only_draw_selected, value = "_only_draw_selected" } )
	...
	local dont_draw_boxes = EWS:CheckBox( self._ews_panel, "Don't draw portal shapes", "" )
		...
		dont_draw_boxes:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "cb_toogle" ), { cb = dont_draw_boxes, value = "_dont_draw_boxes" } )
	...
	local draw_nonportaled = EWS:CheckBox( self._ews_panel, "Draw non-portaled units", "" )
		...
		draw_nonportaled:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "cb_toogle" ), { cb = draw_nonportaled, value = "_draw_units_in_no_portal_state" } )
	...
	local draw_not_current = EWS:CheckBox( self._ews_panel, "Draw units not in current portal", "" )
		...
		draw_not_current:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "cb_toogle" ), { cb = draw_not_current, value = "_draw_not_current" } )
...
function PortalLayer:click_select_unit()
	-- Add unit to portal unit group
	if self._ctrl:down( Idstring( "add_to_portal_unit_group" ) ) then
		if self._current_group then
			local ray = managers.editor:unit_by_raycast( { mask = 1, ray_type = "body editor", sample = true } )
...
function PortalLayer:set_select_unit( unit )
	...
	if alive( unit ) then
		if unit:unit_data().portal_group_shape then
...
--	local unit = self:do_spawn_unit( "portal_point", pos )
...
function PortalLayer:new_portal( portals )
	...
	local g = 0.25 + math.rand( 0.75 )
	local b = 0.25 + math.rand( 0.75 )
	self._portal_shapes[ name ] = { portal = {}, top = 0, bottom = 0, draw_base = 0, r = r, g = g, b = b }
	self:update_shapes_listbox( portals )
...
function PortalLayer:add_unit_list_btn()
	...
	local f = function( unit ) return unit:slot() == 1 end
...
function PortalLayer:remove_unit_list_btn()
	...
	local f = function( unit ) return group:ids()[ unit:unit_data().unit_id ] end
	local dialog = rawget( _G, "SelectUnitByNameModal" ):new( "Add Trigger Unit", f )
	for _,unit in ipairs( dialog:selected_units() ) do
		-- group:remove_unit_id( unit )
		group:remove_unit_id( unit )
...
function PortalLayer:delete_unit( unit )
	if unit:name() == Idstring( self._portal_point_unit ) then
		 for name,shape in pairs( self._portal_shapes ) do
		 	table.delete( shape.portal, unit ) -- Don't care if the unit is in there or not, simply remove if it is.
		 end
	end
...
-- Calculates the mid point between selected point in portal and the next
...
-- Inserts a portal point unit after selected unit in a portal
