...
--	self:viewport():vp():set_post_processor_effect( "World", Idstring("deferred"), Idstring("projection_generation") )
	...
	local params = { res = res, shape = shape, saved_environment = saved_environment }
...
function CoreEditor:_create_dome_occlusion( params )
	...
	-- self:set_camera_near_range( far_range - h )
	-- self:set_camera_near_range( 5 )
	...
	if dome_occ_feed then
		-- dome_occ_feed:set_variable(Idstring("dome_occ_settings"), Vector3( 0, 1200, 0 )) -- Not used at all right now
...
function CoreEditor:_convert_dome_occlusion()
	...
	-- self._camera_controller:_add_meta_data( output_path, "colormap_gradient_alpha_no_mips" )
end
