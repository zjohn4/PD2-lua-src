...
--[[
	Register it in the EWS class.
	EWS:ExtendedMultiChoiceDialog(parent, [caption], [message], [pos], [size], [style], [objects])
]]

if Application:ews_enabled() then
	local ews = getmetatable( rawget(_G, "EWS") )
	assert(ews)
	rawset(ews, "ExtendedMultiChoiceDialog", function(ews, ...) return core_or_local("CoreExtendedMultiChoiceDialog", ...) end)
end

function CoreExtendedMultiChoiceDialog:init(parent, caption, message, pos, size, style, objects)
	self._objects = objects or {}
	self._dialog = EWS:Dialog(parent, caption or "", "", pos or Vector3(-1,-1,0), size or Vector3(450.0, 500.0, 0.0), style or "CAPTION,SYSTEM_MENU,STAY_ON_TOP")
