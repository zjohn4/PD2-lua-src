...
function CoreCutsceneSequencerPanel:tracks()
	if self._tracks == nil then
		self._tracks = {}
		--table.insert(self._tracks, self:audio_track()) -- HACK: Disabled audio track for now, we'll show it once we support audio clips.
...
function CoreCutsceneSequencerPanel:track_at_screen_position(screen_position)
	return table.find_value(self:tracks(), function(track) return track:point_is_inside(screen_position) end)
end
...
function CoreCutsceneSequencerPanel:set_active_film_track(active_track)
	...
	self:_send_event("EVT_EVALUATE_FRAME_AT_PLAYHEAD", { position = function() return position end })
end

function CoreCutsceneSequencerPanel:film_clips()
	return table.inject(self:film_tracks(), {}, function(accumulated, track) return table.list_union(accumulated, track:clips()) end)
end
...
function CoreCutsceneSequencerPanel:selected_film_clips()
	return table.inject(self:film_tracks(), {}, function(accumulated, track) return table.list_union(accumulated, track:selected_clips()) end)
end
		...
		if self:audio_track():remove_clip(clip) or self:key_track():remove_clip(clip) then
			...
			for _, track in ipairs(self:film_tracks()) do
				if track:remove_clip(clip) then
					removed_count = removed_count + 1
					break -- No need to search further - clips are cloned when added to tracks
				end
			end
		end
	end
...
function CoreCutsceneSequencerPanel:_evaluate_frame_at_playhead()
	self:_send_event("EVT_EVALUATE_FRAME_AT_PLAYHEAD", { position = function() return self:playhead_position() end })
end
...
function CoreCutsceneSequencerPanel:_create_panel(parent_frame)
	...
	-- HACK: Disabled audio track for now, we'll show it once we support audio clips.
	self._audio_track = responder_map{ remove_clip=false, default={} }
	-- self._audio_track = EWS:SequencerTrack(self._scrolled_area)
	-- self._audio_track:set_units(100, 500)
	-- self._audio_track:set_icon_bitmap(CoreEWS.image_path("sequencer\\track_icon_audio.bmp"), 11)
	-- self._audio_track:set_background_colour(self:_track_background_colour(true):unpack())
	-- self:_connect_mouse_events(self._audio_track)
	
	-- scrolled_area_sizer:add(self._audio_track, 0, 0, "EXPAND")
	-- self:_create_ornaments_for_track(self._audio_track)
...
function CoreCutsceneSequencerPanel:_on_mouse_left_down(sender, event)
	...
	if sender ~= self._ruler and sender ~= self:audio_track() and sender ~= self:key_track() then
		-- A film track was clicked. Set it as the active track.
...
function CoreCutsceneSequencerPanel:_on_scrolled_area_paint(data, event)
	-- This ensures the ruler is aligned with the scrolled track area.
	local x_offset = self._scrolled_area:calc_scrolled_position(Vector3(0, 0, 0)).x
	self._ruler:set_offset_pixels(x_offset)
	event:skip()
end

-- TrackBehaviour delegate callback. Should normally not be called directly.
