...
function ScriptGraph:add_node( node )
	assert(node:type() == "FlowNode", "[ScriptGraph] The only supported node type is 'flow_node'.") --Just for now...
...
function ScriptGraph:_load_node_info( node )
	...
	for node_info in node:children() do
			...
			if assert(node_info:parameter("type")) == "in" then
				...
				for inf in node_info:children() do
					if inf:name() == "color" then
						color = { tonumber(assert(inf:parameter("r"))), tonumber(assert(inf:parameter("g"))), tonumber(assert(inf:parameter("b"))) }
					end
				end
				...
				for inf in node_info:children() do
					if inf:name() == "connection" then
						table.insert(connection, {id = assert(inf:parameter("id")), slot = assert(inf:parameter("slot")), desc = assert(inf:parameter("desc"))})
		...
		elseif node_info:name() == "color" then
			info.color = { tonumber(assert(node_info:parameter("r"))), tonumber(assert(node_info:parameter("g"))), tonumber(assert(node_info:parameter("b"))) }
		end
	end
	return info
end
...
function ScriptGraph:_create_panel(parent, id, style, window_style)
	...
	--self._graph_view = EWS:GraphView(parent, id or "", self._graph, type)
	self._graph_view = EWS:GraphView(parent, id or "", self._graph, "FLOW") -- only supports FLOW type for now
...
function ScriptGraph:_write_icon(cfg_node, node)
	if node:icon() ~= "" then
		...
		-- This is not the correct way of doing it! Fix it! /AJ
...
function ScriptGraph:_write_connections(slot_node, slot, node, id_map)
	...
	for _,inf in pairs(con_info) do
		...
		for _,dest_slot in ipairs(dest_slots) do
			...
			con_node:set_parameter("desc", "") --Todo
...
function ScriptGraph:_write_input_color(slot_node, slot, node)
	--local r, g, b = node:input_colour(slot)
	--self:_write_output_color_to_node(slot_node, Node("color"), r, g, b)
end
