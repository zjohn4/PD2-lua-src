...
function CoreManagedTreeControl:_change_state(data, event)
	...
	if id > -1 then
		if hit == "ONITEMICON" then
			...
			-- Just toggle between two states.
			if state == 0 then
				state = 1
			elseif state == 1 then 
				state = 0
				...
				return -- Don't do anything if we have a unsupported state.
			end
...
function CoreManagedTreeControl:connect(event_type, script_callback, script_data)
	if event_type == self.CHECKBOX_UPDATED_EVENT_NAME then
		...
		if string.begins(event_type, "EVT_COMMAND_TREE_") then
			-- We wrap EWSTreeEvents to return a node object instead of an item id when calling event:get_item()
			-- When connecting event callbacks, you get at the node like this:
			--     function my_callback(data, event)
			--         local node = event:get_item()
			--     end
			...
			local function tree_event_wrapper(data, event, ...)
				...
				wrapped_event.get_item = function() return item_id ~= -1 and CoreEWSTreeCtrlTreeNode:new(self._tree_ctrl, item_id, self._checkbox_style) or nil end
				wrapped_event.get_old_item = function() return old_item_id ~= -1 and CoreEWSTreeCtrlTreeNode:new(self._tree_ctrl, old_item_id, self._checkbox_style) or nil end
