...
function CoreEWSTreeCtrlTreeNode:children()
	return table.collect(self._tree_ctrl:get_children(self._item_id), function(child_id) return CoreEWSTreeCtrlTreeNode:new(self._tree_ctrl, child_id, self._checkbox_style) end)
end
...
-- CoreBaseTreeNode overloads

function CoreEWSTreeCtrlTreeNode:has_children()
	-- This avoids creating some garbage if we only need to know whether this is a leaf node or not. 
