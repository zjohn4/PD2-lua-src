...
function PointPickerPanel:update(time, delta_time)
	if self:_text_ctrl_has_focus() then	
		self.__current_position = Vector3( self.__x_textctrl:get_value(), 
										   self.__y_textctrl:get_value(), 
										   self.__z_textctrl:get_value() 
										  )
...
function PointPickerPanel:_text_ctrl_has_focus()
	return ( EWS:get_window_in_focus() == self.__x_textctrl ) 
		or ( EWS:get_window_in_focus() == self.__y_textctrl ) 
		or ( EWS:get_window_in_focus() == self.__z_textctrl )
end
