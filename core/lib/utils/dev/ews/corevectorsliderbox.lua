...
function VectorSliderBox:_create_slider()
	local new_slider = EWS:Slider(self._parent_frame, 0.0, 0.0, self._slider_max)
...
function VectorSliderBox:set_value(vector)
	...
	if self._vector3 then 
		...
		self:_send_event("EVT_VALUE_CHANGED", vector:with_z(0.0))
	end
end
...
function Vector2SliderBox:init(parent_frame, title, min, max, step)
	...
	self.__slider_z = EWS:Slider(self._parent_frame, 0.0, 0.0, 0)
