...
function LoadLevelSuite:init()
	...
	self:add_step(
		...
		CoreSmoketestCommonSuite.CallAndWaitEventSubstep.step_arguments(callback(self, self, "load_level"), Idstring("game_state_ingame"))
		)
		
	self:add_step(
		"in_game",
		CoreSmoketestCommonSuite.DelaySubstep,
		CoreSmoketestCommonSuite.DelaySubstep.step_arguments(1.0)
		)
