...
local function dump_collect(tab, t, level, max_level)
	if level < max_level then
		for k, v in pairs(tab) do
			...
			if v == tab then
				t[k_str] = {_type = "recursive", _value = "(" .. tostring({}) .. ") UNKNOWN"}
			elseif v_type == "function" then
		        local info = debug.getinfo(v, "S")
		        if info.what == "Lua" then
		           	t[k_str] = {_type = v_type, _value = info.source .. ":" .. info.linedefined}
		        end
		    elseif v_type == "table" then
		    	t[k_str] = {_type = v_type, _value = dump_collect(v, {}, level + 1, max_level)}
		    elseif v_type == "userdata" then
			    ...
			    if meta and meta["tostring"] then
					...
					t[k_str] = {_type = v_type, _value = "(" .. tostring({}) .. ") UNKNOWN"}
		    	end
		    else
		    	-- Assume tostring will work.
		    	t[k_str] = {_type = v_type, _value = tostring(v)}
			end
		end
	else
		t["(" .. tostring({}) .. ") UNKNOWN"] = {_type = "unexplored", _value = "(" .. tostring({}) .. ") UNKNOWN"}
	end
...
function core_lua_dump(file_name, root, max_level, no_bin)
	...
	if no_bin then
		string_to_binary = function(str) return str end 
	end
