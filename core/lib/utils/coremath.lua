...
Module CoreMath adds several function to the standard
Lua math library. It also defines some helper functions. 
...
function rgb_to_hsv(r, g, b)
	...
	if max == min then
		hue = 0 -- Gray, unable to determine hue.
...
function width_mul( aspect_ratio )
	return (1.0 / (4.0 / 3.0)) * aspect_ratio
end
...
-- Takes a vector_table with percentages that equals 100%
-- and returns the index of the element that was randomly choosen.
-- Ex. { 30, 65, 5 } ( = 100% )
...
-- Dates retrieved from os.date( "*t" ). Returns nil if they are equal. Use os.time() if you don't need better precision than 143 seconds.
function os.get_oldest_date( date1, date2 )
	...
	elseif( date1.yday < date2.yday ) then
		return date2
	elseif( date1.hour > date2.hour ) then	-- No need to check "isdst" variable because of "yday" check.
		return date1
...
-- Random as it worked in dieselscript, and added a second 'a' for randomize between 'a' and 'b'
...
-- Takes params l1, l2 which defines a line and p which is a point somewhere else.
-- Return a Vector3 which is the closets point on the line to the point p.
...
-- Takes params l1, l2 which defines a line and p which is a point somewhere else.
-- Returns the closest distance between the point p and the line l1,l2 (also returns the closest point on the line)
...
function math.world_to_obj(obj, point)
	...
	-- local rot = Rotation:rotation_difference(Rotation(), obj:rotation() )
...
function math.obj_to_world(obj, point)
	...
	--local rot = Rotation:rotation_difference(obj:rotation(), Rotation())
