...
function GameStateMachine:change_state( state, params )
	...
	if not self:can_change_state( state ) then
		...
		-- assert( self:can_change_state( state ), "[GameStateMachine] Requesting invalid transition " .. transition_debug_string )
	else
...
-- The actual state change is performed at the end of the frame
function GameStateMachine:_do_state_change()
	...
	for i_transition, transition in ipairs( self._queued_transitions ) do
		...
		self._current_state = new_state      -- a new state_change() within a transition, think
		trans_func( old_state, new_state, params )   -- before changing...
	end
