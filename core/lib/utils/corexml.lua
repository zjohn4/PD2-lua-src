...
-- Creates a xml formated string from a value. Takes class or table c, value name, tab t, and unit unit
-- t and unit are optional
...
-- Called from save_value_string when a table value is found
...
function parse_value_node( node )
	...
	if type == "table" then
		...
		for table_node in node:children() do
			...
			name = tonumber( name ) or name	-- If name is a number it should generate a indexed table
