...
CoreCutsceneOptimizer.ANIMATION_BLOB_PART_DURATION = 5 -- Split animation data into N-second chunks to keep contiguous memory requirements in check.

function CoreCutsceneOptimizer:init()
	self.super.init(self)
	self.__compression_enabled = { win32 = true, x360 = true, ps3 = true }
end

function CoreCutsceneOptimizer:export_to_compile_destination(dest, optimized_cutscene_path)
	-- TODO: Export to destination(dest, optimized_cutscene_path) -> dest:update(...) etc.
...
function CoreCutsceneOptimizer:_write_cutscene_xml(path, animation_blobs)
	...
	if not table.empty(self:_all_controlled_unit_names(true)) then
		...
		local add_controlled_unit = function(unit_type, unit_name)
			...
			-- HACK: If animation patches are applied, use the "patched" blend_set for playback on that Unit.
		...
		for _, unit_name in ipairs(self:_all_controlled_unit_names()) do
			local unit_type = self:_all_controlled_unit_types()[unit_name] -- We do it this way to ensure output is sorted by name.
	...
	if self:_has_cameras() then
		CoreCutsceneKey:create(CoreChangeCameraCutsceneKey.ELEMENT_NAME):_save_under(keys_node) -- Add a camera change to the default camera at frame 0.
	end
...
function CoreCutsceneOptimizer:_add_unit_visibility_keys(keys_node)
	-- Insert unit visibility keys at clip joins.
	-- Assumes that everything is visible when the cutscene starts.
	...
	for _, clip in ipairs(self.__clips) do
		...
		for _, unit_name in ipairs(unit_names) do
			local existing_visibility_key = table.find_value(self.__cutscene_keys, function(key) return key:frame() == clip:start_time() and key.ELEMENT_NAME == CoreUnitVisibleCutsceneKey.ELEMENT_NAME and key:unit_name() == unit_name end)
			if existing_visibility_key then
				...
				if visible ~= was_visible[unit_name] then
					...
					visibility_key.is_valid_unit_name = function() return true end -- Monkey-patch validity test so we can write out our new cutscene key.
...
function CoreCutsceneOptimizer:_add_discontinuity_keys(keys_node)
	-- Insert discontinuity keys at clip joins where a segment of time has been cut out.
	...
	for _, clip in ipairs(self.__clips) do
		if previous_clip == nil or clip:metadata():footage() ~= previous_clip:metadata():footage() or clip:start_time_in_source() ~= previous_clip:end_time_in_source() then 
			local existing_discontinuity_key = table.find_value(self.__cutscene_keys, function(key) return key:frame() == clip:start_time() and key.ELEMENT_NAME == CoreDiscontinuityCutsceneKey.ELEMENT_NAME end)
...
function CoreCutsceneOptimizer:_write_cutscene_unit_xml(path)
	...
	local add_extension_node = function(name, class)
...
function CoreCutsceneOptimizer:_create_merged_animation()
	-- Cut toghether the animation data for each unit into a continuous sequence that spans the duration of the cutscene.
	-- Create a map of { unit name => the joined-up animation }
	...
	if self:_has_cameras() then
		-- For the camera animation, we only sample data from the currently active camera.
		unit_animation_map["camera"] = self:_get_joined_camera_animation()
	end
	
	-- Merge the animations for each unit into a single animation controlling all units at once.
	-- In order to do this, bone names are prefixed with the unit name so they won't collide.
...
function CoreCutsceneOptimizer:_write_animation_blobs(full_animation, dest, part_path_pattern)
	-- Commit the animation to the database as platform-specific blobs.
	local animation_blob_names = {}
	local default_settings = { position_tolerance = 0.1, rotation_tolerance = 0.0025, pack_positions = false, pack_rotations = false }
...
function CoreCutsceneOptimizer:_write_animation_part(dest, path, animation, compressed_animation)
	...
	-- If the path contains platform properties, export only for those platforms.
	...
	if #platforms_to_export == 0 then
		-- Path does not mention any specific platforms - export for every platform.
