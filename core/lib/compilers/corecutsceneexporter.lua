...
function CoreCutsceneExporter:add_clip(clip)
	table.insert_sorted(self.__clips, clip, function(a, b) return a:start_time() < b:start_time() end)
	self:_clear_cached_lists()
end

function CoreCutsceneExporter:add_key(cutscene_key)
	table.insert_sorted(self.__cutscene_keys, cutscene_key, function(a, b) return a:frame() < b:frame() end)
	self:_clear_cached_lists()
end
...
function CoreCutsceneExporter:contains_optimized_footage()
	return table.find_value(self.__clips, function(clip) return clip:metadata():footage()._cutscene:is_optimized() end) ~= nil
end

function CoreCutsceneExporter:contains_unoptimized_footage()
	return table.find_value(self.__clips, function(clip) return not clip:metadata():footage()._cutscene:is_optimized() end) ~= nil
end
...
function CoreCutsceneExporter:_problem_map()
	local problem_map = {}
	local add_problem = function(problem) problem_map[problem] = true end
	
	if #self.__cutscene_keys == 0 and self:frame_count() == 0 then
		add_problem("The project contains no data.")
	else
		local previous_clip = responder_map{ end_time=0, start_time=0, metadata=responder_map{ is_valid=true } }
		...
		-- HACK: Disabled. This feature requires engine support.
		if false or table.empty(problem_map) then
...
function CoreCutsceneExporter:_all_controlled_unit_types(include_cameras)
	...
	return include_cameras and self.__all_controlled_unit_types or table.remap(self.__all_controlled_unit_types, function(unit_name, unit_type) return unit_name, not string.begins(unit_name, "camera") and unit_type or nil end)
end
...
function CoreCutsceneExporter:_get_final_animation(unit_name)
	self.__final_animation_cache = self.__final_animation_cache or setmetatable({}, { __mode = "v" }) -- Weak values mean these will get collected if they're not used for a while.

	local final_animation = self.__final_animation_cache[unit_name]
	if not alive(final_animation) then
		final_animation = self:_get_joined_animation(unit_name) or false -- We put a false into the lookup map to prevent multiple join attempts.
		...
		if final_animation and unit_type ~= nil and unit_type ~= "locator" then
			...
			-- Strip bones that will be replaced.
			do
				...
				local kept_bones = table.inject(replaced_blend_sets, original_bones, function(result, blend_set)
					...
					return table.find_all_values(result, function(bone) return not table.contains(blend_set_bones, bone) end)
				end)
				
				final_animation = self:_process_animation("strip", final_animation, kept_bones)
			end
			
			-- Merge in the replacement animations.
			for blend_set, animation_name in pairs(patches) do
				...
				-- Make sure the patch animation is the same length as the final animation.
...
function CoreCutsceneExporter:_get_joined_animation(unit_name_or_func)
	self.__joined_animation_cache = self.__joined_animation_cache or setmetatable({}, { __mode = "v" }) -- Weak values mean these will get collected if they're not used for a while.
	
	local unit_name_func = type(unit_name_or_func) ~= "function" and function() return tostring(unit_name_or_func) end or unit_name_or_func
	...
	if not alive(joined_animation) then
		joined_animation = self:_join_animations(unit_name_func) or false -- We put a false into the lookup map to prevent multiple join attempts.
		if unit_name then
...
function CoreCutsceneExporter:_get_joined_camera_animation()
	return self:_get_joined_animation(function(clip) return clip and clip:metadata():camera() end)
end

function CoreCutsceneExporter:_get_footage_animation(cutscene, unit_name)
	self.__footage_animation_cache = self.__footage_animation_cache or setmetatable({}, { __mode = "v" }) -- Weak values mean these will get collected if they're not used for a while.
	...
	if not alive(footage_animation) then
		...
		footage_animation = footage_animation_name and AnimationCutter:load(footage_animation_name) or false -- We put a false into the lookup map to prevent multiple load attempts.

		-- Before adding it to the cache, strip animation on bones that are not found in the model.
...
function CoreCutsceneExporter:_clear_cached_lists()
	-- Clear cached lists, forcing re-examination when requested.
	self.__problems = nil
