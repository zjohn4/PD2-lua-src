...
-- Preprocessor regex.
--table.insert(CoreLuaPreprocessor.preprocessors, {regx = "%-%-%sAndreas%sis%sthe%sbest!", sub = "cat_print(\"debug\", \"Andreas sux!\")", skip = {}})
...
function CoreLuaPreprocessor:preprocess(path, code)
	...
	if self.DEBUG and nr > 0 then cat_print( "spam", out) end
	return out
end
