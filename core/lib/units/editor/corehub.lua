...
function CoreHub:init( unit )
	...
	self._hed.actions_data = {} -- Used by hub unit to hold data for each of its actions
	self._hed.triggers_data = {} -- Used by hub unit to keep be able to save trigger unit ids
	self._hed.default_test_hub = false
	self._hed.use_as_start = false
	self._hed.never_start = false
	
	self._hed.start_from_relay = "none" -- Has to be core right now
	self._hed.hub_entered_from_zone = "none" -- Has to be core right now
...
function CoreHub:set_actions()
	if not self._actions_ctrlrs then -- The gui hasn't been created yet
		return
	end
...
-- Returns a sorted table with action names
...
function CoreHub:set_triggers()
	if not 	self._triggers_ctrlrs then -- The gui hasn't been created yet
		return
	end
...
-- Returns a sorted table with trigger names
...
function CoreHub:set_required_triggers()
	...
	if self._hed.required_triggers ~= "all" then
		if tonumber( self._hed.required_triggers ) > #self._hed.triggers-1 then
...
function CoreHub:id_string( unit )
	...
	-- return unit:unit_data().name_id..' ('..unit:unit_data().unit_id..')'
	-- return unit:name()..' '..unit:unit_data().unit_id
end

function CoreHub:save_mission_action( file, t, hub )
	HubElement.save_mission_action( self, file, t, hub, true ) -- Last parameter will prevent the save to save all the values
end
...
function CoreHub:layer_finished()
	...
	for key,value in pairs( hed.triggers_data ) do
		local v = value
		if type_name( value ) == "number" then -- New data with id in value, or value is id
			v = { unit_id = v, type = "" }
		end
...
function CoreHub:add_action( a )
	...
	self._hed.actions_data[ s ] = { unit_id = a:unit_data().unit_id, action_delay = 0, type = "" }
...
function CoreHub:add_trigger( t )
	...
	self._hed.triggers_data[ s ] = { unit_id = t:unit_data().unit_id, type = "" }
...
function CoreHub:_build_panel()
	...
	use_as_start_cb:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = use_as_start_cb, value = "use_as_start" } )
	...
	trigger_once_cb:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = trigger_once_cb, value = "trigger_once" } )
	...
	trigger_on_inverse_cb:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = trigger_on_inverse_cb, value = "trigger_on_inverse" } )
	...
	local actions_sizer = EWS:StaticBoxSizer( self._panel, "VERTICAL", "Actions")
		...
		local action_delay_sizer = EWS:BoxSizer( "HORIZONTAL" )
			...
			-- action_delay:connect( "EVT_COMMAND_TEXT_UPDATED", callback( self, self, "set_element_data" ), { ctrlr = action_delay, callback = "set_action_delay" } )
			action_delay:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = action_delay, callback = "set_action_delay" } )
			action_delay:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = action_delay, callback = "set_action_delay" } )
		
		actions_sizer:add( action_delay_sizer, 0, 0, "EXPAND" )
	
	self._panel_sizer:add( actions_sizer, 0, 0, "EXPAND" )
	
	self._actions_ctrlrs = { actions = actions, action_delay = action_delay, action_types = action_types }
	...
	local triggers_sizer = EWS:StaticBoxSizer( self._panel, "VERTICAL", "Triggers" )
		...
		local required_trigger_sizer = EWS:BoxSizer( "HORIZONTAL" )
			...
			self._required_triggers:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = self._required_triggers, value = "required_triggers" } )	
