...
function CoreLogicChanceOperatorUnitElement:draw_links( t, dt, selected_unit, all_units )
	...
	for _,id in ipairs( self._hed.elements ) do
		...
		if draw then
			-- Application:draw_link( { from_unit = self._unit, to_unit = unit,  r = 0.75, g = 0.75, b = 0.25 } )
...
function CoreLogicChanceOperatorUnitElement:add_element()
	...
	if ray and ray.unit then
		if ray.unit:name() == Idstring( "core/units/mission_elements/logic_chance/logic_chance" ) then
...
function CoreLogicChanceOperatorUnitElement:_build_panel( panel, panel_sizer )
	...
	local operation_params = {
		...
		sizer 				= panel_sizer,
		default				= "none",
		options				= { "add_chance", "subtract_chance", "reset", "set_chance" },
