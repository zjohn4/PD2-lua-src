...
function CorePhysicsPushUnitElement:_build_panel( panel, panel_sizer )	
	...
	local range_sizer = EWS:StaticBoxSizer( panel, "VERTICAL", "Range" )
		...
		range:connect( "EVT_SCROLL_CHANGED", callback( self, self, "set_element_data" ), { ctrlr = range, value = "physicspush_range" } )
		range:connect( "EVT_SCROLL_THUMBTRACK", callback( self, self, "set_element_data" ), { ctrlr = range, value = "physicspush_range" } )
	...
	local velocity_sizer = EWS:StaticBoxSizer( panel, "VERTICAL", "Velocity" )
		...
		velocity:connect( "EVT_SCROLL_CHANGED", callback( self, self, "set_element_data" ), { ctrlr = velocity, value = "physicspush_velocity" } )
		velocity:connect( "EVT_SCROLL_THUMBTRACK", callback( self, self, "set_element_data" ), { ctrlr = velocity, value = "physicspush_velocity" } )
	...
	local mass_sizer = EWS:StaticBoxSizer( panel, "VERTICAL", "Mass" )
		...
		mass:connect( "EVT_SCROLL_CHANGED", callback( self, self, "set_element_data" ), { ctrlr = mass, value = "physicspush_mass" } )
		mass:connect( "EVT_SCROLL_THUMBTRACK", callback( self, self, "set_element_data" ), { ctrlr = mass, value = "physicspush_mass" } )
...
function CorePhysicsPushUnitElement:add_to_mission_package()
	managers.editor:add_to_world_package( { category = "physic_effects", name = "core/physic_effects/hubelement_push", continent = self._unit:unit_data().continent } )
end
