...
-- Makes sure that the selected clip still exists in the selected sequence
...
function CoreWorldCameraTriggerUnitElement:_build_panel( panel, panel_sizer )	
	...
	local sequence_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		self._sequences:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = self._sequences, value = "worldcamera_trigger_sequence" } )
	...
	local after_clip_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		self._after_clip:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = self._after_clip, value = "worldcamera_trigger_after_clip" } )
