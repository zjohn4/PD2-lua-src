...
function CoreAreaUnitElement:init( unit )
	...
	self._move_shape_rep = RepKey:new( { "up", "down", "right", "left", "num 4", "num 6" } )
...
-- Populates the shapes list gui tih the shapes
...
-- Sets what shape type will be created when creating
function CoreAreaUnitElement:set_shape_type( types )
	self._shape_type = types:get_value()
end

-- Select a shape from the shape list
...
-- Sets the start altitude of the current shape (makes it 3D)
...
-- Sets the height of the current shape (makes it 3D)
...
-- Sets the shape back to be a 2D shape
...
-- Sets a size multiplier for the shape to use while changing the size
...
-- Sets the new size based on the size multiplier. It then sets the multiplier to one and resets the size slider
...
-- If there is a current shape the height, altitude and size sliders will be enabled and set accoring to shape values.
-- Otherwise it will disable the sliders and reset the values for them
...
-- Calls draw to visualize the shapes
function CoreAreaUnitElement:update_selected( time )	
	for _,shape in pairs( self._hed.shapes ) do
		self:draw( shape, 0.0, 0.5 + ((1+math.sin(time*100))*0.5)*0.5, 0.0 )
	end
	if self._current_shape then
...
-- The draw function visualizes a shape. It does it differently depending on type.
...
function CoreAreaUnitElement:update_editing( t, dt )
	...
	-- Update the settings of currently creating shape
	...
	-- Set rotation of a shape (only used by plane)
	if current_shape then
		local rot_axis
...
-- Only used to load old formated data type (was changed 2008-05-28)
function CoreAreaUnitElement:load_data( data )
	if not data then -- Using new save and load values
		return
	end
...
-- rounds the current position considering the grid size
...
-- Creates a property table for a shape based on start and end positions
-- The properies differs depending on shape type
...
-- Called when create button is released
function CoreAreaUnitElement:create_shape_release()
	self._creating = false
end

-- Starts grab move of shape(s)
...
function CoreAreaUnitElement:_build_panel( panel, panel_sizer )	
	...
	local types_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		for _,type in ipairs( { "on_enter", "on_exit", "toggle" } ) do
...
function CoreAreaUnitElement:_altitude_ctrlr( panel, name, value, type, sizer )
	local ctrl_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		spin:connect( "EVT_SCROLL_LINEUP", callback( self, self, "set_altitude_spin" ), { ctrl = ctrl, step = 1, type = type } )
		spin:connect( "EVT_SCROLL_LINEDOWN", callback( self, self, "set_altitude_spin" ), { ctrl = ctrl, step = -1, type = type } )
