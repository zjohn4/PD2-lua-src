...
function CoreSpawnUnitUnitElement:_build_panel( panel, panel_sizer )
	...
	velocity:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = velocity, value = "unit_spawn_velocity" } )
	velocity:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = velocity, value = "unit_spawn_velocity" } )
...
function CoreSpawnUnitUnitElement:add_to_mission_package()
	managers.editor:add_to_world_package( { category = "units", name = self._hed.unit_name, continent = self._unit:unit_data().continent } )
end
