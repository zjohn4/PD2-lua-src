...
function CoreRumbleHubElement:init( unit )
	...
	self._hed.rumble_peak = 1.0
	self._hed.rumble_attack = 1.0
	self._hed.rumble_sustain = 2.0
	self._hed.rumble_release = 1.0
...
function CoreRumbleHubElement:_build_panel( panel, panel_sizer )	
	...
	local rumble_sizer = EWS:BoxSizer( "HORIZONTAL" )
		...
		local engines_sizer = EWS:BoxSizer( "HORIZONTAL" )
			...
			for _,engine in ipairs( { "both", "left", "right" } ) do 
			...
			engines:connect( "EVT_COMMAND_COMBOBOX_SELECTED", callback( self, self, "set_element_data" ), { ctrlr = engines, value = "rumble_engine" } )
		...
		local peak_sizer = EWS:BoxSizer( "HORIZONTAL" )
			...
			peak:connect( "EVT_COMMAND_TEXT_UPDATED", callback( self, self, "set_element_data" ), { ctrlr = peak, value = "rumble_peak" } )
		...
		local attack_sizer = EWS:BoxSizer( "HORIZONTAL" )
			...
			attack:connect( "EVT_COMMAND_TEXT_UPDATED", callback( self, self, "set_element_data" ), { ctrlr = attack, value = "rumble_attack" } )
		...
		local sustain_sizer = EWS:BoxSizer( "HORIZONTAL" )
			...
			sustain:connect( "EVT_COMMAND_TEXT_UPDATED", callback( self, self, "set_element_data" ), { ctrlr = sustain, value = "rumble_sustain" } )
		...
		local release_sizer = EWS:BoxSizer( "HORIZONTAL" )
			...
			release:connect( "EVT_COMMAND_TEXT_UPDATED", callback( self, self, "set_element_data" ), { ctrlr = release, value = "rumble_release" } )
