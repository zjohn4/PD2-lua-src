...
function CoreAiArea:spawn(unit_name, spawn_point_name)
	...
	local ray = World:raycast("ray", spawn_point:position() + spawn_point:rotation():z() * 100.0, spawn_point:position() - spawn_point:rotation():z() * 500.0, "slot_mask", managers.slot:get_mask("statics")) --should be "bosses"
	if(ray) then
...
function CoreSpawnSystem:init( unit )
	...
	--self._linked_unit_map = nil
	--self._enabled_unit_map = nil
			...
			if( ( lv1_element_name == "socket" ) and ( lv1_name ~= "" ) ) then
			    for lv2 in lv1:children() do
			        if( lv2:name() == "unit" ) then
						self._enabled_unit_map = self._enabled_unit_map or {}	-- Socket map
...
-- This is slow, so use it sparely (works only on sockets):
