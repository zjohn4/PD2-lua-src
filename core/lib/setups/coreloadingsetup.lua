...
C o r e L o a d i n g S e t u p
-------------------------------

CoreLoadingSetup provides the basics hooks for the loading
environment (which is executed on a separate thread with
separate Lua memory). The loading environment is meant to be
lightweight so the full CoreSystem is not required. Instead
there is a separate implementation of 'class' and 'callback'
