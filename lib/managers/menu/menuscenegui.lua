...
function MenuSceneGui:update( t, dt )
	if self._shown_infamy_text_t then
		if self._shown_infamy_text_t <= t then
			...
			self._infamy_increased_text = self._panel:text( { text = managers.localization:to_upper_text( current_rank == 0 and "menu_infamy_rank_reached" or "menu_infamy_rank_increased", { infamy_rank = tostring( current_rank + 1 ) } ), font = tweak_data.menu.pd2_large_font, font_size = tweak_data.menu.pd2_large_font_size, align = "center", vertical = "top", layer = 1 } )
