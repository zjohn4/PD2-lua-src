...
--[[
	...
	for tier, tier_data in ipairs( data.tiers ) do
		...
		if add_infamy_glow then
			local glow = tier_panel:bitmap( { name = "cost_glow", w = 56, h = 56, texture = "guis/textures/pd2/crimenet_marker_glow", blend_mode = "add", color = tweak_data.screen_colors.button_stage_3, rotation = 360 } )
	...
	if( self._selected_item and self._selected_item._skill_panel ) then
		...
		if self._selected_item._tier then
			if not unlocked then
				...
				prerequisite_text = prerequisite_text .. managers.localization:text( points_needed == 1 and "st_menu_points_to_unlock_tier_singular" or "st_menu_points_to_unlock_tier", {points=points_needed, tier=self._selected_item._tier} ) .. "\n"
			end
