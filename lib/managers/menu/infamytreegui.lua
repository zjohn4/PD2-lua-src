...
function InfamyTreeGui:_setup()
	...
	local title_text = self._panel:text( { name = "infamytree_text", text = utf8.to_upper( managers.localization:text( "menu_infamytree" ) ), align = "left", vertical = "top", h = tweak_data.menu.pd2_large_font_size, font_size = tweak_data.menu.pd2_large_font_size, font = tweak_data.menu.pd2_large_font, color = tweak_data.screen_colors.text } )
	
	local title_bg_text = self._fullscreen_panel:text( { name = "infamytree_text", text = utf8.to_upper( managers.localization:text( "menu_infamytree" ) ), h = 90, align = "left", vertical = "top", font_size = tweak_data.menu.pd2_massive_font_size, font = tweak_data.menu.pd2_massive_font, color = tweak_data.screen_colors.button_stage_3, alpha = 0.4, blend_mode = "add", layer = 1 } )
	...
	BoxGuiObject:new( self._tree_panel, { sides = { 1, 1, 1, 1 } } )
	...
	self._description_panel:text( { name = "description_title", text = "", x = 10, y = 10, layer = 1, font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, align = "left", vertical = "top", halign = "scale", valign = "scale", wrap = true, word_wrap = true, color = tweak_data.screen_colors.text } )
	self._description_panel:text( {name = "description_warning", text = "", x = 10, y = 10, layer = 1, font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, align = "left", vertical = "top", halign = "scale", valign = "scale", wrap = true, word_wrap = true, color = tweak_data.screen_colors.important_1 } )
	self._description_panel:text( { name = "description_text", text = "", x = 10, y = 10, layer = 1, font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, align = "left", vertical = "top", halign = "scale", valign = "scale", wrap = true, word_wrap = true, color = tweak_data.screen_colors.text, h = tweak_data.menu.pd2_small_font_size + 10 } )
	
	BoxGuiObject:new( self._description_panel, { sides = { 1, 1, 1, 1 } } )
	...
	local y = y
	local item = { panel = self._tree_panel:panel( { x = x, y = y, w = item_width, h = item_height } ) }
	...
	for count = 1, tree_rows * tree_cols do
		item = { panel = self._tree_panel:panel( { x = x, y = y, w = item_width, h = item_height } ) }
		table.insert( self._tree_items, item )
		
		x = x + item_width
		if self._tree_panel:w() <= x then
			x = 0
			y = y - item_height
		end
	end
	...
	for index, item in pairs( self._tree_items ) do
		if tweak_data.infamy.tree[ index ] then
			...
			item.panel:bitmap( { name = "image", x = pos, y = border_size, w = size, h = size, texture = "guis/dlcs/infamous/textures/pd2/infamous_tree/infamous_tree_atlas", texture_rect = { texture_rect_x * 128, texture_rect_y * 128, 128, 128 }, color = color, layer = 1, blend_mode = "add" } )
			item.panel:text( { name = "text", x = 0, y = item_height - tweak_data.menu.pd2_small_font_size - border_size, w = item_width, text = "", layer = 1, wrap = false, word_wrap = false, font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, align = "center" } )
			
			local glow = item.panel:bitmap( { name = "unlock_glow", w = size * 2, h = size * 2, texture = "guis/textures/pd2/crimenet_marker_glow", blend_mode = "add", color = tweak_data.screen_colors.button_stage_3, rotation = 360 } )
			...
			item.owned = false
			item.unlocked = false
			item.panel:bitmap( { name = "image", x = pos, y = border_size, w = size, h = size, texture = "guis/dlcs/infamous/textures/pd2/infamous_tree/infamous_tree_atlas", texture_rect = { texture_rect_x * 128, texture_rect_y * 128, 128, 128 }, color = tweak_data.screen_colors.item_stage_3, layer = 1, blend_mode = "add" } )
			item.panel:text( { name = "text", x = 0, y = item_height - tweak_data.menu.pd2_small_font_size - border_size, w = item_width, text = "", layer = 1, wrap = false, word_wrap = false, font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, align = "center" } )
		end
		
		item.border = BoxGuiObject:new( item.panel, { sides = { 0, 0, 0, 0 } } )
	end
	
	
	
	local points_text = self._panel:text( { name = "points", text = utf8.to_upper( managers.localization:text( "st_menu_infamy_available_points", { points = managers.infamy:points() } ) ), layer = 1, wrap = false, word_wrap = false, font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, color = tweak_data.screen_colors.text, align = "right" } )
	...
	if managers.menu:is_pc_controller() then
		local back_text = self._panel:text( { name = "back_button", text = utf8.to_upper( managers.localization:text( "menu_back" ) ), align = "right", vertical = "bottom", h = tweak_data.menu.pd2_large_font_size, font_size = tweak_data.menu.pd2_large_font_size, font = tweak_data.menu.pd2_large_font, blend_mode = "add", color = tweak_data.screen_colors.button_stage_3 } )
...
function InfamyTreeGui:_update_description( name, unlocked )
	...
	if name then
		...
		local infamy_tweak = tweak_data.infamy.items[ name ]
		local category_list = { mask = "masks", texture = "textures", material = "materials", color = "colors" }
		...
		for bonus, item in ipairs( infamy_tweak.upgrades ) do
			...
			if category == "colors" then
				desc_upgrade:bitmap( { name = item_id.."_color_bg", x = 10, y = y, w = icon_size, h = icon_size, texture = "guis/textures/pd2/blackmarket/icons/colors/color_bg", layer = 2 } )
				desc_upgrade:bitmap( { name = item_id.."_color_1", x = 10, y = y, w = icon_size, h = icon_size, texture = "guis/textures/pd2/blackmarket/icons/colors/color_02", layer = 1, color = tweak_data.blackmarket.colors[ item_id ].colors[1] } )
				desc_upgrade:bitmap( { name = item_id.."_color_2", x = 10, y = y, w = icon_size, h = icon_size, texture = "guis/textures/pd2/blackmarket/icons/colors/color_01", layer = 1, color = tweak_data.blackmarket.colors[ item_id ].colors[2] } )
			elseif category == "textures" then
				...
				if DB:has( Idstring( "texture" ), icon_texture ) then
					...
					local panel = desc_upgrade:panel( { name = item_id.."_image", x = 10, y = y, w = icon_size, h = icon_size, layer = 1 } )
					local texture_count = managers.menu_component:request_texture( icon_texture, callback( self, self, "_texture_done_clbk", { panel = panel, render_template = "VertexColorTexturedPatterns" } ) )
				...
				if DB:has( Idstring( "texture" ), icon_texture ) then
					if category == "materials" then
						...
						desc_upgrade:bitmap( { name = item_id.."_image", x = 10, y = y, w = icon_size, h = icon_size, texture = icon_texture, layer = 1 } )
					end
				end
			end
			
			local icon_text = desc_upgrade:text( { name = item_id .. "_text", x = icon_size + 20, y = y, h = icon_size, layer = 1, wrap = false, word_wrap = false, font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, vertical = "center" } )
...
function InfamyTreeGui:_texture_done_clbk( params, texture_ids )
	if alive( params.panel ) then
		params.panel:bitmap( { texture = texture_ids, w = params.panel:w(), h = params.panel:h(), render_template = params.render_template } )
	end
end
...
function InfamyTreeGui:_select_item( index )
	if self._selected_item ~= index then
		if self._selected_item then
			...
			selected_item.border:create_sides( selected_item.panel, { sides = { 0, 0, 0, 0 } } )
		...
		item.border:create_sides( item.panel, { sides = { 2, 2, 2, 2 } } )
