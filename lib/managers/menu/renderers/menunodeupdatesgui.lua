	...
	if self._tweak_data.db_url then
		if self._panel then
			...
			if alive( desc_text ) then
				local bitmap = self._panel:bitmap( { texture = "guis/textures/pd2/endscreen/exp_ring", color = Color( 0.1, 1, 1 ), alpha = 0.75, w = 32, h = 32, layer = 2, render_template = "VertexColorTexturedRadial" } )
				bitmap:set_position( desc_text:position() )
				bitmap:animate( animate_loading_texture )
				
				
				local loading_text = self._panel:text( { text = managers.localization:to_upper_text( "menu_http_loading" ), font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, color = tweak_data.screen_colors.text } )
...
function MenuNodeUpdatesGui:setup()	
	...
	if self._tweak_data.button then
		local top_button = panel:panel( { name = "top_button", w = 32, h = 32 } )
		
		local w, h = 0, 0
		if self._tweak_data.button.text_id then
			local text = top_button:text( { text = managers.localization:to_upper_text( self._tweak_data.button.text_id ), align = "right", vertical = "top", font_size = tweak_data.menu.pd2_medium_font_size, font = tweak_data.menu.pd2_medium_font, color = tweak_data.screen_colors.button_stage_3, halign = "right", valign = "top" } )
		...
		if self._tweak_data.button.image then
			local bitmap = top_button:bitmap( { texture = self._tweak_data.button.image, color = tweak_data.screen_colors.button_stage_3, halign = "right", valign = "top" } )
			bitmap:set_top( 0 )
	...
	local previous_updates_panel = panel:panel( { name = "previous_content_updates", w = w, h = small_width / 2 + self.PADDING * 2, x = 0, y = latest_update_panel:bottom() + 30 } )
	
	
	local previous_update_text = panel:text( { name = "previous_update_text", text = self._tweak_data.choice_id and managers.localization:to_upper_text( self._tweak_data.choice_id ) or "", font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, color = tweak_data.screen_colors.text } )
	...
	for index, data in ipairs( previous_updates ) do
		...
		local text = panel:text( { name = data.name_id, text = text_string, font = tweak_data.menu.pd2_small_font, font_size = tweak_data.menu.pd2_small_font_size, color = tweak_data.screen_colors.text } )
		self:make_fine_text( text )
