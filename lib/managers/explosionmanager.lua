...
function ExplosionManager:add_sustain_effect( effect_id, sustain_time )
	table.insert( self._sustain_effects, { id = effect_id, expire_t = Application:time() + sustain_time } )
end

function ExplosionManager:give_local_player_dmg( pos, range, damage )
	local player = managers.player:player_unit()
	if player then
		player:character_damage():damage_explosion( { position = pos, range = range, damage = damage } )
	end
end
...
function ExplosionManager:detect_and_give_dmg( params )
	...
	-- if user_unit then
	managers.groupai:state():propagate_alert( { "explosion", hit_pos, 10000, alert_filter, user_unit } )
	-- end
...
--			print( "final damage", hit_unit, damage )
	...
	if owner then
		...
		managers.statistics:shot_fired( { hit = next( characters_hit ) and true or false, weapon_unit = owner } )
	end
...
function ExplosionManager:player_feedback( position, normal, range, custom_params )
	...
	if player then
		...
		local params = {
			"camera_shake", "multiplier", camera_shake_mul,
			"camera_shake", "amplitude", 0.5,
			"camera_shake", "attack", 0.05,
			"camera_shake", "sustain", 0.15,
			"camera_shake", "decay", 0.5,
...
function ExplosionManager:project_decal( ray, from, to, on_unit )
	...
	if ray then
		...
		if redir_name ~= empty_idstr then
			World:effect_manager():spawn( { effect = redir_name, position = ray.position, normal = ray.normal } )
		end
		
		local id = World:effect_manager():spawn( { effect = Idstring( "effects/particles/fire/small_light_fire" ), position = ray.position, normal = ray.normal } )
