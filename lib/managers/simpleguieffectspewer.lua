...
function SimpleGUIEffectSpewer:_spew_update( t, dt )
	...
	while self.__particle_spawn_t <= 0 do
		if self._max_num_particles == 0 or self._max_num_particles > table.size( self.__living_particles ) then
			...
			rot = rot == 0 and 360 or rot
			local new_particle = self._particle_panel:bitmap( { texture = self._particle_textures[ math.random( #self._particle_textures ) ], rotation = rot, blend_mode = self._particle_blend_mode } )
			...
			if self._particle_colors then
				local red_data = self._particle_colors.red or { 1, 1 }
				local green_data = self._particle_colors.green or { 1, 1 }
				local blue_data = self._particle_colors.blue or { 1, 1 }
			...
			self.__living_particles[ new_particle:key() ] = { particle = new_particle, w = w, h = h, dir_x = dir_x, dir_y = dir_y, particle_speed = particle_speed, plive_time = plive_time, pt = 0, start_rotation = rot, gravity = 0, sway_distance = sway_distance, sway_speed = sway_speed, sway_t = math.rand( 1 ), flip_speed = particle_flip_speed, flip_dir = math.random() < 0.5 and -1 or 1 }
		end
...
function SimpleGUIEffectSpewer:init( params )
	params = params or { spew_time = 1, plive_time = 1, spawn_interval = 0.1 }
	...
	self._spew_panel = self._spew_parent_panel:panel( { x = self._spew_panel_x - 5, y = self._spew_panel_y - 5, w = 9, h = 9, layer = self._layer } )
...
function SimpleGUIEffectSpewer.sample_boom()
	...
	SimpleGUIEffectSpewer:new( {	spew_time = 0.5,
																min_plive_time = 0.6,
																max_plive_time = 1,
																spawn_interval = 0.04,
	...
	SimpleGUIEffectSpewer:new( {	spew_time = 0,
																spew_at_start = true,
																plive_time = 1.6,
																particle_speed = 0,
																max_num_particles = 1,
																spawn_interval = 0.1,
																particle_colors = { red = 0.8, green = 0.8, blue = 0.1 },
	...
	SimpleGUIEffectSpewer:new( {	spew_time = 0,
																...
																max_num_particles = 1,
																spawn_interval = 0.1,
																particle_colors = { red = 0.8, green = 0.5, blue = 0.1 },
...
function SimpleGUIEffectSpewer.get_skill_spewers( x, y, panel )
	...
	local colors = { red = { skill_color.red - 0.1, skill_color.red + 0.1 }, green = { skill_color.green - 0.1, skill_color.green + 0.1 }, blue = { skill_color.blue - 0.1, skill_color.blue + 0.1 } }
