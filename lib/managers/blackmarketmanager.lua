...
function BlackMarketManager:_setup_melee_weapons()
	...
	for melee_weapon, _ in pairs( tweak_data.blackmarket.melee_weapons ) do
		melee_weapons[ melee_weapon ] = { unlocked = false, equipped = false, owned = false, durability = 1, level = 0, skill_based = false }
...
function BlackMarketManager:aquire_default_weapons( only_enable )
	...
	if melee_weapon and not melee_weapon.unlocked then
		if not managers.upgrades:aquired( self._defaults.melee_weapon ) then
			if only_enable then
