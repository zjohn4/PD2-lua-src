...
function MissionAssetsManager:sync_unlock_asset( asset_id, peer )
	...
	if managers.chat and peer then
		...
		if asset_tweak_data then
			managers.chat:feed_system_message( ChatManager.GAME, managers.localization:text( "menu_chat_peer_unlocked_asset", { name = peer:name(), asset = managers.localization:text( asset_tweak_data.name_id ) } ) )
		end
	end
end
