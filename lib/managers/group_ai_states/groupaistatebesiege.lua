...
function GroupAIStateBesiege:_spawn_in_group( spawn_group, spawn_group_type, grp_objective, ai_task )
	...
	local _get_special_unit_type_count = function( special_type )
	...
	while i <= #valid_unit_types do
		...
		if( cat_data.special_type ) and
			( cat_data.max_amount ) and
			( _get_special_unit_type_count( cat_data.special_type ) + ( spawn_entry.amount_min or 0 ) > cat_data.max_amount )
		then
	...
	while wanted_nr_units > 0 and #valid_unit_types ~= 0 do
		...
		if( cat_data.special_type ) and
			( cat_data.max_amount ) and
			( _get_special_unit_type_count( cat_data.special_type ) >= cat_data.max_amount )
		then
