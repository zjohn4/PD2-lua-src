...
function SpecialObjectiveGroupElement:add_unit_list_btn()
	...
	local f = function( unit )
		...
		if string.find( unit:name():s(), "ai_enemy_group", 1, true ) or
			string.find( unit:name():s(), "ai_spawn_enemy", 1, true ) or
			string.find( unit:name():s(), "ai_civilian_group", 1, true ) or
			string.find( unit:name():s(), "ai_spawn_civilian", 1, true ) or
			string.find( unit:name():s(), "point_special_objective", 1, true ) or
			string.find( unit:name():s(), "ai_so_group", 1, true ) then
			return true
		end
		return false
	...
	for _, unit in ipairs( dialog:selected_units() ) do
		...
		if string.find( unit:name():s(), "ai_enemy_group", 1, true ) or
			string.find( unit:name():s(), "ai_spawn_enemy", 1, true ) or
			string.find( unit:name():s(), "ai_civilian_group", 1, true ) or
			string.find( unit:name():s(), "ai_spawn_civilian", 1, true )
		then
			self._hed.spawn_instigator_ids = self._hed.spawn_instigator_ids or {}
			table.insert( self._hed.spawn_instigator_ids, id )
		else
			if string.find( unit:name():s(), "point_special_objective", 1, true ) or
				string.find( unit:name():s(), "ai_so_group", 1, true )
			then
				self._hed.followup_elements = self._hed.followup_elements or {}
				table.insert( self._hed.followup_elements, id )
			end
...
function SpecialObjectiveGroupElement:remove_unit_list_btn()
	local f = function( unit ) return	( self._hed.spawn_instigator_ids and table.contains( self._hed.spawn_instigator_ids, unit:unit_data().unit_id ) )
																		or ( self._hed.followup_elements and table.contains( self._hed.followup_elements, unit:unit_data().unit_id ) ) end
