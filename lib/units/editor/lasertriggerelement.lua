...
LaserTriggerUnitElement.ON_EXECUTED_ALTERNATIVES = { "enter", "leave", "empty", "while_inside" }
LaserTriggerUnitElement.USES_INSTIGATOR_RULES = true
LaserTriggerUnitElement.CLOSE_DISTANCE = 25
LaserTriggerUnitElement.COLORS = { red = { 1, 0, 0 }, green = { 0, 1, 0 }, blue = { 0, 0, 1 } }
	...
	if self._creating_connection then
		if self._creating_connection == index then
			...
			if not self:_check_remove_connection( self._creating_connection, index ) then
				...
				table.insert( self._hed.connections, { from = self._creating_connection, to = index } )
...
function LaserTriggerUnitElement:_build_panel( panel, panel_sizer )
	...
	visual_only:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = visual_only, value = "visual_only" } )
	...
	skip_dummies:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = skip_dummies, value = "skip_dummies" } )
	...
	cycle_interval:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data"), { ctrlr = cycle_interval, value = "cycle_interval" } )
	cycle_interval:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = cycle_interval, value = "cycle_interval" } )
	...
	cycle_active_amount:connect( "EVT_COMMAND_TEXT_ENTER", callback( self, self, "set_element_data" ), { ctrlr = cycle_active_amount, value = "cycle_active_amount" } )
	cycle_active_amount:connect( "EVT_KILL_FOCUS", callback( self, self, "set_element_data" ), { ctrlr = cycle_active_amount, value = "cycle_active_amount" } )
	...
	cycle_random:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = cycle_random, value = "cycle_random" } )
