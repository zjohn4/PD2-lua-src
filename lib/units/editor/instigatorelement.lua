...
function InstigatorOperatorUnitElement:add_element()
	...
	if ray and ray.unit then
		if ray.unit:name() == Idstring( "units/dev_tools/mission_elements/logic_instigator/logic_instigator" ) then
...
function InstigatorOperatorUnitElement:_build_panel( panel, panel_sizer )
	...
	local operation_params = {
		...
		sizer = panel_sizer,
		default = "none",
		options = { "set", "clear", "add_first", "add_last", "use_first", "use_last", "use_random", "use_all" },
	...
	keep_on_use:connect( "EVT_COMMAND_CHECKBOX_CLICKED", callback( self, self, "set_element_data" ), { ctrlr = keep_on_use, value = "keep_on_use" } )
...
function InstigatorTriggerUnitElement:_build_panel( panel, panel_sizer )
	...
	local trigger_type_params = {
		...
		options = { "death", "set", "changed", "cleared" },
