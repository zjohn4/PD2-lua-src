...
function SentryGunBase:check_interact_blocked( player )
	...
	local result =
		( not alive( self._unit ) ) or
		( not self._unit:character_damage():dead() or true ) and
		( self._unit:weapon():ammo_ratio() == 1 ) or
		
		( not self:get_net_event_id( player ) ) or
		false
...
local refill_ratios = { 1, 0.75, 0.5, 0.25, 0.1 }
