...
function CopDamage:damage_melee( attack_data )
	...
	if result.type == "death" then
		local data = { name = self._unit:base()._tweak_table, head_shot = head, weapon_unit = attack_data.weapon_unit, name_id = attack_data.name_id, variant = attack_data.variant }
		managers.statistics:killed_by_anyone( data )
		...
		if attack_data.attacker_unit == managers.player:player_unit() then
			...
			if not is_civlian and managers.groupai:state():whisper_mode() then
			...
			if not is_civlian and not self:_type_gangster( self._unit:base()._tweak_table ) then
				if Global.game_settings.level_id == "nightclub" then
					if attack_data.name_id and attack_data.name_id == "fists" then
