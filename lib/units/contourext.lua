...
function ContourExt:add( type, marking_strength, time_multiplier )
	...
	table.insert( self._contour_list, { type = type, marking_strength = marking_strength, opacity = opacity, target_opacity = opacity and 0 } )
	self:_change()
end
...
function ContourExt:_change()
	...
	for key, data in ipairs( self._contour_list ) do
		if not id or self._types[ data.type ].priority < priority then
			id = key
