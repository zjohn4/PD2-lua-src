...
function DigitalGui:setup()
	...
	if self.BG_COLOR then
		self._bg_rect = self._panel:rect( { color = self.BG_COLOR, layer = -1 } )
	end
	
	local font_size = self.FONT_SIZE
	self._title_text = self._panel:text( { text = "01:23", y = 0, font = self.FONT, align = "center", vertical = "center", font_size = font_size, layer = 0, visible = true, color = self.DIGIT_COLOR } )
	...
	if self.TYPE == "timer" then
		if not self._timer_paused then
			...
			if Network:is_server() then
				if self._next_timer_sync < Application:time() then
...
function DigitalGui:set_bg_color_type( type )
	...
	if self.BG_COLOR then
		self._bg_rect = self._bg_rect or self._panel:rect( { color = self.BG_COLOR, layer = -1 } )
